...
const GrokLogo = ({ className = "w-5 h-5", active = false }: { className?: string; active?: boolean }) => (
    <svg viewBox="0 0 24 24" fill="none" className={className}>
        ...
        <path
            ...
            fill={active ? "rgba(255,255,255,0.3)" : "transparent"}
        />
    </svg>
);
    ...
    const generateScriptsWithAI = async () => {
        ...
        try {
            ...
            const middleSceneIdeas = [
                "เจาะลึกจุดเด่น/รีวิวเพิ่มเติม",
                "พิสูจน์ผลลัพธ์/เปรียบเทียบ",
                "โชว์การใช้งานจริง/สถานการณ์ต่างๆ",
                "เล่าประสบการณ์ผู้ใช้/รีวิวจริง",
                "เจาะรายละเอียดเพิ่ม/ฟีเจอร์เด่น",
                "ตอบคำถามที่พบบ่อย/ข้อสงสัย",
                "โชว์ before-after/ผลลัพธ์ที่เห็นได้ชัด",
                "เปรียบเทียบกับตัวเลือกอื่น/ทำไมต้องตัวนี้",
            ];
            ...
            if (sceneCount === 1) {
                ...
                // 4-10 scenes: hook → demo → middle scenes → CTA
...
🔑 คำสำคัญที่ต้องใส่ในสคริปต์: ${mustUseKeywords.trim()}` : ''}
${avoidKeywords.trim() ? `

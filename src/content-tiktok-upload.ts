...
const uploadVideoFile = async (videoUrl: string, useCache = false): Promise<boolean> => {
  ...
  // ── Strategy 1: Get pre-cached video from service worker ──
  if (useCache) {
...
const addHashtags = async (hashtags: string[]): Promise<boolean> => {
  ...
  for (const tag of hashtags) {
    ...
    await delay(500);
    // Press space or Enter to confirm hashtag
...
const addProductLink = async (productId: string): Promise<boolean> => {
  ...
  // Scroll down to find "เพิ่มลิงก์" section
  const scrollContainer = document.querySelector('main') || document.documentElement;
  for (let i = 0; i < 5; i++) {
    scrollContainer.scrollTop += 300;
    await delay(500);
    const addLinkBtn = findByText('เพิ่มลิงก์') || findByText('+ เพิ่ม') || findByText('Add link');
    if (addLinkBtn) break;
  }

  // Click "เพิ่มลิงก์" or the link section
  const addLinkBtn = findByText('เพิ่มลิงก์') || findByText('+ เพิ่ม') || findByText('Add link');
  if (!addLinkBtn) {
  ...
  // The dialog appears with "ประเภทของลิงก์" → "สินค้า" should be default
  // Click "ถัดไป" (Next)
  const nextBtn1 = await waitForElement(
    () => findByText('ถัดไป', 'button') || findByText('Next', 'button'),
    5000
  );
  if (nextBtn1) {
    ...
    warn('Could not find ถัดไป button for link type');
    return false;
  }

  // Product selection page - find product by ID
  // Wait for product list to load
  await waitForElement(
    () => {
      const text = document.body.textContent || '';
      return text.includes(productId) ? document.body : null;
    },
    10000
  );

  // Find the radio button or checkbox for our product
  let productSelected = false;

  // Strategy A: Find the row containing our product ID and click the radio/checkbox
  const allElements = Array.from(document.querySelectorAll('*'));
  for (const el of allElements) {
    const text = (el.textContent || '').trim();
    if (text === productId || text.includes(productId)) {
      // Walk up to find the row container
      let row: Element | null = el;
      for (let i = 0; i < 10 && row; i++) {
        row = row.parentElement;
        if (!row) break;
        // Look for checkbox/radio in this container
        const checkbox = row.querySelector('input[type="radio"], input[type="checkbox"], [role="radio"], [role="checkbox"]');
        if (checkbox) {
          clickElement(checkbox);
          productSelected = true;
          log('Selected product by checkbox/radio');
          break;
        }
        // Also try clicking the row itself (some UIs select on row click)
        const tc = (row.textContent || '').length;
        if (tc > 20 && tc < 500 && row.querySelector('img')) {
          clickElement(row);
          productSelected = true;
          log('Selected product by row click');
          break;
        }
      }
  ...
  if (!productSelected) {
    // Strategy B: Click on any element containing the product ID
    const idEl = Array.from(document.querySelectorAll('span, td, div, p')).find(el =>
      (el.textContent || '').trim().includes(productId)
    );
    if (idEl) {
      clickElement(idEl);
      productSelected = true;
      log('Selected product by ID text click');
    }
  }

  if (!productSelected) {
    warn('Could not find product with ID: ' + productId);
    return false;
  }
  ...
  // Click "เพิ่ม" (Add) button to confirm
  const addBtn = findByText('เพิ่ม', 'button') || findByText('Add', 'button');
  if (addBtn) {
    clickElement(addBtn);
    log('Clicked เพิ่ม (confirmed product)');
    await delay(1500);
  }

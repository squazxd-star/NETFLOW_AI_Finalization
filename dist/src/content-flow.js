...
.nf-proc-skipped {
    ...
    `}function Vt(){at||(at=document.createElement("style"),at.id="netflow-overlay-styles",at.textContent=be(Y),document.head.appendChild(at))}function Wt(e){e.innerHTML="",W.forEach((t,r)=>{const i=document.createElement("div");i.className="nf-proc-row nf-proc-waiting",i.id=`nf-proc-${t.stepId}`,i.innerHTML=`
            <span class="nf-proc-num">${r+1}</span>
            <span class="nf-proc-dot"></span>
            <span class="nf-proc-label">${t.label}</span>
            <span class="nf-proc-badge">(queued)</span>
        `,e.appendChild(i)})}function we(){const e=document.getElementById("nf-terminal");if(!e)return;Wt(e);const t=document.getElementById("nf-step-counter");t&&(t.textContent=`0/${W.length}`)}function Ut(e,t){let s="";for(let g=0;g<32;g++){const x=g/32*Math.PI*2,R=(g+.2)/32*Math.PI*2,f=(g+.5)/32*Math.PI*2,u=(g+.8)/32*Math.PI*2,v=(g+1)/32*Math.PI*2;s+=`${g===0?"M":"L"}${(120+104*Math.cos(x)).toFixed(1)},${(120+104*Math.sin(x)).toFixed(1)} `,s+=`L${(120+104*Math.cos(R)).toFixed(1)},${(120+104*Math.sin(R)).toFixed(1)} `,s+=`L${(120+116*Math.cos(f)).toFixed(1)},${(120+116*Math.sin(f)).toFixed(1)} `,s+=`L${(120+104*Math.cos(u)).toFixed(1)},${(120+104*Math.sin(u)).toFixed(1)} `,s+=`L${(120+104*Math.cos(v)).toFixed(1)},${(120+104*Math.sin(v)).toFixed(1)} `}s+="Z";const c=24,l=100,w=90;let I="";for(let g=0;g<c;g++){const x=g/c*Math.PI*2,R=(g+.25)/c*Math.PI*2,f=(g+.75)/c*Math.PI*2,u=(g+1)/c*Math.PI*2;I+=`${g===0?"M":"L"}${(120+w*Math.cos(x)).toFixed(1)},${(120+w*Math.sin(x)).toFixed(1)} `,I+=`L${(120+l*Math.cos(R)).toFixed(1)},${(120+l*Math.sin(R)).toFixed(1)} `,I+=`L${(120+l*Math.cos(f)).toFixed(1)},${(120+l*Math.sin(f)).toFixed(1)} `,I+=`L${(120+w*Math.cos(u)).toFixed(1)},${(120+w*Math.sin(u)).toFixed(1)} `}I+="Z";let O="";for(let g=0;g<64;g++){const x=g/64*Math.PI*2,R=g%4===0?117:119,f=g%4===0?124:122,u=g%4===0?.8:.4,v=g%4===0?.7:.35;O+=`<line x1="${(120+R*Math.cos(x)).toFixed(1)}" y1="${(120+R*Math.sin(x)).toFixed(1)}" x2="${(120+f*Math.cos(x)).toFixed(1)}" y2="${(120+f*Math.sin(x)).toFixed(1)}" stroke="rgba(${e},${v})" stroke-width="${u}"/>`}const E=26,k=78,b=68;let a="";for(let g=0;g<E;g++){const x=g/E*Math.PI*2,R=(g+.2)/E*Math.PI*2,f=(g+.5)/E*Math.PI*2,u=(g+.8)/E*Math.PI*2,v=(g+1)/E*Math.PI*2;a+=`${g===0?"M":"L"}${(120+b*Math.cos(x)).toFixed(1)},${(120+b*Math.sin(x)).toFixed(1)} `,a+=`L${(120+b*Math.cos(R)).toFixed(1)},${(120+b*Math.sin(R)).toFixed(1)} `,a+=`L${(120+k*Math.cos(f)).toFixed(1)},${(120+k*Math.sin(f)).toFixed(1)} `,a+=`L${(120+b*Math.cos(u)).toFixed(1)},${(120+b*Math.sin(u)).toFixed(1)} `,a+=`L${(120+b*Math.cos(v)).toFixed(1)},${(120+b*Math.sin(v)).toFixed(1)} `}a+="Z";let M="";for(let g=0;g<48;g++){const x=g/48*Math.PI*2,R=g%4===0?79:80,f=g%4===0?85:83,u=g%4===0?.6:.3,v=g%4===0?.6:.3;M+=`<line x1="${(120+R*Math.cos(x)).toFixed(1)}" y1="${(120+R*Math.sin(x)).toFixed(1)}" x2="${(120+f*Math.cos(x)).toFixed(1)}" y2="${(120+f*Math.sin(x)).toFixed(1)}" stroke="rgba(${t},${v})" stroke-width="${u}"/>`}function S(g,x,R,f,u){let v="";for(let A=0;A<R;A++){const _=A/R*Math.PI*2,C=(A+.25)/R*Math.PI*2,T=(A+.75)/R*Math.PI*2,L=(A+1)/R*Math.PI*2;v+=`${A===0?"M":"L"}${(g+u*Math.cos(_)).toFixed(1)},${(x+u*Math.sin(_)).toFixed(1)} `,v+=`L${(g+f*Math.cos(C)).toFixed(1)},${(x+f*Math.sin(C)).toFixed(1)} `,v+=`L${(g+f*Math.cos(T)).toFixed(1)},${(x+f*Math.sin(T)).toFixed(1)} `,v+=`L${(g+u*Math.cos(L)).toFixed(1)},${(x+u*Math.sin(L)).toFixed(1)} `}return v+"Z"}const D=42,$=[],y=S(120,120,14,18,13);$.push(`<g class="nf-kinetic-sub" style="transform-origin:120px 120px">
        <path d="${y}" fill="none" stroke="rgba(${e},0.8)" stroke-width="1.2"/>
    ...
    </g>`);for(let g=0;g<8;g++){const x=g/8*Math.PI*2,R=120+D*Math.cos(x),f=120+D*Math.sin(x),v=S(R,f,10,14,10),A=g%2===0?"":"animation-direction:reverse;";$.push(`<g class="nf-kinetic-sub" style="transform-origin:${R.toFixed(1)}px ${f.toFixed(1)}px;${A}">
...
`);let B="";for(let g=0;g<8;g++){const x=g/8*Math.PI*2,R=120+10*Math.cos(x),f=120+10*Math.sin(x),u=120+(D-10)*Math.cos(x),v=120+(D-10)*Math.sin(x);B+=`<line x1="${R.toFixed(1)}" y1="${f.toFixed(1)}" x2="${u.toFixed(1)}" y2="${v.toFixed(1)}" stroke="rgba(${t},0.25)" stroke-width="0.5"/>`}return`<svg width="80" height="80" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
    ...
    `,m.appendChild(B);const g=document.createElement("div");g.className="nf-terminal",g.id="nf-terminal",Wt(g),m.appendChild(g);const x=document.createElement("div");x.className="nf-engine-core",x.id="nf-engine-core";const R=document.createElement("div");R.className="nf-engine-frame",["nf-fc-tl","nf-fc-tr","nf-fc-bl","nf-fc-br"].forEach(C=>{const T=document.createElement("div");T.className=`nf-frame-corner ${C}`,R.appendChild(T)}),x.appendChild(R);const f="http://www.w3.org/2000/svg",u=document.createElementNS(f,"svg");u.setAttribute("class","nf-engine-waves"),u.setAttribute("viewBox","0 0 560 140"),u.setAttribute("preserveAspectRatio","none"),u.id="nf-engine-waves";for(let C=0;C<4;C++){const T=document.createElementNS(f,"path");T.setAttribute("fill","none"),T.setAttribute("stroke-width",C<2?"1.5":"1"),T.setAttribute("stroke",C<2?`rgba(${Y.rgb},${.14+C*.1})`:`rgba(${Y.accentRgb},${.1+(C-2)*.08})`),T.setAttribute("data-wave-idx",String(C)),u.appendChild(T)}x.appendChild(u);const v=document.createElement("div");v.className="nf-engine-brand-inner",v.innerHTML=`
        <div class="nf-brand-gear-icon nf-brand-gear-left">
            ${Ut(Y.rgb,Y.accentRgb)}
        </div>
        <div class="nf-brand-inner-text">NETFLOW AI ENGINE V1.0</div>
        <div class="nf-brand-gear-icon nf-brand-gear-right">
            ${Ut(Y.rgb,Y.accentRgb)}
        </div>
    `,x.appendChild(v);const A=document.createElement("div");A.className="nf-engine-stats",A.id="nf-engine-stats",A.innerHTML=[["SCENES","nf-stat-scenes","1/1"],["ELAPSED","nf-stat-elapsed","00:00"],["STEP","nf-stat-step","0/0"],["STATUS","nf-stat-status","READY"],["PROGRESS","nf-stat-progress","—"]].map(([C,T,L])=>`<div class="nf-stat-item"><span class="nf-stat-label">${C}</span><span class="nf-stat-val" id="${T}">${L}</span></div>`).join(""),x.appendChild(A),m.appendChild(x),y.appendChild(m);const _=["nf-mod-tl","nf-mod-tr","nf-mod-bl","nf-mod-br"];lt.forEach((C,T)=>{const L=ye(C);L.classList.add(_[T]),L.id=`nf-mod-${C.id}`,y.appendChild(L)}),e.appendChild(y);for(let C=0;C<30;C++){const T=document.createElement("div");T.className="nf-particle",T.style.left=`${5+Math.random()*90}%`,T.style.bottom=`${Math.random()*40}%`,T.style.animationDuration=`${3+Math.random()*5}s`,T.style.animationDelay=`${Math.random()*4}s`;const L=.3+Math.random()*.4,z=.7+Math.random()*.3;T.style.background=`rgba(${Math.floor(Z*z)}, ${Math.floor(Q*z)}, ${Math.floor(tt*z)}, ${L})`,T.style.width=`${1+Math.random()*2}px`,T.style.height=T.style.width,e.appendChild(T)}return e}function ye(e){const t=document.createElement("div");t.className="nf-module";const r=document.createElement("div");r.className="nf-mod-header",r.innerHTML=`
    ...
    `,t.appendChild(r),e.steps.forEach(o=>{const d=document.createElement("div");d.className="nf-step",d.id=`nf-step-${o.id}`;let p="";o.progress!==void 0&&(p=`
            ...
            ${p}
        `,t.appendChild(d)});const i=document.createElement("div");return i.className="nf-mod-progress",i.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${e.id}"></div>`,t.appendChild(i),t}function ve(){qt=Date.now(),$t=setInterval(()=>{const e=Math.floor((Date.now()-qt)/1e3),t=String(Math.floor(e/60)).padStart(2,"0"),r=String(e%60).padStart(2,"0"),i=document.getElementById("nf-timer");i&&(i.textContent=`${t}:${r}`);const o=document.getElementById("nf-stat-elapsed");o&&(o.textContent=`${t}:${r}`)},1e3)}function Yt(){$t&&(clearInterval($t),$t=null)}const $e=120,Xt=160,jt=.4;let dt=null,Kt=0,Jt=0,Zt=0,bt=[];function Ee(e,t){bt=[];for(let r=0;r<$e;r++){const i=Math.random();let o;i<.22?o=0:i<.4?o=1:i<.55?o=2:i<.68?o=3:i<.84?o=4:o=5;const d=Math.random()*e,p=Math.random()*t,s=50+Math.random()*220,c=Math.random()*Math.PI*2,l=(.003+Math.random()*.008)*(Math.random()>.5?1:-1);bt.push({x:o===0?Math.random()*e:d+Math.cos(c)*s,y:o===0?Math.random()*t:p+Math.sin(c)*s,vx:(Math.random()-.5)*jt,vy:(Math.random()-.5)*jt,radius:1.2+Math.random()*2.5,pulsePhase:Math.random()*Math.PI*2,pulseSpeed:.02+Math.random()*.03,motion:o,oCx:d,oCy:p,oRadius:s,oAngle:c,oSpeed:l})}}function ke(){if(!nt)return;const e=nt;if(mt=e.getContext("2d"),!mt)return;const t=()=>{e.width=window.innerWidth,e.height=window.innerHeight,bt.length===0&&Ee(e.width,e.height)};t(),window.addEventListener("resize",t);let r=null,i=0,o=0,d=!1;function p(){if(!mt||!nt){ht=null;return}if(ht=requestAnimationFrame(p),d=!d,d)return;const s=mt,c=nt.width,l=nt.height;s.fillStyle=`rgba(${Z*.04|0},${Q*.04|0},${tt*.06|0},1)`,s.fillRect(0,0,c,l),(!r||i!==c||o!==l)&&(i=c,o=l,r=s.createRadialGradient(c*.5,l*.5,0,c*.5,l*.5,Math.max(c,l)*.6),r.addColorStop(0,`rgba(${Z*.08|0},${Q*.08|0},${tt*.1|0},0.4)`),r.addColorStop(1,"rgba(0,0,0,0)")),s.fillStyle=r,s.fillRect(0,0,c,l);const w=bt,I=w.length,O=Xt*Xt;for(let b=0;b<I;b++){const a=w[b];if(a.pulsePhase+=a.pulseSpeed,a.motion===0)a.x+=a.vx,a.y+=a.vy,a.x<0?(a.x=0,a.vx=Math.abs(a.vx)*(.8+Math.random()*.4)):a.x>c&&(a.x=c,a.vx=-Math.abs(a.vx)*(.8+Math.random()*.4)),a.y<0?(a.y=0,a.vy=Math.abs(a.vy)*(.8+Math.random()*.4)):a.y>l&&(a.y=l,a.vy=-Math.abs(a.vy)*(.8+Math.random()*.4));else if(a.motion===1)a.oAngle+=a.oSpeed,a.x=a.oCx+Math.cos(a.oAngle)*a.oRadius,a.y=a.oCy+Math.sin(a.oAngle)*a.oRadius,a.oCx+=Math.sin(a.oAngle*.3)*.15,a.oCy+=Math.cos(a.oAngle*.3)*.15;else if(a.motion===2)a.oAngle+=a.oSpeed,a.x=a.oCx+Math.cos(a.oAngle)*a.oRadius,a.y=a.oCy+Math.sin(a.oAngle)*a.oRadius*.5,a.oCx+=Math.sin(a.oAngle*.2)*.1,a.oCy+=Math.cos(a.oAngle*.2)*.1;else if(a.motion===3){a.oAngle+=a.oSpeed;const M=a.oAngle,S=a.oRadius*.7;a.x=a.oCx+S*Math.cos(M),a.y=a.oCy+S*Math.sin(M)*Math.cos(M),a.oCx+=Math.sin(M*.15)*.12,a.oCy+=Math.cos(M*.15)*.12}else if(a.motion===4){a.oAngle+=a.oSpeed*1.2;const M=a.oRadius*(.5+.5*Math.abs(Math.sin(a.oAngle*.15)));a.x=a.oCx+Math.cos(a.oAngle)*M,a.y=a.oCy+Math.sin(a.oAngle)*M,a.oCx+=Math.sin(a.oAngle*.1)*.18,a.oCy+=Math.cos(a.oAngle*.1)*.18}else a.oAngle+=a.oSpeed,a.x+=a.vx*.8,a.y=a.oCy+Math.sin(a.oAngle+a.x*.008)*a.oRadius*.35,a.x<-30?a.x=c+30:a.x>c+30&&(a.x=-30),a.oCy+=Math.sin(a.oAngle*.1)*.08;if(a.motion>0){const M=a.oRadius+50;a.oCx<-M?a.oCx=c+M:a.oCx>c+M&&(a.oCx=-M),a.oCy<-M?a.oCy=l+M:a.oCy>l+M&&(a.oCy=-M)}}s.beginPath(),s.strokeStyle=`rgba(${Z},${Q},${tt},0.06)`,s.lineWidth=.4;const E=new Path2D;for(let b=0;b<I;b++){const a=w[b];for(let M=b+1;M<I;M++){const S=w[M],D=a.x-S.x,$=a.y-S.y,y=D*D+$*$;y<O&&(1-y/O<.4?(s.moveTo(a.x,a.y),s.lineTo(S.x,S.y)):(E.moveTo(a.x,a.y),E.lineTo(S.x,S.y)))}}if(s.stroke(),s.strokeStyle=`rgba(${Z},${Q},${tt},0.18)`,s.lineWidth=.8,s.stroke(E),!dt||Kt!==Z||Jt!==Q||Zt!==tt){dt=document.createElement("canvas");const b=48;dt.width=b,dt.height=b;const a=dt.getContext("2d"),M=a.createRadialGradient(b/2,b/2,0,b/2,b/2,b/2);M.addColorStop(0,`rgba(${Z},${Q},${tt},0.9)`),M.addColorStop(.3,`rgba(${Z},${Q},${tt},0.35)`),M.addColorStop(1,`rgba(${Z},${Q},${tt},0)`),a.fillStyle=M,a.fillRect(0,0,b,b),Kt=Z,Jt=Q,Zt=tt}const k=dt;for(let b=0;b<I;b++){const a=w[b],M=.6+.4*Math.sin(a.pulsePhase),S=a.radius*5*(.8+M*.4);s.globalAlpha=.5+M*.4,s.drawImage(k,a.x-S/2,a.y-S/2,S,S)}s.globalAlpha=1,s.fillStyle="rgba(255,255,255,0.45)",s.beginPath();for(let b=0;b<I;b++){const a=w[b];if(a.radius>2){const M=.6+.4*Math.sin(a.pulsePhase),S=a.radius*(.8+M*.4)*.35;s.moveTo(a.x+S,a.y),s.arc(a.x,a.y,S,0,Math.PI*2)}}s.fill()}p()}function Ce(){ht!==null&&(cancelAnimationFrame(ht),ht=null),nt=null,mt=null,bt=[]}let wt=null;const Ct=560,Me=140,Qt=Ct/2,te=Me/2,ee=[];for(let e=0;e<=Ct;e+=8){const t=Math.abs(e-Qt)/Qt;ee.push(Math.pow(Math.min(1,t*1.6),.6))}const Ie=[0,1,2,3].map(e=>({amp:10+e*5,freq:(1.2+e*.35)*Math.PI*2/Ct,off:e*.6,spd:.7+e*.12}));let _t=!1;function ne(){if(gt=requestAnimationFrame(ne),_t=!_t,_t)return;if(St+=.07,!wt){const t=document.getElementById("nf-engine-waves");if(!t){gt=null;return}wt=Array.from(t.querySelectorAll("path"))}const e=[];for(let t=0;t<wt.length;t++){const r=Ie[t],i=St*r.spd+r.off;e.length=0,e.push(`M 0 ${te}`);let o=0;for(let d=0;d<=Ct;d+=8){const p=te+r.amp*ee[o++]*Math.sin(d*r.freq+i);e.push(`L${d} ${p*10+.5|0}`)}wt[t].setAttribute("d",e.join(" "))}}function Se(){St=0,ne(),ke(),Et=setInterval(()=>{const e=document.getElementById("nf-stat-freq"),t=document.getElementById("nf-stat-lat1"),r=document.getElementById("nf-stat-lat2"),i=document.getElementById("nf-stat-buf");e&&(e.textContent=`${(4.5+Math.random()*.5).toFixed(1)} GHz`),t&&(t.textContent=`${Math.floor(12+Math.random()*10)}ms`),r&&(r.textContent=`${Math.floor(12+Math.random()*10)}ms`),i&&(i.textContent=`${Math.floor(90+Math.random()*9)}%`)},2e3)}function oe(){gt!==null&&(cancelAnimationFrame(gt),gt=null),Et&&(clearInterval(Et),Et=null),wt=null,Ce()}function Tt(){let e=0;const t=W.filter(l=>l.status!=="skipped").length;for(const l of W){const w=document.getElementById(`nf-proc-${l.stepId}`);if(!w)continue;w.className="nf-proc-row";const I=w.querySelector(".nf-proc-badge");switch(l.status){case"done":w.classList.add("nf-proc-done"),I&&(I.textContent="✅ done"),e++;break;case"active":w.classList.add("nf-proc-active"),I&&(I.textContent=l.progress!==void 0&&l.progress>0?`⏳ ${l.progress}%`:"⏳ active");break;case"error":w.classList.add("nf-proc-error"),I&&(I.textContent="❌ error");break;case"skipped":w.classList.add("nf-proc-skipped"),I&&(I.textContent="— skip");break;default:w.classList.add("nf-proc-waiting"),I&&(I.textContent="(queued)")}}const r=W.findIndex(l=>l.status==="active"),i=r>=0?r+1:e>=t&&t>0?W.length:e,o=document.getElementById("nf-step-counter");o&&(o.textContent=`${i}/${W.length}`);const d=document.querySelector(".nf-core-title-val"),p=document.querySelector(".nf-status-dot");e>=t&&t>0?(d&&(d.textContent="COMPLETE",d.style.color=Y.doneHex),p&&(p.style.background=Y.doneHex,p.style.boxShadow=`0 0 8px rgba(${Y.doneRgb},0.7)`)):W.some(w=>w.status==="error")?(d&&(d.textContent="ERROR",d.style.color="#f87171"),p&&(p.style.background="#ef4444",p.style.boxShadow="0 0 8px rgba(239,68,68,0.7)")):W.some(w=>w.status==="active")&&d&&(d.textContent="ACTIVE",d.style.color=Y.hex,d.style.textShadow=`0 0 10px rgba(${Y.rgb},0.5)`);const s=document.getElementById("nf-terminal"),c=s==null?void 0:s.querySelector(".nf-proc-active");c&&s&&c.scrollIntoView({behavior:"smooth",block:"center"})}function ie(){X&&X.isConnected||(Vt(),X=document.createElement("button"),X.id="nf-toggle-btn",X.className="nf-toggle-visible",X.innerHTML=st?"⚡":"✕",X.title="ซ่อน/แสดง Netflow Overlay",X.onclick=()=>At(),document.body.appendChild(X))}function At(){V&&(ie(),st?(V.classList.remove("nf-hidden"),V.classList.add("nf-visible"),X&&(X.innerHTML="✕"),st=!1):(V.classList.remove("nf-visible"),V.classList.add("nf-hidden"),X&&(X.innerHTML="⚡"),st=!0))}const ae={red:"themes/theme-red.jpg",yellow:"themes/theme-yellow.jpg",blue:"themes/theme-blue.jpg",purple:"themes/theme-purple.jpg",green:"themes/theme-green.jpg"};function re(){const e=document.getElementById("nf-core-monitor");if(!e)return;let t=ut;if(!t)try{t=localStorage.getItem("netflow_app_theme")||"red"}catch{t="red"}const r=ae[t]||ae.red;let i;try{i=chrome.runtime.getURL(r)}catch{i=`/${r}`}const o=Y.rgb;e.style.backgroundImage=["linear-gradient(180deg, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.78) 45%, rgba(0,0,0,0.90) 100%)",`linear-gradient(180deg, rgba(${o},0.25) 0%, rgba(${o},0.12) 50%, rgba(${o},0.20) 100%)`,`url('${i}')`].join(", "),e.style.backgroundSize="auto, auto, cover",e.style.backgroundPosition="center, center, center",e.style.backgroundRepeat="no-repeat, no-repeat, no-repeat",e.style.setProperty("--nf-bg-set","1"),e.style.border=`1.5px solid rgba(${o},0.45)`,e.style.boxShadow=`0 0 70px rgba(${o},0.22), 0 0 140px rgba(${o},0.1), inset 0 1px 0 rgba(${o},0.15)`}function Bt(e=1){if(Y=he(),Ht(),V&&V.isConnected){st&&At();return}if(V&&!V.isConnected&&(V=null),at&&(at.remove(),at=null),Vt(),kt=e,W=Pt(e),e>1){const t=lt.find(i=>i.id==="video");if(t){const i=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:"waiting",progress:0}];for(let o=2;o<=e;o++)i.push({id:`scene${o}-prompt`,label:`Scene ${o} Prompt`,status:"waiting"}),i.push({id:`scene${o}-gen`,label:`Scene ${o} Generate`,status:"waiting"}),i.push({id:`scene${o}-wait`,label:`Scene ${o} รอผล`,status:"waiting",progress:0});t.steps=i}const r=lt.find(i=>i.id==="render");if(r){const i=r.steps.find(d=>d.id==="download");i&&(i.label="ดาวน์โหลด 720p");const o=r.steps.find(d=>d.id==="upscale");o&&(o.label="Full Video")}}V=xe(),document.body.appendChild(V),st=!1,ie(),ve(),Se(),requestAnimationFrame(()=>re())}function se(){Yt(),oe(),st=!1,V&&(V.classList.add("nf-fade-out"),setTimeout(()=>{V==null||V.remove(),V=null},500)),X&&(X.remove(),X=null)}const Pe={settings:"CONFIG","upload-char":"UPLOAD","upload-prod":"UPLOAD","img-prompt":"PROMPT","img-generate":"GENERATE","img-wait":"IMG WAIT",animate:"ANIMATE","vid-prompt":"VID PROMPT","vid-generate":"VID GEN","vid-wait":"VID WAIT",download:"DOWNLOAD",upscale:"UPSCALE",open:"OPENING"};function _e(e,t,r){const i=W.findIndex(l=>l.status==="active"),o=W.filter(l=>l.status==="done").length,d=W.length,p=i>=0?i+1:o>=d?d:o,s=document.getElementById("nf-stat-step");s&&(s.textContent=`${p}/${d}`);const c=document.getElementById("nf-stat-scenes");if(c&&(c.textContent=kt>1?`1/${kt}`:"1/1"),t==="active"){const l=document.getElementById("nf-stat-status"),w=Pe[e]||e.toUpperCase();l&&(l.textContent=w)}else if(t==="done"&&o>=d){const l=document.getElementById("nf-stat-status");l&&(l.textContent="COMPLETE")}else if(t==="error"){const l=document.getElementById("nf-stat-status");l&&(l.textContent="ERROR")}if(r!==void 0&&r>0){const l=document.getElementById("nf-stat-progress");l&&(l.textContent=`${Math.min(100,r)}%`)}}function P(e,t,r){if(!V)return;for(const o of lt)for(const d of o.steps)d.id===e&&(d.status=t,r!==void 0&&(d.progress=r));for(const o of W)o.stepId===e&&(o.status=t,r!==void 0&&(o.progress=r));const i=document.getElementById(`nf-step-${e}`);if(i&&(i.className="nf-step",t==="active"?i.classList.add("nf-step-active"):t==="done"?i.classList.add("nf-step-done"):t==="error"&&i.classList.add("nf-step-error")),_e(e,t,r),r!==void 0){const o=document.getElementById(`nf-bar-${e}`);o&&(o.style.width=`${Math.min(100,r)}%`)}Ft(),Tt()}function pt(e){P(e,"skipped");const t=document.getElementById(`nf-step-${e}`);t&&(t.style.opacity="0.2")}function xt(e=4e3){Yt(),oe(),Ft(),Tt(),setTimeout(()=>se(),e)}function Ft(){for(const e of lt){const t=e.steps.filter(c=>c.status!=="skipped").length,r=e.steps.filter(c=>c.status==="done").length,i=e.steps.some(c=>c.status==="active"),o=t>0?Math.round(r/t*100):0,d=document.getElementById(`nf-pct-${e.id}`);d&&(d.textContent=`${o}%`);const p=document.getElementById(`nf-modbar-${e.id}`);p&&(p.style.width=`${o}%`);const s=document.getElementById(`nf-mod-${e.id}`);s&&(s.classList.remove("nf-active","nf-done"),o>=100?s.classList.add("nf-done"):i&&s.classList.add("nf-active"))}}function Te(e){var i,o,d,p;kt=e;const t=new Map;for(const s of W)t.set(s.stepId,{status:s.status,progress:s.progress});W=Pt(e);for(const s of W){const c=t.get(s.stepId);c&&(s.status=c.status,c.progress!==void 0&&(s.progress=c.progress))}if(we(),e>1){const s=lt.find(c=>c.id==="video");if(s){const c=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:((i=s.steps.find(l=>l.id==="animate"))==null?void 0:i.status)||"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:((o=s.steps.find(l=>l.id==="vid-prompt"))==null?void 0:o.status)||"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:((d=s.steps.find(l=>l.id==="vid-generate"))==null?void 0:d.status)||"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:((p=s.steps.find(l=>l.id==="vid-wait"))==null?void 0:p.status)||"waiting",progress:0}];for(let l=2;l<=e;l++)c.push({id:`scene${l}-prompt`,label:`Scene ${l} Prompt`,status:"waiting"}),c.push({id:`scene${l}-gen`,label:`Scene ${l} Generate`,status:"waiting"}),c.push({id:`scene${l}-wait`,label:`Scene ${l} รอผล`,status:"waiting",progress:0});s.steps=c,le(s)}}const r=lt.find(s=>s.id==="render");if(r&&e>1){const s=r.steps.find(l=>l.id==="download");s&&(s.label="ดาวน์โหลด 720p");const c=r.steps.find(l=>l.id==="upscale");c&&(c.label="Full Video"),le(r)}Ft(),Tt()}function le(e){const t=document.getElementById(`nf-mod-${e.id}`);if(!t)return;t.querySelectorAll(".nf-step, .nf-mod-progress").forEach(o=>o.remove()),e.steps.forEach(o=>{const d=document.createElement("div");d.className="nf-step",d.id=`nf-step-${o.id}`;let p="";o.progress!==void 0&&(p=`
                <div class="nf-progress-bar">
                    <div class="nf-progress-fill" id="nf-bar-${o.id}" style="width: 0%"></div>
                </div>
            `),d.innerHTML=`

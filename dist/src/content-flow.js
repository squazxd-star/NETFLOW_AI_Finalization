...
.nf-log-line:last-child {
    ...
    `,o.appendChild(s),e.steps.forEach(r=>{const l=document.createElement("div");l.className="nf-step",l.id=`nf-step-${r.id}`;let d="";r.progress!==void 0&&(d=`
            ...
            ${d}
        `,o.appendChild(l)});const n=document.createElement("div");return n.className="nf-mod-progress",n.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${e.id}"></div>`,o.appendChild(n),o}function Re(){xe=Date.now(),le=setInterval(()=>{const e=Math.floor((Date.now()-xe)/1e3),o=String(Math.floor(e/60)).padStart(2,"0"),s=String(e%60).padStart(2,"0"),n=document.getElementById("nf-timer");n&&(n.textContent=`${o}:${s}`)},1e3)}function $e(){le&&(clearInterval(le),le=null)}function _e(){de=setInterval(()=>{const e=document.getElementById("nf-visualizer");if(!e)return;e.querySelectorAll(".nf-viz-bar").forEach(s=>{const n=3+Math.random()*28;s.style.height=`${n}px`})},180)}function ke(){de&&(clearInterval(de),de=null);const e=document.getElementById("nf-visualizer");e&&e.querySelectorAll(".nf-viz-bar").forEach(o=>{o.style.height="3px"})}function fe(){let e=0;const o=V.filter(c=>c.status!=="skipped").length;for(const c of V){const i=document.getElementById(`nf-proc-${c.stepId}`);if(!i)continue;i.className="nf-proc-row";const a=i.querySelector(".nf-proc-badge");switch(c.status){case"done":i.classList.add("nf-proc-done"),a&&(a.textContent="✅ done"),e++;break;case"active":i.classList.add("nf-proc-active"),a&&(a.textContent=c.progress!==void 0&&c.progress>0?`⏳ ${c.progress}%`:"⏳ active");break;case"error":i.classList.add("nf-proc-error"),a&&(a.textContent="❌ error");break;case"skipped":i.classList.add("nf-proc-skipped"),a&&(a.textContent="— skip");break;default:i.classList.add("nf-proc-waiting"),a&&(a.textContent="(queued)")}}const s=document.getElementById("nf-step-counter");s&&(s.textContent=`${e}/${V.length}`);const n=document.querySelector(".nf-core-title-val"),r=document.querySelector(".nf-status-dot");e>=o&&o>0?(n&&(n.textContent="COMPLETE",n.style.color="#4ade80"),r&&(r.style.background="#4ade80",r.style.boxShadow="0 0 8px rgba(74,222,128,0.7)")):V.some(i=>i.status==="error")?(n&&(n.textContent="ERROR",n.style.color="#f87171"),r&&(r.style.background="#ef4444",r.style.boxShadow="0 0 8px rgba(239,68,68,0.7)")):V.some(i=>i.status==="active")&&n&&(n.textContent="ACTIVE",n.style.color="#4ade80",n.style.textShadow="0 0 10px rgba(74,222,128,0.5)");const l=document.getElementById("nf-terminal"),d=l==null?void 0:l.querySelector(".nf-proc-active");d&&l&&d.scrollIntoView({behavior:"smooth",block:"center"})}function Pe(){U||(Ee(),U=document.createElement("button"),U.id="nf-toggle-btn",U.className="nf-toggle-hidden",U.innerHTML="⚡",U.title="เปิด Netflow Overlay",U.onclick=()=>ge(),document.body.appendChild(U))}function ge(){W&&(Pe(),Z?(W.classList.remove("nf-hidden"),W.classList.add("nf-visible"),U&&(U.classList.remove("nf-toggle-visible"),U.classList.add("nf-toggle-hidden")),Z=!1):(W.classList.remove("nf-visible"),W.classList.add("nf-hidden"),U&&(U.classList.remove("nf-toggle-hidden"),U.classList.add("nf-toggle-visible")),Z=!0))}function Oe(){if(W){Z&&ge();return}Ee(),V=ue(1),ie.length=0,W=Ne(),document.body.appendChild(W),Z=!1,Pe(),Re(),_e()}function ze(){$e(),ke(),Z=!1,W&&(W.classList.add("nf-fade-out"),setTimeout(()=>{W==null||W.remove(),W=null},500)),U&&(U.remove(),U=null)}function m(e,o,s){if(!W)return;for(const r of re)for(const l of r.steps)l.id===e&&(l.status=o,s!==void 0&&(l.progress=s));for(const r of V)r.stepId===e&&(r.status=o,s!==void 0&&(r.progress=s));const n=document.getElementById(`nf-step-${e}`);if(n&&(n.className="nf-step",o==="active"?n.classList.add("nf-step-active"):o==="done"?n.classList.add("nf-step-done"):o==="error"&&n.classList.add("nf-step-error")),s!==void 0){const r=document.getElementById(`nf-bar-${e}`);r&&(r.style.width=`${Math.min(100,s)}%`)}me(),fe()}function ee(e){m(e,"skipped");const o=document.getElementById(`nf-step-${e}`);o&&(o.style.opacity="0.2")}function Ge(e=4e3){$e(),ke(),me(),fe(),setTimeout(()=>ze(),e)}function me(){for(const e of re){const o=e.steps.filter(i=>i.status!=="skipped").length,s=e.steps.filter(i=>i.status==="done").length,n=e.steps.some(i=>i.status==="active"),r=o>0?Math.round(s/o*100):0,l=document.getElementById(`nf-pct-${e.id}`);l&&(l.textContent=`${r}%`);const d=document.getElementById(`nf-modbar-${e.id}`);d&&(d.style.width=`${r}%`);const c=document.getElementById(`nf-mod-${e.id}`);c&&(c.classList.remove("nf-active","nf-done"),r>=100?c.classList.add("nf-done"):n&&c.classList.add("nf-active"))}}function Ue(e){var n,r,l,d;const o=new Map;for(const c of V)o.set(c.stepId,{status:c.status,progress:c.progress});V=ue(e);for(const c of V){const i=o.get(c.stepId);i&&(c.status=i.status,i.progress!==void 0&&(c.progress=i.progress))}if(De(),e>1){const c=re.find(i=>i.id==="video");if(c){const i=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:((n=c.steps.find(a=>a.id==="animate"))==null?void 0:n.status)||"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:((r=c.steps.find(a=>a.id==="vid-prompt"))==null?void 0:r.status)||"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:((l=c.steps.find(a=>a.id==="vid-generate"))==null?void 0:l.status)||"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:((d=c.steps.find(a=>a.id==="vid-wait"))==null?void 0:d.status)||"waiting",progress:0}];for(let a=2;a<=e;a++)i.push({id:`scene${a}-prompt`,label:`Scene ${a} Prompt`,status:"waiting"}),i.push({id:`scene${a}-gen`,label:`Scene ${a} Generate`,status:"waiting"}),i.push({id:`scene${a}-wait`,label:`Scene ${a} รอผล`,status:"waiting",progress:0});c.steps=i,Se(c)}}const s=re.find(c=>c.id==="render");if(s&&e>1){const c=s.steps.find(a=>a.id==="download");c&&(c.label="ดาวน์โหลด 720p");const i=s.steps.find(a=>a.id==="upscale");i&&(i.label="Full Video"),Se(s)}me(),fe()}function Se(e){const o=document.getElementById(`nf-mod-${e.id}`);if(!o)return;o.querySelectorAll(".nf-step, .nf-mod-progress").forEach(r=>r.remove()),e.steps.forEach(r=>{const l=document.createElement("div");l.className="nf-step",l.id=`nf-step-${r.id}`;let d="";r.progress!==void 0&&(d=`
                <div class="nf-progress-bar">
                    <div class="nf-progress-fill" id="nf-bar-${r.id}" style="width: 0%"></div>
                </div>
            `),l.innerHTML=`

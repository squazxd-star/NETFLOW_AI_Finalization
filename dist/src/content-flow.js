...
@media (max-height: 600px) {
    ...
    `,x.appendChild(f);const h=document.createElement("div");h.className="nf-terminal",h.id="nf-terminal",Qe(h),x.appendChild(h);const E=document.createElement("div");E.className="nf-engine-core",E.id="nf-engine-core";const z=document.createElement("div");z.className="nf-engine-frame",["nf-fc-tl","nf-fc-tr","nf-fc-bl","nf-fc-br"].forEach(y=>{const P=document.createElement("div");P.className=`nf-frame-corner ${y}`,z.appendChild(P)}),E.appendChild(z);const B="http://www.w3.org/2000/svg",u=document.createElementNS(B,"svg");u.setAttribute("class","nf-engine-waves"),u.setAttribute("viewBox","0 0 560 140"),u.setAttribute("preserveAspectRatio","none"),u.id="nf-engine-waves";for(let y=0;y<4;y++){const P=document.createElementNS(B,"path");P.setAttribute("fill","none"),P.setAttribute("stroke-width",y<2?"1.5":"1"),P.setAttribute("stroke",y<2?`rgba(${J.rgb},${.14+y*.1})`:`rgba(${J.accentRgb},${.1+(y-2)*.08})`),P.setAttribute("data-wave-idx",String(y)),u.appendChild(P)}E.appendChild(u);const v=document.createElement("div");v.className="nf-engine-brand-inner",v.innerHTML=`
    ...
    `,E.appendChild(v);const M=document.createElement("div");M.className="nf-engine-stats",M.id="nf-engine-stats",M.innerHTML=[["SCENES","nf-stat-scenes","1/1"],["ELAPSED","nf-stat-elapsed","00:00"],["STEP","nf-stat-step","0/0"],["STATUS","nf-stat-status","READY"],["PROGRESS","nf-stat-progress","—"]].map(([y,P,O])=>`<div class="nf-stat-item"><span class="nf-stat-label">${y}</span><span class="nf-stat-val" id="${P}">${O}</span></div>`).join(""),E.appendChild(M),x.appendChild(E),V.appendChild(x);const I=["nf-mod-tl","nf-mod-tr","nf-mod-bl","nf-mod-br"];de.forEach((y,P)=>{const O=Mt(y);O.classList.add(I[P]),O.id=`nf-mod-${y.id}`,V.appendChild(O)}),t.appendChild(V);for(let y=0;y<30;y++){const P=document.createElement("div");P.className="nf-particle",P.style.left=`${5+Math.random()*90}%`,P.style.bottom=`${Math.random()*40}%`,P.style.animationDuration=`${3+Math.random()*5}s`,P.style.animationDelay=`${Math.random()*4}s`;const O=.3+Math.random()*.4,T=.7+Math.random()*.3;P.style.background=`rgba(${Math.floor(ne*T)}, ${Math.floor(oe*T)}, ${Math.floor(ie*T)}, ${O})`,P.style.width=`${1+Math.random()*2}px`,P.style.height=P.style.width,t.appendChild(P)}return t}function Mt(t){const e=document.createElement("div");e.className="nf-module";const i=document.createElement("div");i.className="nf-mod-header",i.innerHTML=`

...
.nf-proc-skipped {
    ...
    `}function Ne(){le||(le=document.createElement("style"),le.id="netflow-overlay-styles",le.textContent=ot(Y),document.head.appendChild(le))}function ze(t){t.innerHTML="",X.forEach((e,c)=>{const a=document.createElement("div");a.className="nf-proc-row nf-proc-waiting",a.id=`nf-proc-${e.stepId}`,a.innerHTML=`
            <span class="nf-proc-num">${c+1}</span>
            <span class="nf-proc-dot"></span>
            <span class="nf-proc-label">${e.label}</span>
            <span class="nf-proc-badge">(queued)</span>
        `,t.appendChild(a)})}function it(){const t=document.getElementById("nf-terminal");if(!t)return;ze(t);const e=document.getElementById("nf-step-counter");e&&(e.textContent=`0/${X.length}`)}function He(t,e){let r="";for(let u=0;u<32;u++){const w=u/32*Math.PI*2,P=(u+.2)/32*Math.PI*2,B=(u+.5)/32*Math.PI*2,k=(u+.8)/32*Math.PI*2,m=(u+1)/32*Math.PI*2;r+=`${u===0?"M":"L"}${(120+104*Math.cos(w)).toFixed(1)},${(120+104*Math.sin(w)).toFixed(1)} `,r+=`L${(120+104*Math.cos(P)).toFixed(1)},${(120+104*Math.sin(P)).toFixed(1)} `,r+=`L${(120+116*Math.cos(B)).toFixed(1)},${(120+116*Math.sin(B)).toFixed(1)} `,r+=`L${(120+104*Math.cos(k)).toFixed(1)},${(120+104*Math.sin(k)).toFixed(1)} `,r+=`L${(120+104*Math.cos(m)).toFixed(1)},${(120+104*Math.sin(m)).toFixed(1)} `}r+="Z";const l=24,p=100,y=90;let M="";for(let u=0;u<l;u++){const w=u/l*Math.PI*2,P=(u+.25)/l*Math.PI*2,B=(u+.75)/l*Math.PI*2,k=(u+1)/l*Math.PI*2;M+=`${u===0?"M":"L"}${(120+y*Math.cos(w)).toFixed(1)},${(120+y*Math.sin(w)).toFixed(1)} `,M+=`L${(120+p*Math.cos(P)).toFixed(1)},${(120+p*Math.sin(P)).toFixed(1)} `,M+=`L${(120+p*Math.cos(B)).toFixed(1)},${(120+p*Math.sin(B)).toFixed(1)} `,M+=`L${(120+y*Math.cos(k)).toFixed(1)},${(120+y*Math.sin(k)).toFixed(1)} `}M+="Z";let G="";for(let u=0;u<64;u++){const w=u/64*Math.PI*2,P=u%4===0?117:119,B=u%4===0?124:122,k=u%4===0?.8:.4,m=u%4===0?.7:.35;G+=`<line x1="${(120+P*Math.cos(w)).toFixed(1)}" y1="${(120+P*Math.sin(w)).toFixed(1)}" x2="${(120+B*Math.cos(w)).toFixed(1)}" y2="${(120+B*Math.sin(w)).toFixed(1)}" stroke="rgba(${t},${m})" stroke-width="${k}"/>`}const b=26,o=78,g=68;let v="";for(let u=0;u<b;u++){const w=u/b*Math.PI*2,P=(u+.2)/b*Math.PI*2,B=(u+.5)/b*Math.PI*2,k=(u+.8)/b*Math.PI*2,m=(u+1)/b*Math.PI*2;v+=`${u===0?"M":"L"}${(120+g*Math.cos(w)).toFixed(1)},${(120+g*Math.sin(w)).toFixed(1)} `,v+=`L${(120+g*Math.cos(P)).toFixed(1)},${(120+g*Math.sin(P)).toFixed(1)} `,v+=`L${(120+o*Math.cos(B)).toFixed(1)},${(120+o*Math.sin(B)).toFixed(1)} `,v+=`L${(120+g*Math.cos(k)).toFixed(1)},${(120+g*Math.sin(k)).toFixed(1)} `,v+=`L${(120+g*Math.cos(m)).toFixed(1)},${(120+g*Math.sin(m)).toFixed(1)} `}v+="Z";let C="";for(let u=0;u<48;u++){const w=u/48*Math.PI*2,P=u%4===0?79:80,B=u%4===0?85:83,k=u%4===0?.6:.3,m=u%4===0?.6:.3;C+=`<line x1="${(120+P*Math.cos(w)).toFixed(1)}" y1="${(120+P*Math.sin(w)).toFixed(1)}" x2="${(120+B*Math.cos(w)).toFixed(1)}" y2="${(120+B*Math.sin(w)).toFixed(1)}" stroke="rgba(${e},${m})" stroke-width="${k}"/>`}function x(u,w,P,B,k){let m="";for(let E=0;E<P;E++){const A=E/P*Math.PI*2,I=(E+.25)/P*Math.PI*2,R=(E+.75)/P*Math.PI*2,N=(E+1)/P*Math.PI*2;m+=`${E===0?"M":"L"}${(u+k*Math.cos(A)).toFixed(1)},${(w+k*Math.sin(A)).toFixed(1)} `,m+=`L${(u+B*Math.cos(I)).toFixed(1)},${(w+B*Math.sin(I)).toFixed(1)} `,m+=`L${(u+B*Math.cos(R)).toFixed(1)},${(w+B*Math.sin(R)).toFixed(1)} `,m+=`L${(u+k*Math.cos(N)).toFixed(1)},${(w+k*Math.sin(N)).toFixed(1)} `}return m+"Z"}const S=42,L=[],$=x(120,120,14,18,13);L.push(`<g class="nf-kinetic-sub" style="transform-origin:120px 120px">
    ...
    </g>`);for(let u=0;u<8;u++){const w=u/8*Math.PI*2,P=120+S*Math.cos(w),B=120+S*Math.sin(w),m=x(P,B,10,14,10),E=u%2===0?"":"animation-direction:reverse;";L.push(`<g class="nf-kinetic-sub" style="transform-origin:${P.toFixed(1)}px ${B.toFixed(1)}px;${E}">
            <path d="${m}" fill="none" stroke="rgba(${e},0.6)" stroke-width="0.9"/>
            <circle cx="${P.toFixed(1)}" cy="${B.toFixed(1)}" r="7" fill="none" stroke="rgba(${t},0.4)" stroke-width="0.5" stroke-dasharray="1.5,1.5"/>
            <circle cx="${P.toFixed(1)}" cy="${B.toFixed(1)}" r="2.5" fill="rgba(${t},0.6)"/>
        </g>`)}const f=L.join(`
`);let F="";for(let u=0;u<8;u++){const w=u/8*Math.PI*2,P=120+10*Math.cos(w),B=120+10*Math.sin(w),k=120+(S-10)*Math.cos(w),m=120+(S-10)*Math.sin(w);F+=`<line x1="${P.toFixed(1)}" y1="${B.toFixed(1)}" x2="${k.toFixed(1)}" y2="${m.toFixed(1)}" stroke="rgba(${e},0.25)" stroke-width="0.5"/>`}return`<svg width="80" height="80" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
    ...
    `,r.appendChild(l);const p=document.createElement("div");p.className="nf-terminal",p.id="nf-terminal",ze(p),r.appendChild(p);const y=document.createElement("div");y.className="nf-engine-core",y.id="nf-engine-core";const M=document.createElement("div");M.className="nf-engine-frame",["nf-fc-tl","nf-fc-tr","nf-fc-bl","nf-fc-br"].forEach(C=>{const x=document.createElement("div");x.className=`nf-frame-corner ${C}`,M.appendChild(x)}),y.appendChild(M);const G="http://www.w3.org/2000/svg",b=document.createElementNS(G,"svg");b.setAttribute("class","nf-engine-waves"),b.setAttribute("viewBox","0 0 560 140"),b.setAttribute("preserveAspectRatio","none"),b.id="nf-engine-waves";for(let C=0;C<6;C++){const x=document.createElementNS(G,"path");x.setAttribute("fill","none"),x.setAttribute("stroke-width",C<3?"1.5":"1"),x.setAttribute("stroke",C<3?`rgba(${Y.rgb},${.12+C*.08})`:`rgba(${Y.accentRgb},${.08+(C-3)*.06})`),x.setAttribute("data-wave-idx",String(C)),b.appendChild(x)}y.appendChild(b);const o=document.createElement("div");o.className="nf-engine-brand-inner",o.innerHTML=`
        <div class="nf-brand-gear-icon nf-brand-gear-left">
            ${He(Y.rgb,Y.accentRgb)}
        </div>
        <div class="nf-brand-inner-text">NETFLOW AI ENGINE V1.0</div>
        <div class="nf-brand-gear-icon nf-brand-gear-right">
            ${He(Y.rgb,Y.accentRgb)}
        </div>
    `,y.appendChild(o);const g=document.createElement("div");g.className="nf-engine-stats",g.id="nf-engine-stats",g.innerHTML=[["SCENES","nf-stat-scenes","1/1"],["ELAPSED","nf-stat-elapsed","00:00"],["STEP","nf-stat-step","0/0"],["STATUS","nf-stat-status","READY"],["PROGRESS","nf-stat-progress","—"]].map(([C,x,S])=>`<div class="nf-stat-item"><span class="nf-stat-label">${C}</span><span class="nf-stat-val" id="${x}">${S}</span></div>`).join(""),y.appendChild(g),r.appendChild(y),s.appendChild(r);const v=["nf-mod-tl","nf-mod-tr","nf-mod-bl","nf-mod-br"];ce.forEach((C,x)=>{const S=rt(C);S.classList.add(v[x]),S.id=`nf-mod-${C.id}`,s.appendChild(S)}),t.appendChild(s);for(let C=0;C<30;C++){const x=document.createElement("div");x.className="nf-particle",x.style.left=`${5+Math.random()*90}%`,x.style.bottom=`${Math.random()*40}%`,x.style.animationDuration=`${3+Math.random()*5}s`,x.style.animationDelay=`${Math.random()*4}s`;const S=.3+Math.random()*.4,L=.7+Math.random()*.3;x.style.background=`rgba(${Math.floor(ne*L)}, ${Math.floor(oe*L)}, ${Math.floor(ie*L)}, ${S})`,x.style.width=`${1+Math.random()*2}px`,x.style.height=x.style.width,t.appendChild(x)}return t}function rt(t){const e=document.createElement("div");e.className="nf-module";const c=document.createElement("div");c.className="nf-mod-header",c.innerHTML=`

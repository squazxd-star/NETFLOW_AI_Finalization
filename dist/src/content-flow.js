...
@media (max-height: 600px) {
    ...
    `}function Xe(){X||(X=document.createElement("style"),X.id="netflow-overlay-styles",X.textContent=Wt(Z),document.head.appendChild(X))}function gt(t){t.innerHTML="",j.forEach((e,r)=>{const a=document.createElement("div");a.className="nf-proc-row nf-proc-waiting",a.id=`nf-proc-${e.stepId}`,a.innerHTML=`
        ...
        `,t.appendChild(a)})}function mt(){const t=document.getElementById("nf-terminal");if(!t)return;gt(t);const e=document.getElementById("nf-step-counter");e&&(e.textContent=`0/${j.length}`)}function ht(t,e){let s="";for(let C=0;C<20;C++){const L=C/20*Math.PI*2,B=(C+.2)/20*Math.PI*2,h=(C+.5)/20*Math.PI*2,i=(C+.8)/20*Math.PI*2,m=(C+1)/20*Math.PI*2;s+=`${C===0?"M":"L"}${(120+100*Math.cos(L)).toFixed(1)},${(120+100*Math.sin(L)).toFixed(1)} `,s+=`L${(120+100*Math.cos(B)).toFixed(1)},${(120+100*Math.sin(B)).toFixed(1)} `,s+=`L${(120+112*Math.cos(h)).toFixed(1)},${(120+112*Math.sin(h)).toFixed(1)} `,s+=`L${(120+100*Math.cos(i)).toFixed(1)},${(120+100*Math.sin(i)).toFixed(1)} `,s+=`L${(120+100*Math.cos(m)).toFixed(1)},${(120+100*Math.sin(m)).toFixed(1)} `}s+="Z";const c=14,d=72,x=62;let v="";for(let C=0;C<c;C++){const L=C/c*Math.PI*2,B=(C+.25)/c*Math.PI*2,h=(C+.75)/c*Math.PI*2,i=(C+1)/c*Math.PI*2;v+=`${C===0?"M":"L"}${(120+x*Math.cos(L)).toFixed(1)},${(120+x*Math.sin(L)).toFixed(1)} `,v+=`L${(120+d*Math.cos(B)).toFixed(1)},${(120+d*Math.sin(B)).toFixed(1)} `,v+=`L${(120+d*Math.cos(h)).toFixed(1)},${(120+d*Math.sin(h)).toFixed(1)} `,v+=`L${(120+x*Math.cos(i)).toFixed(1)},${(120+x*Math.sin(i)).toFixed(1)} `}return v+="Z",`<svg width="72" height="72" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
    ...
    `,D.appendChild(y);const g=document.createElement("div");g.className="nf-terminal",g.id="nf-terminal",gt(g),D.appendChild(g);const w=document.createElement("div");w.className="nf-engine-core",w.id="nf-engine-core";const I=document.createElement("div");I.className="nf-engine-frame",["nf-fc-tl","nf-fc-tr","nf-fc-bl","nf-fc-br"].forEach(u=>{const b=document.createElement("div");b.className=`nf-frame-corner ${u}`,I.appendChild(b)}),w.appendChild(I);const N="http://www.w3.org/2000/svg",E=document.createElementNS(N,"svg");E.setAttribute("class","nf-engine-waves"),E.setAttribute("viewBox","0 0 560 140"),E.setAttribute("preserveAspectRatio","none"),E.id="nf-engine-waves";for(let u=0;u<4;u++){const b=document.createElementNS(N,"path");b.setAttribute("fill","none"),b.setAttribute("stroke-width",u<2?"1.5":"1"),b.setAttribute("stroke",u<2?`rgba(${Z.rgb},${.14+u*.1})`:`rgba(${Z.accentRgb},${.1+(u-2)*.08})`),b.setAttribute("data-wave-idx",String(u)),E.appendChild(b)}w.appendChild(E);const $=document.createElement("div");$.className="nf-engine-brand-inner",$.innerHTML=`

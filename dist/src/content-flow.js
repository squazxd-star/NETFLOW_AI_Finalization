...
@media (max-height: 600px) {
    ...
    `}function et(){de||(de=document.createElement("style"),de.id="netflow-overlay-styles",de.textContent=Mt(Q),document.head.appendChild(de))}function tt(t){t.innerHTML="",j.forEach((e,i)=>{const o=document.createElement("div");o.className="nf-proc-row nf-proc-waiting",o.id=`nf-proc-${e.stepId}`,o.innerHTML=`
            <span class="nf-proc-num">${i+1}</span>
            <span class="nf-proc-dot"></span>
            <span class="nf-proc-label">${e.label}</span>
            <span class="nf-proc-badge">(queued)</span>
        `,t.appendChild(o)})}function Pt(){const t=document.getElementById("nf-terminal");if(!t)return;tt(t);const e=document.getElementById("nf-step-counter");e&&(e.textContent=`0/${j.length}`)}function nt(t,e){let s="";for(let _=0;_<20;_++){const N=_/20*Math.PI*2,R=(_+.2)/20*Math.PI*2,$=(_+.5)/20*Math.PI*2,r=(_+.8)/20*Math.PI*2,C=(_+1)/20*Math.PI*2;s+=`${_===0?"M":"L"}${(120+100*Math.cos(N)).toFixed(1)},${(120+100*Math.sin(N)).toFixed(1)} `,s+=`L${(120+100*Math.cos(R)).toFixed(1)},${(120+100*Math.sin(R)).toFixed(1)} `,s+=`L${(120+112*Math.cos($)).toFixed(1)},${(120+112*Math.sin($)).toFixed(1)} `,s+=`L${(120+100*Math.cos(r)).toFixed(1)},${(120+100*Math.sin(r)).toFixed(1)} `,s+=`L${(120+100*Math.cos(C)).toFixed(1)},${(120+100*Math.sin(C)).toFixed(1)} `}s+="Z";const l=14,d=72,m=62;let w="";for(let _=0;_<l;_++){const N=_/l*Math.PI*2,R=(_+.25)/l*Math.PI*2,$=(_+.75)/l*Math.PI*2,r=(_+1)/l*Math.PI*2;w+=`${_===0?"M":"L"}${(120+m*Math.cos(N)).toFixed(1)},${(120+m*Math.sin(N)).toFixed(1)} `,w+=`L${(120+d*Math.cos(R)).toFixed(1)},${(120+d*Math.sin(R)).toFixed(1)} `,w+=`L${(120+d*Math.cos($)).toFixed(1)},${(120+d*Math.sin($)).toFixed(1)} `,w+=`L${(120+m*Math.cos(r)).toFixed(1)},${(120+m*Math.sin(r)).toFixed(1)} `}return w+="Z",`<svg width="72" height="72" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
    ...
    `,h.appendChild(I);const O=document.createElement("div");O.className="nf-engine-stats",O.id="nf-engine-stats",O.innerHTML=[["SCENES","nf-stat-scenes","1/1"],["ELAPSED","nf-stat-elapsed","00:00"],["STEP","nf-stat-step","0/0"],["STATUS","nf-stat-status","READY"],["PROGRESS","nf-stat-progress","—"]].map(([u,y,z])=>`<div class="nf-stat-item"><span class="nf-stat-label">${u}</span><span class="nf-stat-val" id="${y}">${z}</span></div>`).join(""),h.appendChild(O),x.appendChild(h),k.appendChild(x);const b=["nf-mod-tl","nf-mod-tr","nf-mod-bl","nf-mod-br"];fe.forEach((u,y)=>{const z=_t(u);z.classList.add(b[y]),z.id=`nf-mod-${u.id}`,k.appendChild(z)}),t.appendChild(k);for(let u=0;u<30;u++){const y=document.createElement("div");y.className="nf-particle",y.style.left=`${5+Math.random()*90}%`,y.style.bottom=`${Math.random()*40}%`,y.style.animationDuration=`${3+Math.random()*5}s`,y.style.animationDelay=`${Math.random()*4}s`;const z=.3+Math.random()*.4,G=.7+Math.random()*.3;y.style.background=`rgba(${Math.floor(oe*G)}, ${Math.floor(ie*G)}, ${Math.floor(ae*G)}, ${z})`,y.style.width=`${1+Math.random()*2}px`,y.style.height=y.style.width,t.appendChild(y)}return t}function _t(t){const e=document.createElement("div");e.className="nf-module";const i=document.createElement("div");i.className="nf-mod-header",i.innerHTML=`
    ...
    `,e.appendChild(i),t.steps.forEach(a=>{const p=document.createElement("div");p.className="nf-step",p.id=`nf-step-${a.id}`;let c="";a.progress!==void 0&&(c=`
            ...
            ${c}
        `,e.appendChild(p)});const o=document.createElement("div");return o.className="nf-mod-progress",o.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${t.id}"></div>`,e.appendChild(o),e}function Tt(){Ze=Date.now(),Se=setInterval(()=>{const t=Math.floor((Date.now()-Ze)/1e3),e=String(Math.floor(t/60)).padStart(2,"0"),i=String(t%60).padStart(2,"0"),o=document.getElementById("nf-timer");o&&(o.textContent=`${e}:${i}`);const a=document.getElementById("nf-stat-elapsed");a&&(a.textContent=`${e}:${i}`)},1e3)}function ot(){Se&&(clearInterval(Se),Se=null)}const At=120,it=160,at=.4;let me=null,rt=0,st=0,lt=0,Ee=[];function Rt(t,e){Ee=[];for(let i=0;i<At;i++){const o=Math.random();let a;o<.22?a=0:o<.4?a=1:o<.55?a=2:o<.68?a=3:o<.84?a=4:a=5;const p=Math.random()*t,c=Math.random()*e,s=50+Math.random()*220,l=Math.random()*Math.PI*2,d=(.003+Math.random()*.008)*(Math.random()>.5?1:-1);Ee.push({x:a===0?Math.random()*t:p+Math.cos(l)*s,y:a===0?Math.random()*e:c+Math.sin(l)*s,vx:(Math.random()-.5)*at,vy:(Math.random()-.5)*at,radius:1.2+Math.random()*2.5,pulsePhase:Math.random()*Math.PI*2,pulseSpeed:.02+Math.random()*.03,motion:a,oCx:p,oCy:c,oRadius:s,oAngle:l,oSpeed:d})}}function Bt(){if(!le)return;const t=le;if(ve=t.getContext("2d"),!ve)return;const e=()=>{t.width=window.innerWidth,t.height=window.innerHeight,Ee.length===0&&Rt(t.width,t.height)};e(),window.addEventListener("resize",e);let i=null,o=0,a=0,p=!1;function c(){if(!ve||!le){ye=null;return}if(ye=requestAnimationFrame(c),p=!p,p)return;const s=ve,l=le.width,d=le.height;s.fillStyle=`rgba(${oe*.04|0},${ie*.04|0},${ae*.06|0},1)`,s.fillRect(0,0,l,d),(!i||o!==l||a!==d)&&(o=l,a=d,i=s.createRadialGradient(l*.5,d*.5,0,l*.5,d*.5,Math.max(l,d)*.6),i.addColorStop(0,`rgba(${oe*.08|0},${ie*.08|0},${ae*.1|0},0.4)`),i.addColorStop(1,"rgba(0,0,0,0)")),s.fillStyle=i,s.fillRect(0,0,l,d);const m=Ee,w=m.length,_=it*it;for(let $=0;$<w;$++){const r=m[$];if(r.pulsePhase+=r.pulseSpeed,r.motion===0)r.x+=r.vx,r.y+=r.vy,r.x<0?(r.x=0,r.vx=Math.abs(r.vx)*(.8+Math.random()*.4)):r.x>l&&(r.x=l,r.vx=-Math.abs(r.vx)*(.8+Math.random()*.4)),r.y<0?(r.y=0,r.vy=Math.abs(r.vy)*(.8+Math.random()*.4)):r.y>d&&(r.y=d,r.vy=-Math.abs(r.vy)*(.8+Math.random()*.4));else if(r.motion===1)r.oAngle+=r.oSpeed,r.x=r.oCx+Math.cos(r.oAngle)*r.oRadius,r.y=r.oCy+Math.sin(r.oAngle)*r.oRadius,r.oCx+=Math.sin(r.oAngle*.3)*.15,r.oCy+=Math.cos(r.oAngle*.3)*.15;else if(r.motion===2)r.oAngle+=r.oSpeed,r.x=r.oCx+Math.cos(r.oAngle)*r.oRadius,r.y=r.oCy+Math.sin(r.oAngle)*r.oRadius*.5,r.oCx+=Math.sin(r.oAngle*.2)*.1,r.oCy+=Math.cos(r.oAngle*.2)*.1;else if(r.motion===3){r.oAngle+=r.oSpeed;const C=r.oAngle,v=r.oRadius*.7;r.x=r.oCx+v*Math.cos(C),r.y=r.oCy+v*Math.sin(C)*Math.cos(C),r.oCx+=Math.sin(C*.15)*.12,r.oCy+=Math.cos(C*.15)*.12}else if(r.motion===4){r.oAngle+=r.oSpeed*1.2;const C=r.oRadius*(.5+.5*Math.abs(Math.sin(r.oAngle*.15)));r.x=r.oCx+Math.cos(r.oAngle)*C,r.y=r.oCy+Math.sin(r.oAngle)*C,r.oCx+=Math.sin(r.oAngle*.1)*.18,r.oCy+=Math.cos(r.oAngle*.1)*.18}else r.oAngle+=r.oSpeed,r.x+=r.vx*.8,r.y=r.oCy+Math.sin(r.oAngle+r.x*.008)*r.oRadius*.35,r.x<-30?r.x=l+30:r.x>l+30&&(r.x=-30),r.oCy+=Math.sin(r.oAngle*.1)*.08;if(r.motion>0){const C=r.oRadius+50;r.oCx<-C?r.oCx=l+C:r.oCx>l+C&&(r.oCx=-C),r.oCy<-C?r.oCy=d+C:r.oCy>d+C&&(r.oCy=-C)}}s.beginPath(),s.strokeStyle=`rgba(${oe},${ie},${ae},0.06)`,s.lineWidth=.4;const N=new Path2D;for(let $=0;$<w;$++){const r=m[$];for(let C=$+1;C<w;C++){const v=m[C],B=r.x-v.x,M=r.y-v.y,k=B*B+M*M;k<_&&(1-k/_<.4?(s.moveTo(r.x,r.y),s.lineTo(v.x,v.y)):(N.moveTo(r.x,r.y),N.lineTo(v.x,v.y)))}}if(s.stroke(),s.strokeStyle=`rgba(${oe},${ie},${ae},0.18)`,s.lineWidth=.8,s.stroke(N),!me||rt!==oe||st!==ie||lt!==ae){me=document.createElement("canvas");const $=48;me.width=$,me.height=$;const r=me.getContext("2d"),C=r.createRadialGradient($/2,$/2,0,$/2,$/2,$/2);C.addColorStop(0,`rgba(${oe},${ie},${ae},0.9)`),C.addColorStop(.3,`rgba(${oe},${ie},${ae},0.35)`),C.addColorStop(1,`rgba(${oe},${ie},${ae},0)`),r.fillStyle=C,r.fillRect(0,0,$,$),rt=oe,st=ie,lt=ae}const R=me;for(let $=0;$<w;$++){const r=m[$],C=.6+.4*Math.sin(r.pulsePhase),v=r.radius*5*(.8+C*.4);s.globalAlpha=.5+C*.4,s.drawImage(R,r.x-v/2,r.y-v/2,v,v)}s.globalAlpha=1,s.fillStyle="rgba(255,255,255,0.45)",s.beginPath();for(let $=0;$<w;$++){const r=m[$];if(r.radius>2){const C=.6+.4*Math.sin(r.pulsePhase),v=r.radius*(.8+C*.4)*.35;s.moveTo(r.x+v,r.y),s.arc(r.x,r.y,v,0,Math.PI*2)}}s.fill()}c()}function Ft(){ye!==null&&(cancelAnimationFrame(ye),ye=null),le=null,ve=null,Ee=[]}let Ce=null;const Te=560,Dt=140,ct=Te/2,dt=Dt/2,pt=[];for(let t=0;t<=Te;t+=8){const e=Math.abs(t-ct)/ct;pt.push(Math.pow(Math.min(1,e*1.6),.6))}const Nt=[0,1,2,3].map(t=>({amp:10+t*5,freq:(1.2+t*.35)*Math.PI*2/Te,off:t*.6,spd:.7+t*.12}));let Le=!1;function ft(){if(xe=requestAnimationFrame(ft),Le=!Le,Le)return;if(Oe+=.07,!Ce){const e=document.getElementById("nf-engine-waves");if(!e){xe=null;return}Ce=Array.from(e.querySelectorAll("path"))}const t=[];for(let e=0;e<Ce.length;e++){const i=Nt[e],o=Oe*i.spd+i.off;t.length=0,t.push(`M 0 ${dt}`);let a=0;for(let p=0;p<=Te;p+=8){const c=dt+i.amp*pt[a++]*Math.sin(p*i.freq+o);t.push(`L${p} ${c*10+.5|0}`)}Ce[e].setAttribute("d",t.join(" "))}}function Ot(){Oe=0,ft(),Bt(),_e=setInterval(()=>{const t=document.getElementById("nf-stat-freq"),e=document.getElementById("nf-stat-lat1"),i=document.getElementById("nf-stat-lat2"),o=document.getElementById("nf-stat-buf");t&&(t.textContent=`${(4.5+Math.random()*.5).toFixed(1)} GHz`),e&&(e.textContent=`${Math.floor(12+Math.random()*10)}ms`),i&&(i.textContent=`${Math.floor(12+Math.random()*10)}ms`),o&&(o.textContent=`${Math.floor(90+Math.random()*9)}%`)},2e3)}function ut(){xe!==null&&(cancelAnimationFrame(xe),xe=null),_e&&(clearInterval(_e),_e=null),Ce=null,Ft()}function Ve(){let t=0;const e=j.filter(d=>d.status!=="skipped").length;for(const d of j){const m=document.getElementById(`nf-proc-${d.stepId}`);if(!m)continue;m.className="nf-proc-row";const w=m.querySelector(".nf-proc-badge");switch(d.status){case"done":m.classList.add("nf-proc-done"),w&&(w.textContent="✅ done"),t++;break;case"active":m.classList.add("nf-proc-active"),w&&(w.textContent=d.progress!==void 0&&d.progress>0?`⏳ ${d.progress}%`:"⏳ active");break;case"error":m.classList.add("nf-proc-error"),w&&(w.textContent="❌ error");break;case"skipped":m.classList.add("nf-proc-skipped"),w&&(w.textContent="— skip");break;default:m.classList.add("nf-proc-waiting"),w&&(w.textContent="(queued)")}}const i=j.findIndex(d=>d.status==="active"),o=i>=0?i+1:t>=e&&e>0?j.length:t,a=document.getElementById("nf-step-counter");a&&(a.textContent=`${o}/${j.length}`);const p=document.querySelector(".nf-core-title-val"),c=document.querySelector(".nf-status-dot");t>=e&&e>0?(p&&(p.textContent="COMPLETE",p.style.color=Q.doneHex),c&&(c.style.background=Q.doneHex,c.style.boxShadow=`0 0 8px rgba(${Q.doneRgb},0.7)`)):j.some(m=>m.status==="error")?(p&&(p.textContent="ERROR",p.style.color="#f87171"),c&&(c.style.background="#ef4444",c.style.boxShadow="0 0 8px rgba(239,68,68,0.7)")):j.some(m=>m.status==="active")&&p&&(p.textContent="ACTIVE",p.style.color=Q.hex,p.style.textShadow=`0 0 10px rgba(${Q.rgb},0.5)`);const s=document.getElementById("nf-terminal"),l=s==null?void 0:s.querySelector(".nf-proc-active");l&&s&&l.scrollIntoView({behavior:"smooth",block:"center"})}function gt(){J&&J.isConnected||(et(),J=document.createElement("button"),J.id="nf-toggle-btn",J.className="nf-toggle-visible",J.innerHTML=pe?Qe:Je,J.title="ซ่อน/แสดง Netflow Overlay",J.onclick=()=>Ge(),document.body.appendChild(J))}function Ge(){K&&(gt(),pe?(K.classList.remove("nf-hidden"),K.classList.add("nf-visible"),J&&(J.innerHTML=Je),pe=!1):(K.classList.remove("nf-visible"),K.classList.add("nf-hidden"),J&&(J.innerHTML=Qe),pe=!0))}const mt={red:"themes/theme-red.jpg",yellow:"themes/theme-yellow.jpg",blue:"themes/theme-blue.jpg",purple:"themes/theme-purple.jpg",green:"themes/theme-green.jpg"};function ht(){const t=document.getElementById("nf-core-monitor");if(!t)return;let e=we;if(!e)try{e=localStorage.getItem("netflow_app_theme")||"green"}catch{e="green"}const i=mt[e]||mt.green;let o;try{o=chrome.runtime.getURL(i)}catch{o=`/${i}`}const a=Q.rgb;t.style.backgroundImage=["linear-gradient(180deg, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.78) 45%, rgba(0,0,0,0.90) 100%)",`linear-gradient(180deg, rgba(${a},0.25) 0%, rgba(${a},0.12) 50%, rgba(${a},0.20) 100%)`,`url('${o}')`].join(", "),t.style.backgroundSize="auto, auto, 50%",t.style.backgroundPosition="center, center, center",t.style.backgroundRepeat="no-repeat, no-repeat, no-repeat",t.style.setProperty("--nf-bg-set","1"),t.style.border=`1.5px solid rgba(${a},0.45)`,t.style.boxShadow=`0 0 70px rgba(${a},0.22), 0 0 140px rgba(${a},0.1), inset 0 1px 0 rgba(${a},0.15)`}function qe(t=1){if(Q=It(),Ke(),K&&K.isConnected){pe&&Ge();return}if(K&&!K.isConnected&&(K=null),de&&(de.remove(),de=null),et(),$e=t,j=ze(t),t>1){const e=fe.find(o=>o.id==="video");if(e){const o=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:"waiting",progress:0}];for(let a=2;a<=t;a++)o.push({id:`scene${a}-prompt`,label:`Scene ${a} Prompt`,status:"waiting"}),o.push({id:`scene${a}-gen`,label:`Scene ${a} Generate`,status:"waiting"}),o.push({id:`scene${a}-wait`,label:`Scene ${a} รอผล`,status:"waiting",progress:0});e.steps=o}const i=fe.find(o=>o.id==="render");if(i){const o=i.steps.find(p=>p.id==="download");o&&(o.label="ดาวน์โหลด 720p");const a=i.steps.find(p=>p.id==="upscale");a&&(a.label="Full Video")}}K=St(),document.body.appendChild(K),pe=!1,gt(),Tt(),Ot(),requestAnimationFrame(()=>ht())}function bt(){ot(),ut(),pe=!1,K&&(K.classList.add("nf-fade-out"),setTimeout(()=>{K==null||K.remove(),K=null},500)),J&&(J.remove(),J=null)}const zt={settings:"CONFIG","upload-char":"UPLOAD","upload-prod":"UPLOAD","img-prompt":"PROMPT","img-generate":"GENERATE","img-wait":"IMG WAIT",animate:"ANIMATE","vid-prompt":"VID PROMPT","vid-generate":"VID GEN","vid-wait":"VID WAIT",download:"DOWNLOAD",upscale:"UPSCALE",open:"OPENING"};function Lt(t,e,i){const o=j.findIndex(m=>m.status==="active"),a=j.filter(m=>m.status==="done").length,p=j.length,c=o>=0?o+1:a>=p?p:a,s=document.getElementById("nf-stat-step");s&&(s.textContent=`${c}/${p}`);let l=1;for(const m of j)if(m.status==="active"||m.status==="done")if(m.stepId.startsWith("scene")){const w=m.stepId.match(/^scene(\d+)-/);w&&(l=Math.max(l,parseInt(w[1],10)))}else(m.stepId==="download"||m.stepId==="upscale"||m.stepId==="open")&&(l=$e);const d=document.getElementById("nf-stat-scenes");if(d&&(d.textContent=$e>1?`${l}/${$e}`:"1/1"),e==="active"){const m=document.getElementById("nf-stat-status"),w=zt[t]||t.toUpperCase();m&&(m.textContent=w)}else if(e==="done"&&a>=p){const m=document.getElementById("nf-stat-status");m&&(m.textContent="COMPLETE")}else if(e==="error"){const m=document.getElementById("nf-stat-status");m&&(m.textContent="ERROR")}if(i!==void 0&&i>0){const m=document.getElementById("nf-stat-progress");m&&(m.textContent=`${Math.min(100,i)}%`)}}function S(t,e,i){if(!K)return;for(const a of fe)for(const p of a.steps)p.id===t&&(p.status=e,i!==void 0&&(p.progress=i));for(const a of j)a.stepId===t&&(a.status=e,i!==void 0&&(a.progress=i));const o=document.getElementById(`nf-step-${t}`);if(o&&(o.className="nf-step",e==="active"?o.classList.add("nf-step-active"):e==="done"?o.classList.add("nf-step-done"):e==="error"&&o.classList.add("nf-step-error")),Lt(t,e,i),i!==void 0){const a=document.getElementById(`nf-bar-${t}`);a&&(a.style.width=`${Math.min(100,i)}%`)}He(),Ve()}function he(t){S(t,"skipped");const e=document.getElementById(`nf-step-${t}`);e&&(e.style.opacity="0.2")}function ke(t=4e3){ot(),ut(),He(),Ve(),setTimeout(()=>bt(),t)}function He(){for(const t of fe){const e=t.steps.filter(l=>l.status!=="skipped").length,i=t.steps.filter(l=>l.status==="done").length,o=t.steps.some(l=>l.status==="active"),a=e>0?Math.round(i/e*100):0,p=document.getElementById(`nf-pct-${t.id}`);p&&(p.textContent=`${a}%`);const c=document.getElementById(`nf-modbar-${t.id}`);c&&(c.style.width=`${a}%`);const s=document.getElementById(`nf-mod-${t.id}`);s&&(s.classList.remove("nf-active","nf-done"),a>=100?s.classList.add("nf-done"):o&&s.classList.add("nf-active"))}}function Vt(t){var o,a,p,c;$e=t;const e=new Map;for(const s of j)e.set(s.stepId,{status:s.status,progress:s.progress});j=ze(t);for(const s of j){const l=e.get(s.stepId);l&&(s.status=l.status,l.progress!==void 0&&(s.progress=l.progress))}if(Pt(),t>1){const s=fe.find(l=>l.id==="video");if(s){const l=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:((o=s.steps.find(d=>d.id==="animate"))==null?void 0:o.status)||"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:((a=s.steps.find(d=>d.id==="vid-prompt"))==null?void 0:a.status)||"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:((p=s.steps.find(d=>d.id==="vid-generate"))==null?void 0:p.status)||"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:((c=s.steps.find(d=>d.id==="vid-wait"))==null?void 0:c.status)||"waiting",progress:0}];for(let d=2;d<=t;d++)l.push({id:`scene${d}-prompt`,label:`Scene ${d} Prompt`,status:"waiting"}),l.push({id:`scene${d}-gen`,label:`Scene ${d} Generate`,status:"waiting"}),l.push({id:`scene${d}-wait`,label:`Scene ${d} รอผล`,status:"waiting",progress:0});s.steps=l,wt(s)}}const i=fe.find(s=>s.id==="render");if(i&&t>1){const s=i.steps.find(d=>d.id==="download");s&&(s.label="ดาวน์โหลด 720p");const l=i.steps.find(d=>d.id==="upscale");l&&(l.label="Full Video"),wt(i)}He(),Ve()}function wt(t){const e=document.getElementById(`nf-mod-${t.id}`);if(!e)return;e.querySelectorAll(".nf-step, .nf-mod-progress").forEach(a=>a.remove()),t.steps.forEach(a=>{const p=document.createElement("div");p.className="nf-step",p.id=`nf-step-${a.id}`;let c="";a.progress!==void 0&&(c=`
                <div class="nf-progress-bar">
                    <div class="nf-progress-fill" id="nf-bar-${a.id}" style="width: 0%"></div>
                </div>
            `),p.innerHTML=`

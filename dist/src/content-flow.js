...
@media (max-height: 600px) {
    ...
    `}function et(){de||(de=document.createElement("style"),de.id="netflow-overlay-styles",de.textContent=It(Q),document.head.appendChild(de))}function tt(t){t.innerHTML="",j.forEach((e,i)=>{const o=document.createElement("div");o.className="nf-proc-row nf-proc-waiting",o.id=`nf-proc-${e.stepId}`,o.innerHTML=`
            <span class="nf-proc-num">${i+1}</span>
            <span class="nf-proc-dot"></span>
            <span class="nf-proc-label">${e.label}</span>
            <span class="nf-proc-badge">(queued)</span>
        `,t.appendChild(o)})}function Mt(){const t=document.getElementById("nf-terminal");if(!t)return;tt(t);const e=document.getElementById("nf-step-counter");e&&(e.textContent=`0/${j.length}`)}function nt(t,e){let s="";for(let S=0;S<20;S++){const N=S/20*Math.PI*2,R=(S+.2)/20*Math.PI*2,$=(S+.5)/20*Math.PI*2,r=(S+.8)/20*Math.PI*2,C=(S+1)/20*Math.PI*2;s+=`${S===0?"M":"L"}${(120+100*Math.cos(N)).toFixed(1)},${(120+100*Math.sin(N)).toFixed(1)} `,s+=`L${(120+100*Math.cos(R)).toFixed(1)},${(120+100*Math.sin(R)).toFixed(1)} `,s+=`L${(120+112*Math.cos($)).toFixed(1)},${(120+112*Math.sin($)).toFixed(1)} `,s+=`L${(120+100*Math.cos(r)).toFixed(1)},${(120+100*Math.sin(r)).toFixed(1)} `,s+=`L${(120+100*Math.cos(C)).toFixed(1)},${(120+100*Math.sin(C)).toFixed(1)} `}s+="Z";const l=14,d=72,m=62;let w="";for(let S=0;S<l;S++){const N=S/l*Math.PI*2,R=(S+.25)/l*Math.PI*2,$=(S+.75)/l*Math.PI*2,r=(S+1)/l*Math.PI*2;w+=`${S===0?"M":"L"}${(120+m*Math.cos(N)).toFixed(1)},${(120+m*Math.sin(N)).toFixed(1)} `,w+=`L${(120+d*Math.cos(R)).toFixed(1)},${(120+d*Math.sin(R)).toFixed(1)} `,w+=`L${(120+d*Math.cos($)).toFixed(1)},${(120+d*Math.sin($)).toFixed(1)} `,w+=`L${(120+m*Math.cos(r)).toFixed(1)},${(120+m*Math.sin(r)).toFixed(1)} `}return w+="Z",`<svg width="72" height="72" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
    ...
    `,h.appendChild(_);const O=document.createElement("div");O.className="nf-engine-stats",O.id="nf-engine-stats",O.innerHTML=[["SCENES","nf-stat-scenes","1/1"],["ELAPSED","nf-stat-elapsed","00:00"],["STEP","nf-stat-step","0/0"],["STATUS","nf-stat-status","READY"],["PROGRESS","nf-stat-progress","—"]].map(([u,y,z])=>`<div class="nf-stat-item"><span class="nf-stat-label">${u}</span><span class="nf-stat-val" id="${y}">${z}</span></div>`).join(""),h.appendChild(O),x.appendChild(h),k.appendChild(x);const b=["nf-mod-tl","nf-mod-tr","nf-mod-bl","nf-mod-br"];fe.forEach((u,y)=>{const z=St(u);z.classList.add(b[y]),z.id=`nf-mod-${u.id}`,k.appendChild(z)}),t.appendChild(k);for(let u=0;u<30;u++){const y=document.createElement("div");y.className="nf-particle",y.style.left=`${5+Math.random()*90}%`,y.style.bottom=`${Math.random()*40}%`,y.style.animationDuration=`${3+Math.random()*5}s`,y.style.animationDelay=`${Math.random()*4}s`;const z=.3+Math.random()*.4,G=.7+Math.random()*.3;y.style.background=`rgba(${Math.floor(oe*G)}, ${Math.floor(ie*G)}, ${Math.floor(ae*G)}, ${z})`,y.style.width=`${1+Math.random()*2}px`,y.style.height=y.style.width,t.appendChild(y)}return t}function St(t){const e=document.createElement("div");e.className="nf-module";const i=document.createElement("div");i.className="nf-mod-header",i.innerHTML=`

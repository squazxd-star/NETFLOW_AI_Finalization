...
@media (max-height: 600px) {
    ...
    `}function Xt(){tt||(tt=document.createElement("style"),tt.id="netflow-overlay-styles",tt.textContent=Xe(lt),document.head.appendChild(tt))}function be(e){e.innerHTML="",it.forEach((t,o)=>{const l=document.createElement("div");l.className="nf-proc-row nf-proc-waiting",l.id=`nf-proc-${t.stepId}`,l.innerHTML=`
            <span class="nf-proc-num">${o+1}</span>
            <span class="nf-proc-dot"></span>
            <span class="nf-proc-label">${t.label}</span>
            <span class="nf-proc-badge">(queued)</span>
        `,e.appendChild(l)})}function xe(){const e=document.getElementById("nf-terminal");if(!e)return;be(e);const t=document.getElementById("nf-step-counter");t&&(t.textContent=`0/${it.length}`)}function ye(e,t){let s="";for(let v=0;v<20;v++){const B=v/20*Math.PI*2,R=(v+.2)/20*Math.PI*2,I=(v+.5)/20*Math.PI*2,c=(v+.8)/20*Math.PI*2,k=(v+1)/20*Math.PI*2;s+=`${v===0?"M":"L"}${(120+100*Math.cos(B)).toFixed(1)},${(120+100*Math.sin(B)).toFixed(1)} `,s+=`L${(120+100*Math.cos(R)).toFixed(1)},${(120+100*Math.sin(R)).toFixed(1)} `,s+=`L${(120+112*Math.cos(I)).toFixed(1)},${(120+112*Math.sin(I)).toFixed(1)} `,s+=`L${(120+100*Math.cos(c)).toFixed(1)},${(120+100*Math.sin(c)).toFixed(1)} `,s+=`L${(120+100*Math.cos(k)).toFixed(1)},${(120+100*Math.sin(k)).toFixed(1)} `}s+="Z";const a=14,d=72,m=62;let g="";for(let v=0;v<a;v++){const B=v/a*Math.PI*2,R=(v+.25)/a*Math.PI*2,I=(v+.75)/a*Math.PI*2,c=(v+1)/a*Math.PI*2;g+=`${v===0?"M":"L"}${(120+m*Math.cos(B)).toFixed(1)},${(120+m*Math.sin(B)).toFixed(1)} `,g+=`L${(120+d*Math.cos(R)).toFixed(1)},${(120+d*Math.sin(R)).toFixed(1)} `,g+=`L${(120+d*Math.cos(I)).toFixed(1)},${(120+d*Math.sin(I)).toFixed(1)} `,g+=`L${(120+m*Math.cos(c)).toFixed(1)},${(120+m*Math.sin(c)).toFixed(1)} `}return g+="Z",`<svg width="72" height="72" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
    ...
    `,g.appendChild(I);const c=document.createElement("div");c.className="nf-engine-stats",c.id="nf-engine-stats",c.innerHTML=[["SCENES","nf-stat-scenes","1/1"],["ELAPSED","nf-stat-elapsed","00:00"],["STEP","nf-stat-step","0/0"],["STATUS","nf-stat-status","READY"],["PROGRESS","nf-stat-progress","—"],["FPS","nf-stat-fps","--"]].map(([$,x,C])=>`<div class="nf-stat-item"><span class="nf-stat-label">${$}</span><span class="nf-stat-val" id="${x}">${C}</span></div>`).join(""),g.appendChild(c),a.appendChild(g),s.appendChild(a);const k=["nf-mod-tl","nf-mod-tr","nf-mod-bl","nf-mod-br"];ut.forEach(($,x)=>{const C=Ze($);C.classList.add(k[x]),C.id=`nf-mod-${$.id}`,s.appendChild(C)}),e.appendChild(s);for(let $=0;$<10;$++){const x=document.createElement("div");x.className="nf-particle",x.style.left=`${5+Math.random()*90}%`,x.style.bottom=`${Math.random()*40}%`,x.style.animationDuration=`${3+Math.random()*5}s`,x.style.animationDelay=`${Math.random()*4}s`;const C=.3+Math.random()*.4,y=.7+Math.random()*.3;x.style.background=`rgba(${Math.floor(ct*y)}, ${Math.floor(dt*y)}, ${Math.floor(pt*y)}, ${C})`,x.style.width=`${1+Math.random()*2}px`,x.style.height=x.style.width,e.appendChild(x)}return e}function Ze(e){const t=document.createElement("div");t.className="nf-module";const o=document.createElement("div");o.className="nf-mod-header",o.innerHTML=`
        <div class="nf-mod-title">${e.title}</div>
        <span class="nf-mod-pct" id="nf-pct-${e.id}">0%</span>
    `,t.appendChild(o),e.steps.forEach(r=>{const p=document.createElement("div");p.className="nf-step",p.id=`nf-step-${r.id}`;let i="";r.progress!==void 0&&(i=`
            ...
            ${i}
        `,t.appendChild(p)});const l=document.createElement("div");return l.className="nf-mod-progress",l.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${e.id}"></div>`,t.appendChild(l),t}function Je(){he=Date.now(),Wt=setInterval(()=>{const e=Math.floor((Date.now()-he)/1e3),t=String(Math.floor(e/60)).padStart(2,"0"),o=String(e%60).padStart(2,"0"),l=document.getElementById("nf-timer");l&&(l.textContent=`${t}:${o}`);const r=document.getElementById("nf-stat-elapsed");r&&(r.textContent=`${t}:${o}`)},1e3)}function ve(){Wt&&(clearInterval(Wt),Wt=null)}const tn=50,$e=120,Ee=.4;let It=null,ke=0,Ce=0,Te=0,Bt=[];function en(e,t){Bt=[];for(let o=0;o<tn;o++){const l=Math.random();let r;l<.22?r=0:l<.4?r=1:l<.55?r=2:l<.68?r=3:l<.84?r=4:r=5;const p=Math.random()*e,i=Math.random()*t,s=50+Math.random()*220,a=Math.random()*Math.PI*2,d=(.003+Math.random()*.008)*(Math.random()>.5?1:-1);Bt.push({x:r===0?Math.random()*e:p+Math.cos(a)*s,y:r===0?Math.random()*t:i+Math.sin(a)*s,vx:(Math.random()-.5)*Ee,vy:(Math.random()-.5)*Ee,radius:1.2+Math.random()*2.5,pulsePhase:Math.random()*Math.PI*2,pulseSpeed:.02+Math.random()*.03,motion:r,oCx:p,oCy:i,oRadius:s,oAngle:a,oSpeed:d})}}function _e(){if(!ft)return;const e=ft;if(_t=e.getContext("2d"),!_t)return;const t=()=>{e.width=window.innerWidth,e.height=window.innerHeight,Bt.length===0&&en(e.width,e.height)};t(),window.addEventListener("resize",t);let o=null,l=0,r=0,p=0;function i(){if(!_t||!ft){ht=null;return}if(ht=requestAnimationFrame(i),jt++,p++,p%3!==0)return;const s=_t,a=ft.width,d=ft.height;s.fillStyle=`rgba(${ct*.04|0},${dt*.04|0},${pt*.06|0},1)`,s.fillRect(0,0,a,d),(!o||l!==a||r!==d)&&(l=a,r=d,o=s.createRadialGradient(a*.5,d*.5,0,a*.5,d*.5,Math.max(a,d)*.6),o.addColorStop(0,`rgba(${ct*.08|0},${dt*.08|0},${pt*.1|0},0.4)`),o.addColorStop(1,"rgba(0,0,0,0)")),s.fillStyle=o,s.fillRect(0,0,a,d);const m=Bt,g=m.length,v=$e*$e;for(let I=0;I<g;I++){const c=m[I];if(c.pulsePhase+=c.pulseSpeed,c.motion===0)c.x+=c.vx,c.y+=c.vy,c.x<0?(c.x=0,c.vx=Math.abs(c.vx)*(.8+Math.random()*.4)):c.x>a&&(c.x=a,c.vx=-Math.abs(c.vx)*(.8+Math.random()*.4)),c.y<0?(c.y=0,c.vy=Math.abs(c.vy)*(.8+Math.random()*.4)):c.y>d&&(c.y=d,c.vy=-Math.abs(c.vy)*(.8+Math.random()*.4));else if(c.motion===1)c.oAngle+=c.oSpeed,c.x=c.oCx+Math.cos(c.oAngle)*c.oRadius,c.y=c.oCy+Math.sin(c.oAngle)*c.oRadius,c.oCx+=Math.sin(c.oAngle*.3)*.15,c.oCy+=Math.cos(c.oAngle*.3)*.15;else if(c.motion===2)c.oAngle+=c.oSpeed,c.x=c.oCx+Math.cos(c.oAngle)*c.oRadius,c.y=c.oCy+Math.sin(c.oAngle)*c.oRadius*.5,c.oCx+=Math.sin(c.oAngle*.2)*.1,c.oCy+=Math.cos(c.oAngle*.2)*.1;else if(c.motion===3){c.oAngle+=c.oSpeed;const k=c.oAngle,$=c.oRadius*.7;c.x=c.oCx+$*Math.cos(k),c.y=c.oCy+$*Math.sin(k)*Math.cos(k),c.oCx+=Math.sin(k*.15)*.12,c.oCy+=Math.cos(k*.15)*.12}else if(c.motion===4){c.oAngle+=c.oSpeed*1.2;const k=c.oRadius*(.5+.5*Math.abs(Math.sin(c.oAngle*.15)));c.x=c.oCx+Math.cos(c.oAngle)*k,c.y=c.oCy+Math.sin(c.oAngle)*k,c.oCx+=Math.sin(c.oAngle*.1)*.18,c.oCy+=Math.cos(c.oAngle*.1)*.18}else c.oAngle+=c.oSpeed,c.x+=c.vx*.8,c.y=c.oCy+Math.sin(c.oAngle+c.x*.008)*c.oRadius*.35,c.x<-30?c.x=a+30:c.x>a+30&&(c.x=-30),c.oCy+=Math.sin(c.oAngle*.1)*.08;if(c.motion>0){const k=c.oRadius+50;c.oCx<-k?c.oCx=a+k:c.oCx>a+k&&(c.oCx=-k),c.oCy<-k?c.oCy=d+k:c.oCy>d+k&&(c.oCy=-k)}}s.beginPath(),s.strokeStyle=`rgba(${ct},${dt},${pt},0.06)`,s.lineWidth=.4;const B=new Path2D;for(let I=0;I<g;I++){const c=m[I];for(let k=I+1;k<g;k++){const $=m[k],x=c.x-$.x,C=c.y-$.y,y=x*x+C*C;y<v&&(1-y/v<.4?(s.moveTo(c.x,c.y),s.lineTo($.x,$.y)):(B.moveTo(c.x,c.y),B.lineTo($.x,$.y)))}}if(s.stroke(),s.strokeStyle=`rgba(${ct},${dt},${pt},0.18)`,s.lineWidth=.8,s.stroke(B),!It||ke!==ct||Ce!==dt||Te!==pt){It=document.createElement("canvas");const I=48;It.width=I,It.height=I;const c=It.getContext("2d"),k=c.createRadialGradient(I/2,I/2,0,I/2,I/2,I/2);k.addColorStop(0,`rgba(${ct},${dt},${pt},0.9)`),k.addColorStop(.3,`rgba(${ct},${dt},${pt},0.35)`),k.addColorStop(1,`rgba(${ct},${dt},${pt},0)`),c.fillStyle=k,c.fillRect(0,0,I,I),ke=ct,Ce=dt,Te=pt}const R=It;for(let I=0;I<g;I++){const c=m[I],k=.6+.4*Math.sin(c.pulsePhase),$=c.radius*5*(.8+k*.4);s.globalAlpha=.5+k*.4,s.drawImage(R,c.x-$/2,c.y-$/2,$,$)}s.globalAlpha=1,s.fillStyle="rgba(255,255,255,0.45)",s.beginPath();for(let I=0;I<g;I++){const c=m[I];if(c.radius>2){const k=.6+.4*Math.sin(c.pulsePhase),$=c.radius*(.8+k*.4)*.35;s.moveTo(c.x+$,c.y),s.arc(c.x,c.y,$,0,Math.PI*2)}}s.fill()}i()}function nn(){ht!==null&&(cancelAnimationFrame(ht),ht=null),ft=null,_t=null,Bt=[]}let Dt=null;const Qt=560,on=140,Se=Qt/2,Ie=on/2,Ae=[];for(let e=0;e<=Qt;e+=8){const t=Math.abs(e-Se)/Se;Ae.push(Math.pow(Math.min(1,t*1.6),.6))}const an=[0,1,2,3].map(e=>({amp:10+e*5,freq:(1.2+e*.35)*Math.PI*2/Qt,off:e*.6,spd:.7+e*.12}));let oe=!1;function ie(){if(mt=requestAnimationFrame(ie),oe=!oe,oe)return;if(ee+=.07,!Dt){const t=document.getElementById("nf-engine-waves");if(!t){mt=null;return}Dt=Array.from(t.querySelectorAll("path"))}const e=[];for(let t=0;t<Dt.length;t++){const o=an[t],l=ee*o.spd+o.off;e.length=0,e.push(`M 0 ${Ie}`);let r=0;for(let p=0;p<=Qt;p+=8){const i=Ie+o.amp*Ae[r++]*Math.sin(p*o.freq+l);e.push(`L${p} ${i*10+.5|0}`)}Dt[t].setAttribute("d",e.join(" "))}}function rn(){ee=0,jt=0,ne=performance.now(),ie(),_e(),Kt=setInterval(()=>{const e=performance.now(),t=(e-ne)/1e3;if(t>0){we=Math.round(jt/t);const o=document.getElementById("nf-stat-fps");o&&(o.textContent=`${we}`)}jt=0,ne=e},1e3)}function Pe(){mt!==null&&(cancelAnimationFrame(mt),mt=null),Kt&&(clearInterval(Kt),Kt=null),Dt=null,nn()}function Zt(){let e=0;const t=it.filter(d=>d.status!=="skipped").length;for(const d of it){const m=document.getElementById(`nf-proc-${d.stepId}`);if(!m)continue;m.className="nf-proc-row";const g=m.querySelector(".nf-proc-badge");switch(d.status){case"done":m.classList.add("nf-proc-done"),g&&(g.textContent="✅ done"),e++;break;case"active":m.classList.add("nf-proc-active"),g&&(g.textContent=d.progress!==void 0&&d.progress>0?`⏳ ${d.progress}%`:"⏳ active");break;case"error":m.classList.add("nf-proc-error"),g&&(g.textContent="❌ error");break;case"skipped":m.classList.add("nf-proc-skipped"),g&&(g.textContent="— skip");break;default:m.classList.add("nf-proc-waiting"),g&&(g.textContent="(queued)")}}const o=it.findIndex(d=>d.status==="active"),l=o>=0?o+1:e>=t&&t>0?it.length:e,r=document.getElementById("nf-step-counter");r&&(r.textContent=`${l}/${it.length}`);const p=document.querySelector(".nf-core-title-val"),i=document.querySelector(".nf-status-dot");e>=t&&t>0?(p&&(p.textContent="COMPLETE",p.style.color=lt.doneHex),i&&(i.style.background=lt.doneHex,i.style.boxShadow=`0 0 8px rgba(${lt.doneRgb},0.7)`)):it.some(m=>m.status==="error")?(p&&(p.textContent="ERROR",p.style.color="#f87171"),i&&(i.style.background="#ef4444",i.style.boxShadow="0 0 8px rgba(239,68,68,0.7)")):it.some(m=>m.status==="active")&&p&&(p.textContent="ACTIVE",p.style.color=lt.hex,p.style.textShadow=`0 0 10px rgba(${lt.rgb},0.5)`);const s=document.getElementById("nf-terminal"),a=s==null?void 0:s.querySelector(".nf-proc-active");a&&s&&a.scrollIntoView({behavior:"smooth",block:"center"})}function Me(){st&&st.isConnected||(Xt(),st=document.createElement("button"),st.id="nf-toggle-btn",st.className="nf-toggle-visible",st.innerHTML=$t?ge:me,st.title="ซ่อน/แสดง Netflow Overlay",st.style.cssText="position:fixed !important;top:20px !important;right:20px !important;z-index:2147483647 !important;width:48px !important;height:48px !important;border-radius:50% !important;border:2px solid rgba(255,255,255,0.5) !important;background:rgba(0,0,0,0.85) !important;color:#fff !important;font-size:23px !important;cursor:pointer !important;display:flex !important;align-items:center !important;justify-content:center !important;pointer-events:auto !important;",st.onclick=()=>Re(),document.body.appendChild(st))}function Re(){H&&(Me(),$t?(H.classList.remove("nf-hidden"),H.classList.add("nf-visible"),H.style.opacity="1",H.style.pointerEvents="auto",st&&(st.innerHTML=me),$t=!1,_t&&ft&&ht===null&&_e(),mt===null&&ie()):(H.classList.remove("nf-visible"),H.classList.add("nf-hidden"),H.style.opacity="0",H.style.pointerEvents="none",st&&(st.innerHTML=ge),$t=!0,ht!==null&&(cancelAnimationFrame(ht),ht=null),mt!==null&&(cancelAnimationFrame(mt),mt=null)))}const Be={red:"themes/theme-red.jpg",yellow:"themes/theme-yellow.jpg",blue:"themes/theme-blue.jpg",purple:"themes/theme-purple.jpg",green:"themes/theme-green.jpg"};function De(){const e=document.getElementById("nf-core-monitor");if(!e)return;let t=Rt;if(!t)try{t=localStorage.getItem("netflow_app_theme")||"blue"}catch{t="blue"}const o=Be[t]||Be.blue;let l;try{l=chrome.runtime.getURL(o)}catch{l=`/${o}`}const r=lt.rgb;e.style.backgroundImage=["linear-gradient(180deg, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.78) 45%, rgba(0,0,0,0.90) 100%)",`linear-gradient(180deg, rgba(${r},0.25) 0%, rgba(${r},0.12) 50%, rgba(${r},0.20) 100%)`,`url('${l}')`].join(", "),e.style.backgroundSize="auto, auto, 50%",e.style.backgroundPosition="center, center, center",e.style.backgroundRepeat="no-repeat, no-repeat, no-repeat",e.style.setProperty("--nf-bg-set","1"),e.style.border=`1.5px solid rgba(${r},0.45)`,e.style.boxShadow=`0 0 70px rgba(${r},0.22), 0 0 140px rgba(${r},0.1), inset 0 1px 0 rgba(${r},0.15)`}function Jt(e=1){if(lt=Ye(),ue(),H&&H.isConnected){H.style.cssText="position:fixed !important;inset:0 !important;z-index:2147483647 !important;pointer-events:auto !important;opacity:1 !important;background:#0a0a0f !important;overflow:hidden !important;font-family:Inter,system-ui,-apple-system,sans-serif !important;",(!tt||!tt.isConnected)&&(tt=null,Xt()),setTimeout(()=>{if(H)try{tt!=null&&tt.sheet&&tt.sheet.cssRules.length>0&&(H.style.removeProperty("background"),H.style.removeProperty("font-family"),H.style.removeProperty("overflow"))}catch{}},200);for(const t of ut)for(const o of t.steps)o.status="waiting",o.progress=o.progress!==void 0?0:void 0;St=e,it=Yt(e),xe();for(const t of ut)ae(t);if(te(),Zt(),!H.querySelector(".nf-stop-btn")){const t=document.createElement("button");t.className="nf-stop-btn",t.innerHTML='<span class="nf-stop-icon"></span> หยุด',t.style.cssText="position:absolute !important;top:14px !important;right:110px !important;z-index:2147483646 !important;cursor:pointer !important;pointer-events:auto !important;background:rgba(255,60,60,0.08) !important;border:1px solid rgba(255,60,60,0.25) !important;border-radius:8px !important;color:rgba(255,100,100,0.8) !important;font-size:13px !important;padding:6px 14px !important;font-family:inherit !important;display:flex !important;align-items:center !important;gap:6px !important;",t.onclick=()=>{var o;window.__NETFLOW_STOP__=!0;try{Lt("⛔ ผู้ใช้หยุดการทำงาน")}catch{}try{typeof chrome<"u"&&((o=chrome.runtime)!=null&&o.sendMessage)&&chrome.runtime.sendMessage({action:"AUTOMATION_STOPPED"})}catch{}},H.appendChild(t)}$t&&Re();return}H&&!H.isConnected&&(H=null),tt&&(tt.remove(),tt=null),Xt();for(const t of ut)for(const o of t.steps)o.status="waiting",o.progress=o.progress!==void 0?0:void 0;if(St=e,it=Yt(e),e>1){const t=ut.find(l=>l.id==="video");if(t){const l=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:"waiting",progress:0}];for(let r=2;r<=e;r++)l.push({id:`scene${r}-prompt`,label:`Scene ${r} Prompt`,status:"waiting"}),l.push({id:`scene${r}-gen`,label:`Scene ${r} Generate`,status:"waiting"}),l.push({id:`scene${r}-wait`,label:`Scene ${r} รอผล`,status:"waiting",progress:0});t.steps=l}const o=ut.find(l=>l.id==="render");if(o){const l=o.steps.find(p=>p.id==="download");l&&(l.label="ดาวน์โหลด 720p");const r=o.steps.find(p=>p.id==="upscale");r&&(r.label="Full Video")}}H=Qe(),H.style.cssText="position:fixed !important;inset:0 !important;z-index:2147483647 !important;pointer-events:auto !important;opacity:1 !important;background:#0a0a0f !important;overflow:hidden !important;font-family:Inter,system-ui,-apple-system,sans-serif !important;",document.body.appendChild(H),H.classList.add("nf-visible"),$t=!1,Me(),Je(),rn(),requestAnimationFrame(()=>De()),setTimeout(()=>{if(H)try{tt!=null&&tt.sheet&&tt.sheet.cssRules.length>0&&(H.style.removeProperty("background"),H.style.removeProperty("font-family"),H.style.removeProperty("overflow"))}catch{}},200)}function Oe(){ve(),Pe(),$t=!1,H&&(H.classList.add("nf-fade-out"),setTimeout(()=>{H==null||H.remove(),H=null},500)),st&&(st.remove(),st=null)}const sn={settings:"SETTINGS","upload-char":"UPLOAD","upload-prod":"UPLOAD","img-prompt":"PROMPT","img-generate":"GENERATE","img-wait":"IMG WAIT",animate:"ANIMATE","vid-prompt":"VID PROMPT","vid-generate":"VID GEN","vid-wait":"VID WAIT",download:"DOWNLOAD",upscale:"UPSCALE",open:"OPENING"};function ln(e,t,o){const l=it.findIndex(g=>g.status==="active"),r=it.filter(g=>g.status==="done").length,p=it.length,i=l>=0?l+1:r>=p?p:r,s=document.getElementById("nf-stat-step");s&&(s.textContent=`${i}/${p}`);let a=1;for(const g of it)if(g.status==="active"||g.status==="done")if(g.stepId.startsWith("scene")){const v=g.stepId.match(/^scene(\d+)-/);v&&(a=Math.max(a,parseInt(v[1],10)))}else(g.stepId==="download"||g.stepId==="upscale"||g.stepId==="open")&&(a=St);const d=document.getElementById("nf-stat-scenes");if(d&&(d.textContent=St>1?`${a}/${St}`:"1/1"),t==="active"){const g=document.getElementById("nf-stat-status"),v=sn[e]||e.toUpperCase();g&&(g.textContent=v)}else if(t==="done"&&r>=p){const g=document.getElementById("nf-stat-status");g&&(g.textContent="COMPLETE")}else if(t==="error"){const g=document.getElementById("nf-stat-status");g&&(g.textContent="ERROR")}const m=document.getElementById("nf-stat-progress");m&&(o!==void 0&&o>0?m.textContent=`${Math.min(100,o)}%`:t==="active"&&(m.textContent="—"))}function P(e,t,o){if(!H)return;for(const r of ut)for(const p of r.steps)p.id===e&&(p.status=t,o!==void 0&&(p.progress=o));for(const r of it)r.stepId===e&&(r.status=t,o!==void 0&&(r.progress=o));const l=document.getElementById(`nf-step-${e}`);if(l&&(l.className="nf-step",t==="active"?l.classList.add("nf-step-active"):t==="done"?l.classList.add("nf-step-done"):t==="error"&&l.classList.add("nf-step-error")),ln(e,t,o),o!==void 0){const r=document.getElementById(`nf-bar-${e}`);r&&(r.style.width=`${Math.min(100,o)}%`)}te(),Zt()}function At(e){P(e,"skipped");const t=document.getElementById(`nf-step-${e}`);t&&(t.style.opacity="0.2")}function Ot(e=4e3){ve(),Pe(),te(),Zt(),setTimeout(()=>Oe(),e)}function te(){for(const e of ut){const t=e.steps.filter(a=>a.status!=="skipped").length,o=e.steps.filter(a=>a.status==="done").length,l=e.steps.some(a=>a.status==="active"),r=t>0?Math.round(o/t*100):0,p=document.getElementById(`nf-pct-${e.id}`);p&&(p.textContent=`${r}%`);const i=document.getElementById(`nf-modbar-${e.id}`);i&&(i.style.width=`${r}%`);const s=document.getElementById(`nf-mod-${e.id}`);s&&(s.classList.remove("nf-active","nf-done"),r>=100?s.classList.add("nf-done"):l&&s.classList.add("nf-active"))}}function cn(e){var l,r,p,i;St=e;const t=new Map;for(const s of it)t.set(s.stepId,{status:s.status,progress:s.progress});it=Yt(e);for(const s of it){const a=t.get(s.stepId);a&&(s.status=a.status,a.progress!==void 0&&(s.progress=a.progress))}if(xe(),e>1){const s=ut.find(a=>a.id==="video");if(s){const a=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:((l=s.steps.find(d=>d.id==="animate"))==null?void 0:l.status)||"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:((r=s.steps.find(d=>d.id==="vid-prompt"))==null?void 0:r.status)||"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:((p=s.steps.find(d=>d.id==="vid-generate"))==null?void 0:p.status)||"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:((i=s.steps.find(d=>d.id==="vid-wait"))==null?void 0:i.status)||"waiting",progress:0}];for(let d=2;d<=e;d++)a.push({id:`scene${d}-prompt`,label:`Scene ${d} Prompt`,status:"waiting"}),a.push({id:`scene${d}-gen`,label:`Scene ${d} Generate`,status:"waiting"}),a.push({id:`scene${d}-wait`,label:`Scene ${d} รอผล`,status:"waiting",progress:0});s.steps=a,ae(s)}}const o=ut.find(s=>s.id==="render");if(o&&e>1){const s=o.steps.find(d=>d.id==="download");s&&(s.label="ดาวน์โหลด 720p");const a=o.steps.find(d=>d.id==="upscale");a&&(a.label="Full Video"),ae(o)}te(),Zt()}function ae(e){const t=document.getElementById(`nf-mod-${e.id}`);if(!t)return;t.querySelectorAll(".nf-step, .nf-mod-progress").forEach(r=>r.remove()),e.steps.forEach(r=>{const p=document.createElement("div");p.className="nf-step",p.id=`nf-step-${r.id}`;let i="";r.progress!==void 0&&(i=`
                <div class="nf-progress-bar">
                    <div class="nf-progress-fill" id="nf-bar-${r.id}" style="width: 0%"></div>
                </div>
            `),p.innerHTML=`
            ...
            <span class="nf-step-label">${r.label}</span>
            ${i}
        `,t.appendChild(p)});const l=document.createElement("div");l.className="nf-mod-progress",l.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${e.id}"></div>`,t.appendChild(l)}function Lt(e){e.replace(/^\[Netflow AI\]\s*/,"")}let Pt=null,Et=null;const dn=new Promise(e=>{Et=e,setTimeout(()=>e(null),2e3)});try{chrome.runtime.sendMessage({type:"GET_TAB_ID"},e=>{!chrome.runtime.lastError&&(e!=null&&e.tabId)&&(Pt=e.tabId,console.log(`[Netflow AI] Tab ID: ${Pt}`)),Et&&(Et(Pt),Et=null)})}catch{Et&&(Et(null),Et=null)}function wt(){return Pt?`netflow_pending_action_${Pt}`:"netflow_pending_action"}function Le(){try{chrome.runtime.sendMessage({type:"AUTOMATION_FINISHED"})}catch{}}const n=e=>{console.log(`[Netflow AI] ${e}`);try{Lt(e)}catch{}try{chrome.runtime.sendMessage({action:"FLOW_LOG",level:"info",msg:e})}catch{}},M=e=>{console.warn(`[Netflow AI] ${e}`);try{Lt(`⚠️ ${e}`)}catch{}try{chrome.runtime.sendMessage({action:"FLOW_LOG",level:"warn",msg:`⚠️ ${e}`})}catch{}};(()=>{const e=(o,l)=>{const r=o.tagName.toLowerCase(),p=o.id?`#${o.id}`:"",i=o.className&&typeof o.className=="string"?"."+o.className.trim().split(/\s+/).join("."):"",s=o.getBoundingClientRect(),a={};for(const c of o.attributes)["class","id","style"].includes(c.name)||(a[c.name]=c.value.length>80?c.value.slice(0,80)+"…":c.value);const d=(o.textContent||"").trim().slice(0,120),m=Array.from(o.querySelectorAll('i, [class*="icon"]')).map(c=>{var k;return(k=c.textContent)==null?void 0:k.trim()}).filter(Boolean).join(", "),g=[];let v=o.parentElement;for(let c=0;c<5&&v;c++){const k=v.tagName.toLowerCase(),$=v.id?`#${v.id}`:"",x=v.className&&typeof v.className=="string"?"."+v.className.trim().split(/\s+/).slice(0,2).join("."):"";g.push(`${k}${$}${x}`),v=v.parentElement}const B=l==="click"?`%c🖱️ CLICK %c<${r}${p}${i}>`:`%c👆 HOVER %c<${r}${p}${i}>`;console.groupCollapsed(B,l==="click"?"background:#e74c3c;color:#fff;padding:2px 6px;border-radius:3px;font-weight:bold":"background:#3498db;color:#fff;padding:2px 6px;border-radius:3px;font-weight:bold","color:#f39c12;font-weight:bold"),console.log("Element:",o),console.log("Selector:",`${r}${p}${i}`),console.log("Rect:",{x:Math.round(s.x),y:Math.round(s.y),w:Math.round(s.width),h:Math.round(s.height)}),Object.keys(a).length&&console.log("Attributes:",a),d&&console.log("Text:",d),m&&console.log("Icons:",m),g.length&&console.log("Ancestors:",g.join(" > ")),console.groupEnd()};document.addEventListener("click",o=>{const l=o.target;l!=null&&l.closest("#netflow-engine-overlay")||e(l,"click")},!0);let t=null;document.addEventListener("mouseover",o=>{const l=o.target;l!==t&&(l!=null&&l.closest("#netflow-engine-overlay")||(t=l,e(l,"hover")))},!0),console.log("%c[Netflow AI] 🔍 Debug Element Inspector ACTIVE — click/hover elements to inspect","background:#2ecc71;color:#000;padding:4px 8px;border-radius:4px;font-weight:bold")})();function re(e=3e3){n(`🔒 จะปิดแท็บ automation ใน ${e/1e3} วินาที...`),setTimeout(()=>{try{chrome.runtime.sendMessage({action:"CLOSE_AUTOMATION_TAB"},t=>{chrome.runtime.lastError?M(`ปิดแท็บไม่ได้: ${chrome.runtime.lastError.message}`):n(`✅ ปิดแท็บแล้ว: ${t==null?void 0:t.message}`)})}catch(t){M(`ปิดแท็บผิดพลาด: ${t.message}`)}},e)}async function se(){try{if(await new Promise(r=>{try{chrome.runtime.sendMessage({type:"PEEK_CACHED_VIDEO"},p=>{if(chrome.runtime.lastError){r(!1);return}r(!!(p!=null&&p.cached))})}catch{r(!1)}})){n("[TikTok] ✅ Background มี video cached อยู่แล้ว (จาก download) — ข้าม page capture เพื่อไม่ overwrite");const r=document.querySelectorAll("video");for(const p of r){const i=p.src||p.currentSrc||"";if(i)return i}return null}n("[TikTok] ไม่มี cached video — จะ capture จากหน้า...");const t=document.querySelectorAll("video");let o=null,l=0;for(const r of t){let p=r.src||"";if(!p){const a=r.querySelector("source");a&&(p=a.getAttribute("src")||"")}if(!p&&r.currentSrc&&(p=r.currentSrc),!p)continue;if(et()){o||(o=p,l=1);continue}const i=r.getBoundingClientRect(),s=i.width*i.height;i.width>50&&s>l&&(l=s,o=p)}if(!o)return n("[TikTok] ไม่พบ video URL บนหน้า"),null;n(`[TikTok] พบ video URL: ${o.substring(0,80)}... (area=${l.toFixed(0)})`);try{n("[TikTok] กำลัง fetch video blob จาก content script (มี auth)...");const r=await fetch(o);if(!r.ok)return n(`[TikTok] fetch failed: HTTP ${r.status}`),await Ne(o),o;const p=await r.blob(),i=(p.size/1024/1024).toFixed(1);n(`[TikTok] Video blob fetched: ${i} MB, type: ${p.type}`),p.size<1e5&&n(`[TikTok] ⚠️ Blob เล็กเกินไป (${p.size} bytes) — อาจเป็น thumbnail`);const s=await new Promise((a,d)=>{const m=new FileReader;m.onloadend=()=>a(m.result),m.onerror=()=>d(new Error("FileReader error")),m.readAsDataURL(p)});n(`[TikTok] Data URL พร้อม: ${(s.length/1024/1024).toFixed(1)} MB`),await new Promise(a=>{chrome.runtime.sendMessage({type:"CACHE_VIDEO_DATA",data:s},d=>{chrome.runtime.lastError?n(`[TikTok] CACHE_VIDEO_DATA error: ${chrome.runtime.lastError.message}`):d!=null&&d.success?n("[TikTok] ✅ Video cached in background service worker"):n(`[TikTok] CACHE_VIDEO_DATA failed: ${d==null?void 0:d.error}`),a()})})}catch(r){n(`[TikTok] Content script fetch error: ${r.message}`),await Ne(o)}return o}catch(e){return n(`[TikTok] captureVideoUrl error: ${e.message}`),null}}async function Ne(e){if(e.startsWith("https://"))try{await new Promise(t=>{chrome.runtime.sendMessage({type:"PRE_FETCH_VIDEO",url:e},o=>{chrome.runtime.lastError?n(`[TikTok] PRE_FETCH_VIDEO error: ${chrome.runtime.lastError.message}`):o!=null&&o.success?n(`[TikTok] Video pre-fetched via background: ${((o.size||0)/1024/1024).toFixed(1)} MB`):n(`[TikTok] PRE_FETCH_VIDEO failed: ${o==null?void 0:o.error}`),t()})})}catch{}}function le(e){if(e){if(window.__VIDEO_COMPLETE_SENT__){n("[TikTok] ⚠️ VIDEO_GENERATION_COMPLETE already sent, skipping duplicate");return}window.__VIDEO_COMPLETE_SENT__=!0;try{chrome.runtime.sendMessage({type:"VIDEO_GENERATION_COMPLETE",videoUrl:e,source:"veo"}),n("[TikTok] Sent VIDEO_GENERATION_COMPLETE (source=veo)")}catch{}}}const Y=/Mac|iPhone|iPad|iPod/i.test(navigator.userAgent),ce=/Win/i.test(navigator.userAgent),Fe=Y?"🍎 Mac":ce?"🪟 Win":"🐧 Other";n(`สคริปต์โหลดบนหน้า Google Flow แล้ว ${Fe}`),window.__VIDEO_COMPLETE_SENT__=!1;class de extends Error{constructor(){super("AUTOMATION_STOPPED"),this.name="NetflowAbortError"}}let Nt=null,kt=null,ze=!1;const Mt=new Map;let Ve=0;function pn(){if(Nt)return Nt;try{const e=new Blob(["self.onmessage=function(e){var d=e.data;setTimeout(function(){self.postMessage(d.id)},d.ms)};"],{type:"application/javascript"});return Nt=new Worker(URL.createObjectURL(e)),Nt.onmessage=t=>{const o=Mt.get(t.data);o&&(Mt.delete(t.data),o())},console.log("[Netflow AI] ⚡ Web Worker timer created — background tab throttling defeated"),Nt}catch{return console.warn("[Netflow AI] Web Worker timer unavailable (CSP?) — trying port relay"),null}}function fn(){if(kt)return kt;if(ze)return null;try{return kt=chrome.runtime.connect({name:"timer"}),kt.onMessage.addListener(e=>{const t=Mt.get(e.id);t&&(Mt.delete(e.id),t())}),kt.onDisconnect.addListener(()=>{kt=null}),console.log("[Netflow AI] ⚡ Port relay timer connected — background tab throttling defeated"),kt}catch{return ze=!0,console.warn("[Netflow AI] Port relay unavailable — falling back to setTimeout"),null}}const f=e=>new Promise((t,o)=>{if(window.__NETFLOW_STOP__)return o(new de);let l=!1;const r=()=>{if(!l){if(l=!0,window.__NETFLOW_STOP__)return o(new de);t()}};setTimeout(r,e);const p=pn();if(p){const a=++Ve;Mt.set(a,r),p.postMessage({id:a,ms:e});try{chrome.runtime.sendMessage({type:"TIMER_DELAY",ms:e+2e3},()=>{chrome.runtime.lastError||r()})}catch{}return}try{chrome.runtime.sendMessage({type:"TIMER_DELAY",ms:e},()=>{chrome.runtime.lastError?setTimeout(r,e):r()});return}catch{}const i=fn();if(i){const a=++Ve;Mt.set(a,r),i.postMessage({cmd:"delay",id:a,ms:e});return}const s=setTimeout(r,e);f._lastId=s});function Ct(){return!!window.__NETFLOW_STOP__}const et=()=>document.hidden;let Ue=0;async function Tt(){if(!document.hidden)return!1;const e=Date.now();if(e-Ue<15e3)return!1;Ue=e;try{return n("🔄 Tab ซ่อนอยู่ — ขอ background สลับ tab ชั่วคราวเพื่ออัพเดท DOM"),chrome.runtime.sendMessage({type:"BRIEF_ACTIVATE_TAB"}),await f(1500),!0}catch{return!1}}async function bt(){if(!document.hidden)return!0;n("🔄 Tab ซ่อนอยู่ — ดึงหน้าต่าง Chrome ขึ้นมาข้างหน้า...");try{await new Promise(t=>chrome.runtime.sendMessage({type:"FOCUS_TAB"},()=>t()));const e=Date.now();for(;document.hidden&&Date.now()-e<5e3;)await f(200);return document.hidden?(n("⚠️ Tab ยังซ่อนอยู่หลัง 5 วินาที"),!1):(n("✅ Tab กลับมาแสดงผลแล้ว — รอ DOM render 3 วิ"),await f(3e3),!0)}catch{return n("⚠️ ensureTabVisible ล้มเหลว"),!1}}function qe(){var o;const e=["audio generation failed","couldn't generate","could not generate","failed to generate","generation failed","ไม่สามารถสร้าง","สร้างไม่สำเร็จ","try again later","ลองอีกครั้งภายหลัง","ลองใหม่อีกครั้ง","something went wrong","เกิดข้อผิดพลาด","safety filter","policy violation","content policy","might violate","violate our policies","อาจละเมิด","unable to generate","ไม่สามารถสร้างวิดีโอ","couldn't generate video","couldn't generate image"],t=document.querySelectorAll("div, span, p, h1, h2, h3, li");for(const l of t){if(l.closest("#netflow-engine-overlay"))continue;const r=(l.textContent||"").trim().toLowerCase();if(!(r.length>200||r.length<5)){for(const p of e)if(r.includes(p))return((o=l.textContent)==null?void 0:o.trim())||p}}return null}function un(e){let t=e;const o=[/STRICT FACE & HEAD LOCK:[^.]*\./gi,/BODY LOCK:[^.]*\./gi,/HAIR LOCK:[^.]*\./gi,/FACE LOCK[^.]*\./gi,/PRODUCT IDENTITY LOCK:[^.]*\./gi,/LABEL LOCK:[^.]*\./gi,/PRODUCT EVERY FRAME:[^.]*\./gi,/TRANSITION STABILITY:[^.]*\./gi,/ANTI[_-]DUPLICATION:[^.]*\./gi,/ANTI[_-]TEXT[^.]*\./gi,/ANTI[_-]MORPH[^.]*\./gi,/ANTI[_-]DISTORTION[^.]*\./gi,/ANTI[_-]ADDITION[^.]*\./gi,/ANTI[_-]FLOATING[^.]*\./gi,/PROPS vs PRODUCT:[^.]*\./gi,/BRAND IDENTITY FREEZE[^.]*\./gi,/BRAND MORPHING[^.]*\./gi,/PRODUCT SIZE \(CRITICAL\):[^.]*\./gi,/PRODUCT SIZE REALISM:[^.]*\./gi,/VOICE DISCIPLINE:[^.]*\./gi,/ZERO INVENTION:[^.]*\./gi,/REALISM:[^.]*\./gi,/SCREEN CONTENT[^.]*\./gi,/SINGLE UTENSIL RULE[^.]*\./gi,/PRODUCT LOCK[^.]*\./gi,/FACE & HEAD LOCK[^.]*\./gi,/CLOTHING FIDELITY[^.]*\./gi,/FRONT[_-]FACING[^.]*\./gi];for(const i of o)t=t.replace(i,"");const l=["DO NOT","NEVER","FORBIDDEN","MUST NOT","ABSOLUTELY NO","IMMUTABLE","LOCKED","HIGHEST PRIORITY","#1 FORBIDDEN","Do NOT let","Do NOT add","Do NOT generate","Do NOT simplify","Do NOT invent","ZERO on-screen","NO split screen","NO collage","NO side-by-side","NO divided frames","never morph","never simplify","never change shape","never disappear","never be hidden","never exit","BRAND MORPHING IS","objects MUST NOT magically"];return t=t.split(/(?<=[.!])\s+/).filter(i=>!l.some(s=>i.includes(s))).join(" "),t=t.replace(/\s{2,}/g," ").trim(),t.length>1200&&(t=t.replace(/Render with extreme surface detail[^.]*\./gi,""),t=t.replace(/High-fidelity visual detail[^.]*\./gi,""),t=t.replace(/Product lit with soft rim light[^.]*\./gi,""),t=t.replace(/visible material texture[^.]*\./gi,""),t=t.replace(/Fluid motion, cinematic motion blur[^.]*\./gi,""),t=t.replace(/AI-observed appearance:[^.]*\./gi,""),t=t.replace(/Reference clothing:[^.]*\./gi,""),t=t.replace(/\s{2,}/g," ").trim()),n(`🛡️ Safe retry prompt: ${e.length} → ${t.length} chars (${Math.round((1-t.length/e.length)*100)}% reduction)`),t}async function Q(e){if(et()){e.dispatchEvent(new MouseEvent("click",{bubbles:!0,cancelable:!0})),e.click();return}const t=e.getBoundingClientRect(),o=t.left+t.width/2,l=t.top+t.height/2,r={bubbles:!0,cancelable:!0,clientX:o,clientY:l,button:0};e.dispatchEvent(new PointerEvent("pointerdown",{...r,pointerId:1,isPrimary:!0,pointerType:"mouse"})),e.dispatchEvent(new MouseEvent("mousedown",r)),await f(80),e.dispatchEvent(new PointerEvent("pointerup",{...r,pointerId:1,isPrimary:!0,pointerType:"mouse"})),e.dispatchEvent(new MouseEvent("mouseup",r)),e.dispatchEvent(new MouseEvent("click",r)),await f(50),e.click()}function Ft(e){const t=e.getBoundingClientRect(),o=t.left+t.width/2,l=t.top+t.height/2,r={bubbles:!0,cancelable:!0,clientX:o,clientY:l};e.dispatchEvent(new PointerEvent("pointerenter",{...r,pointerId:1,isPrimary:!0,pointerType:"mouse"})),e.dispatchEvent(new MouseEvent("mouseenter",r)),e.dispatchEvent(new PointerEvent("pointerover",{...r,pointerId:1,isPrimary:!0,pointerType:"mouse"})),e.dispatchEvent(new MouseEvent("mouseover",r)),e.dispatchEvent(new PointerEvent("pointermove",{...r,pointerId:1,isPrimary:!0,pointerType:"mouse"})),e.dispatchEvent(new MouseEvent("mousemove",r))}function gn(e){const t=[],o=document.querySelectorAll("i, span[class*='icon'], span[class*='material'], span[class*='google-symbols'], [data-icon]");for(const l of o){if((l.textContent||"").trim()!==e)continue;let p=l,i=null,s=1/0;for(let a=0;a<20&&p&&(p=p.parentElement,!(!p||p===document.body));a++){if(et()){a>=3&&p.children.length>0&&!i&&(i=p);continue}const d=p.getBoundingClientRect();if(d.width>100&&d.height>80&&d.width<window.innerWidth*.6&&d.top>=-10&&d.bottom<=window.innerHeight+10){const m=d.width*d.height;m<s&&(i=p,s=m)}}i&&!t.includes(i)&&t.push(i)}return t.sort((l,r)=>{const p=l.getBoundingClientRect(),i=r.getBoundingClientRect();return p.left-i.left}),t}function pe(e=!1){const t=[],o=document.querySelectorAll("video");for(const i of o){let s=i.parentElement;for(let a=0;a<10&&s;a++){if(et()){if(a>=3&&s.children.length>0){t.push({el:s,left:0});break}s=s.parentElement;continue}const d=s.getBoundingClientRect();if(d.width>120&&d.height>80&&d.width<window.innerWidth*.7&&d.top>=-50&&d.left<window.innerWidth*.75){t.push({el:s,left:d.left});break}s=s.parentElement}}const l=document.querySelectorAll("i, span.material-symbols-outlined, span.google-symbols, .google-symbols");for(const i of l){const s=(i.textContent||"").trim();if(s==="play_arrow"||s==="play_circle"||s==="videocam"){let a=i.parentElement;for(let d=0;d<10&&a;d++){if(et()){if(d>=3&&a.children.length>0){t.push({el:a,left:0});break}a=a.parentElement;continue}const m=a.getBoundingClientRect();if(m.width>120&&m.height>80&&m.width<window.innerWidth*.7&&m.top>=-50&&m.left<window.innerWidth*.75){t.push({el:a,left:m.left});break}a=a.parentElement}}}const r=document.querySelectorAll("img");for(const i of r){const s=(i.alt||"").toLowerCase();if(s.includes("video")||s.includes("วิดีโอ")){let a=i.parentElement;for(let d=0;d<10&&a;d++){if(et()){if(d>=3&&a.children.length>0){t.push({el:a,left:0});break}a=a.parentElement;continue}const m=a.getBoundingClientRect();if(m.width>120&&m.height>80&&m.width<window.innerWidth*.7&&m.top>=-50&&m.left<window.innerWidth*.75){t.push({el:a,left:m.left});break}a=a.parentElement}}}const p=Array.from(new Set(t.map(i=>i.el))).map(i=>t.find(s=>s.el===i));if(p.sort((i,s)=>i.left-s.left),p.length>0){const i=p[0].el,s=i.getBoundingClientRect();return e||n(`🎬 พบการ์ดวิดีโอที่ (${s.left.toFixed(0)},${s.top.toFixed(0)}) ขนาด ${s.width.toFixed(0)}x${s.height.toFixed(0)}`),i}return e||n("🎬 ไม่พบการ์ดวิดีโอ"),null}function mn(){const e=gn("image");if(e.length>0){const o=e[0].getBoundingClientRect();return n(`🖼️ พบการ์ดรูปภาพ ${e.length} ใบ — ใบแรกที่ (${o.left.toFixed(0)},${o.top.toFixed(0)}) ขนาด ${o.width.toFixed(0)}x${o.height.toFixed(0)}`),e[0]}const t=document.querySelectorAll("canvas");for(const o of t){let l=o.parentElement;for(let r=0;r<10&&l;r++){if(et()){if(r>=3&&l.children.length>0)return n("🖼️ พบการ์ดรูปภาพจาก <canvas> (minimized mode)"),l;l=l.parentElement;continue}const p=l.getBoundingClientRect();if(p.width>100&&p.height>80&&p.width<window.innerWidth*.6)return n(`🖼️ พบการ์ดรูปภาพจาก <canvas> สำรองที่ (${p.left.toFixed(0)},${p.top.toFixed(0)})`),l;l=l.parentElement}}return n("🖼️ ไม่พบการ์ดรูปภาพ"),null}function hn(e,t){var s;const[o,l]=e.split(","),r=((s=o.match(/:(.*?);/))==null?void 0:s[1])||"image/png",p=atob(l),i=new Uint8Array(p.length);for(let a=0;a<p.length;a++)i[a]=p.charCodeAt(a);return new File([i],t,{type:r})}async function wn(e,t=1024,o=.8){try{if(e.length<5e5)return n(`🗜️ รูปเล็กพอ (${(e.length/1024).toFixed(0)} KB base64) — ไม่บีบอัด`),e;n(`🗜️ รูปใหญ่ (${(e.length/1024).toFixed(0)} KB base64) — กำลังบีบอัด...`);const l=new Image;await new Promise((m,g)=>{l.onload=()=>m(),l.onerror=()=>g(new Error("Image load failed")),l.src=e});let{width:p,height:i}=l;if(p>t||i>t){const m=t/Math.max(p,i);p=Math.round(p*m),i=Math.round(i*m)}const s=document.createElement("canvas");s.width=p,s.height=i;const a=s.getContext("2d");if(!a)return e;a.drawImage(l,0,0,p,i);const d=s.toDataURL("image/jpeg",o);return n(`🗜️ บีบอัดแล้ว: ${(e.length/1024).toFixed(0)} KB → ${(d.length/1024).toFixed(0)} KB (${p}x${i})`),s.width=0,s.height=0,d}catch(l){return M(`🗜️ บีบอัดล้มเหลว: ${l.message} — ใช้รูปต้นฉบับ`),e}}function xt(e){var r;const t=[],o=new WeakSet,l=["i.google-symbols","i[class*='google-symbols']",".material-symbols-outlined",".material-icons",".material-symbols-rounded",".material-symbols-sharp","i[class*='material']","span[class*='material']","i[class*='icon']","span[class*='icon']","[data-icon]","[class*='gm-icon']","[class*='gmat-icon']","i"];for(const p of l){for(const i of document.querySelectorAll(p))if(((r=i.textContent)==null?void 0:r.trim())===e){const s=i.closest("button");s&&!o.has(s)&&(o.add(s),t.push(s))}if(t.length>0)break}if(t.length===0)for(const p of document.querySelectorAll("button")){const i=(p.getAttribute("aria-label")||"").toLowerCase();(i===e.toLowerCase()||i.includes(e.toLowerCase()))&&(o.has(p)||(o.add(p),t.push(p)))}return t}function bn(){const e=["add","add_2","add_circle","add_circle_outline","attach_file","attach_file_add","attachment","note_add"];let t=[];for(const i of e)if(t=xt(i),t.length>0)break;if(t.length>0){let i=null,s=0;for(const a of t){const d=a.getBoundingClientRect();d.bottom>window.innerHeight*.6&&d.y>s&&(s=d.y,i=a)}if(i)return n(`พบปุ่ม "+" ของ Prompt Bar (icon) ที่ y=${s.toFixed(0)}`),i}n("ไม่พบปุ่มเพิ่มจากไอคอน — ลอง fallback ทั้งหมด");const o=["add","attach","upload","create","insert","plus","เพิ่ม","แนบ","อัปโหลด","สร้าง"];for(const i of document.querySelectorAll("button")){const s=(i.getAttribute("aria-label")||"").toLowerCase(),a=(i.getAttribute("title")||"").toLowerCase();if(o.some(d=>s.includes(d)||a.includes(d))){if(et())return n('พบปุ่ม "+" (aria/title) hidden mode'),i;const d=i.getBoundingClientRect();if(d.bottom>window.innerHeight*.6&&d.width<80&&d.height<80)return n(`พบปุ่ม "+" (aria="${s}" title="${a}") ที่ y=${d.y.toFixed(0)}`),i}}const l=document.querySelectorAll("button");for(const i of l){const s=(i.textContent||"").trim();if(s!=="+"&&s!=="add"&&s!=="Add")continue;if(et())return i;const a=i.getBoundingClientRect();if(a.bottom>window.innerHeight*.6&&a.width<80&&a.height<80)return n(`พบปุ่ม "+" (text="${s}") ที่ y=${a.y.toFixed(0)}`),i}const r=document.querySelector('[data-slate-editor="true"], [role="textbox"][contenteditable="true"]');if(r){const i=r.getBoundingClientRect();let s=null,a=1/0;for(const d of l){const m=d.getBoundingClientRect();if(m.width<10||m.height<10||m.width>100||m.height>100||Math.abs(m.top-i.top)>80)continue;const g=Math.abs(m.left-i.left)+Math.abs(m.top-i.top);g<a&&(a=g,s=d)}if(s)return n(`พบปุ่ม "+" (ใกล้ prompt bar, dist=${a.toFixed(0)})`),s}for(const i of l){const s=i.querySelector("svg");if(!s)continue;const a=s.querySelectorAll("path, line, polygon"),d=Array.from(a).map(m=>m.getAttribute("d")||"").join(" ");if(d.includes("M12")||d.includes("M11")||d.includes("M10")){if(et())return i;const m=i.getBoundingClientRect();if(m.bottom>window.innerHeight*.6&&m.width<80&&m.height<80)return n(`พบปุ่ม "+" (SVG) ที่ y=${m.y.toFixed(0)}`),i}}const p=[];for(const i of l){const s=i.getBoundingClientRect();if(s.bottom>window.innerHeight*.6&&s.width>0){const a=(i.textContent||"").trim().substring(0,30),d=i.getAttribute("aria-label")||"",m=(i.className||"").substring(0,40),g=i.querySelector("i, span[class*='icon'], svg")?"has-icon":"no-icon";p.push(`"${a}" aria="${d}" cls="${m}" ${g} y=${s.y.toFixed(0)}`)}}return M(`ไม่พบปุ่ม "+" — ปุ่มที่พบบริเวณล่าง (${p.length}): ${p.slice(0,5).join(" | ")}`),null}function fe(){for(const l of["arrow_forward","send","arrow_upward","navigate_next","arrow_right"]){const r=xt(l);let p=null,i=0;for(const s of r){const a=s.getBoundingClientRect();a.y>i&&(i=a.y,p=s)}if(p)return n(`พบปุ่ม Generate จากไอคอน "${l}" ที่ y=${i.toFixed(0)}`),p}const e=document.querySelectorAll("button");let t=null,o=0;for(const l of e){if(et())break;const r=l.getBoundingClientRect();if(r.bottom>window.innerHeight*.7&&r.right>window.innerWidth*.5){const p=Math.abs(r.width-r.height)<10&&r.width<60,i=r.y+r.x+(p?1e3:0);i>o&&(o=i,t=l)}}if(t)return n("พบปุ่ม Generate จากตำแหน่งขวาล่าง"),t;for(const l of e){const r=(l.getAttribute("aria-label")||"").toLowerCase();if(r.includes("generate")||r.includes("submit")||r.includes("send")||r.includes("สร้าง"))return l}return null}function zt(){const e=document.querySelectorAll("textarea");for(const l of e)if(et()||l.getBoundingClientRect().bottom>window.innerHeight*.5)return l;const t=document.querySelectorAll('[contenteditable="true"]');for(const l of t)if(et()||l.getBoundingClientRect().bottom>window.innerHeight*.5)return l;const o=document.querySelectorAll("input[type='text'], input:not([type])");for(const l of o){const r=l.placeholder||"";if(r.includes("สร้าง")||r.includes("prompt")||r.includes("describe"))return l}return e.length>0?e[e.length-1]:null}async function Vt(e,t){var o,l,r,p;e.focus(),await f(300),n("วางข้อความ: วิธี 1 — Slate beforeinput insertFromPaste");try{const i=new DataTransfer;i.setData("text/plain",t),i.setData("text/html",`<p>${t.replace(/\n/g,"<br>")}</p>`);const s=new InputEvent("beforeinput",{bubbles:!0,cancelable:!0,inputType:"insertFromPaste",dataTransfer:i});e.dispatchEvent(s),n("วางข้อความ: ส่ง beforeinput insertFromPaste แล้ว");const a=new InputEvent("input",{bubbles:!0,inputType:"insertFromPaste",dataTransfer:i});e.dispatchEvent(a),await f(800);const d=(e.textContent||"").replace(/คุณต้องการสร้างอะไร|What do you want to create/gi,"").trim();if(d.length>20){n(`วางข้อความ: ✅ วิธี 1 สำเร็จ (${d.length} ตัวอักษร)`);return}n(`วางข้อความ: วิธี 1 — ไม่พบข้อความ (ได้ ${d.length} ตัวอักษร)`)}catch(i){n(`วางข้อความ: วิธี 1 ล้มเหลว: ${i.message}`)}n("วางข้อความ: วิธี 2 — Slate beforeinput insertText");try{e.focus(),await f(100);const i=new InputEvent("beforeinput",{bubbles:!0,cancelable:!0,inputType:"insertText",data:t});e.dispatchEvent(i);const s=new InputEvent("input",{bubbles:!0,inputType:"insertText",data:t});e.dispatchEvent(s),await f(800);const a=(e.textContent||"").replace(/คุณต้องการสร้างอะไร|What do you want to create/gi,"").trim();if(a.length>20){n(`วางข้อความ: ✅ วิธี 2 สำเร็จ (${a.length} ตัวอักษร)`);return}n("วางข้อความ: วิธี 2 — ไม่พบข้อความ")}catch(i){n(`วางข้อความ: วิธี 2 ล้มเหลว: ${i.message}`)}n("วางข้อความ: วิธี 3 — ClipboardEvent paste");try{e.focus(),await f(200);const i=new DataTransfer;i.setData("text/plain",t),i.setData("text/html",`<p>${t.replace(/\n/g,"<br>")}</p>`);const s=new ClipboardEvent("paste",{bubbles:!0,cancelable:!0,clipboardData:i});e.dispatchEvent(s),await f(800);const a=(e.textContent||"").replace(/คุณต้องการสร้างอะไร|What do you want to create/gi,"").trim();if(a.length>20){n(`วางข้อความ: ✅ วิธี 3 สำเร็จ (${a.length} ตัวอักษร)`);return}n("วางข้อความ: วิธี 3 — ไม่พบข้อความ")}catch(i){n(`วางข้อความ: วิธี 3 ล้มเหลว: ${i.message}`)}n("วางข้อความ: วิธี 4 — navigator.clipboard + execCommand paste");try{if((o=navigator.clipboard)!=null&&o.writeText)await navigator.clipboard.writeText(t),n("วางข้อความ: คัดลอกไปคลิปบอร์ดผ่าน navigator.clipboard");else{const s=document.createElement("textarea");s.value=t,s.style.cssText="position:fixed;left:-9999px;top:-9999px;opacity:0",document.body.appendChild(s),s.focus(),s.select(),document.execCommand("copy"),document.body.removeChild(s),n("วางข้อความ: คัดลอกไปคลิปบอร์ดผ่าน execCommand")}e.focus(),await f(200),document.execCommand("paste"),await f(500);const i=(e.textContent||"").replace(/คุณต้องการสร้างอะไร|What do you want to create/gi,"").trim();if(i.length>20){n(`วางข้อความ: ✅ วิธี 4 สำเร็จ (${i.length} ตัวอักษร)`);return}}catch(i){n(`วางข้อความ: วิธี 4 ล้มเหลว: ${i.message}`)}n("วางข้อความ: วิธี 5 — React fiber Slate editor");try{const i=Object.keys(e).find(s=>s.startsWith("__reactFiber$")||s.startsWith("__reactInternalInstance$"));if(i){let s=e[i];for(let a=0;a<30&&s;a++){const d=s.memoizedProps,m=s.memoizedState;if((l=d==null?void 0:d.editor)!=null&&l.insertText){n("วางข้อความ: พบ Slate editor ผ่าน fiber props");const g=d.editor;g.selection,g.insertText(t),n("วางข้อความ: ✅ วิธี 5 — แทรกผ่าน editor.insertText");return}if((p=(r=m==null?void 0:m.memoizedState)==null?void 0:r.editor)!=null&&p.insertText){n("วางข้อความ: พบ Slate editor ผ่าน fiber state"),m.memoizedState.editor.insertText(t),n("วางข้อความ: ✅ วิธี 5 — แทรกผ่าน state editor");return}s=s.return}n("วางข้อความ: พบ Fiber แต่ไม่พบ Slate editor ใน tree")}else n("วางข้อความ: ไม่พบ React fiber บน element")}catch(i){n(`วางข้อความ: วิธี 5 ล้มเหลว: ${i.message}`)}n("วางข้อความ: ⚠️ ลองครบทั้ง 5 วิธีแล้ว — ตรวจสอบผลใน Console")}function Ut(){const e=new Set,t=document.querySelectorAll('img, canvas, video, [role="img"], [style*="background-image"], [class*="thumb"], [class*="preview"], [class*="upload"], [class*="attachment"]');for(const o of t){if(o.closest("#netflow-engine-overlay")||(o instanceof HTMLImageElement||o instanceof HTMLVideoElement)&&!o.src)continue;if(et()){e.add(o);continue}const l=o.getBoundingClientRect(),r=l.width>20&&l.width<220&&l.height>20&&l.height<220,p=l.bottom>window.innerHeight*.55,i=o.offsetParent!==null||getComputedStyle(o).position==="fixed";r&&p&&i&&e.add(o)}return e.size}async function He(e,t=5e3){var s;const o=Date.now(),l=["upload","upload_file","upload_2","cloud_upload","file_upload","add_photo_alternate","photo_library"],r=["upload image","อัปโหลดรูปภาพ","upload","อัปโหลด","upload file","add image","เพิ่มรูป","เพิ่มรูปภาพ"],p=new Set(["tab","tablist","tabpanel","navigation"]);for(;Date.now()-o<t;){for(const g of document.querySelectorAll("button")){if(g===e)continue;const v=g.querySelector("i");if(!v)continue;const B=(v.textContent||"").trim().toLowerCase();if(B==="upload"||B==="upload_file"||B==="cloud_upload"){const R=g.getBoundingClientRect();if(R.width>0&&R.height>0&&R.y>window.innerHeight*.4)return n(`🎯 พบปุ่ม Upload โดยตรง (icon="${B}" y=${R.y.toFixed(0)})`),g}}const a=[],d=e.getAttribute("aria-controls");if(d){const g=document.getElementById(d);g&&a.push(g)}const m=e.getAttribute("aria-owns");if(m){const g=document.getElementById(m);g&&a.push(g)}for(const g of["[data-radix-portal]","[data-radix-popper-content-wrapper]",'[role="dialog"]','[role="menu"]','[role="listbox"]',"[data-radix-menu-content]","[data-radix-dropdown-menu-content]","[data-radix-popover-content]",'[class*="popover"]','[class*="dropdown"]','[class*="menu-content"]','[class*="dialog"]'])for(const v of document.querySelectorAll(g))a.push(v);for(const g of document.querySelectorAll("[id]"))(g.id||"").match(/^radix-/)&&a.push(g);for(const g of a)for(const v of g.querySelectorAll("button, [role='menuitem'], [role='option'], li, div[tabindex], a")){if(v===e)continue;const B=(v.getAttribute("role")||"").toLowerCase();if(p.has(B))continue;const R=v.querySelector("i, span[class*='icon'], span[class*='material']"),I=((s=R==null?void 0:R.textContent)==null?void 0:s.trim().toLowerCase())||"";if(l.includes(I))return n(`พบปุ่ม Upload (icon="${I}")`),v;const c=(v.textContent||"").trim().toLowerCase(),k=Array.from(v.querySelectorAll("span, div, p")).map(x=>{var C;return((C=x.textContent)==null?void 0:C.trim().toLowerCase())||""});if(r.some(x=>c===x||k.some(C=>C===x))){if(c==="image"||c==="video"||c==="รูปภาพ"||c==="วิดีโอ")continue;return n(`พบปุ่ม Upload (text="${c.substring(0,40)}")`),v}const $=(v.getAttribute("aria-label")||"").toLowerCase();if(r.some(x=>$.includes(x)))return n(`พบปุ่ม Upload (aria="${$}")`),v}if(Date.now()-o>t/2)for(const g of document.querySelectorAll("button, [role='menuitem']")){if(g===e)continue;const v=(g.getAttribute("role")||"").toLowerCase();if(p.has(v))continue;const B=(g.textContent||"").trim().toLowerCase();if(B==="image"||B==="video"||B==="รูปภาพ"||B==="วิดีโอ")continue;const R=g.getBoundingClientRect();if(!(R.width===0||R.height===0||R.y<window.innerHeight*.4)&&r.some(I=>B===I||B.includes(I))&&B.length<50)return n(`พบปุ่ม Upload (global search, text="${B.substring(0,40)}")`),g}await f(500)}const i=[];for(const a of["[data-radix-portal]",'[role="dialog"]','[role="menu"]']){const d=document.querySelectorAll(a);if(d.length>0)for(const m of d){const g=m.querySelectorAll("button, [role='menuitem']");for(const v of g)i.push(`[${a}] "${(v.textContent||"").trim().substring(0,30)}"`)}}return M(`ไม่พบปุ่ม Upload — พบ elements ใน dialogs: ${i.slice(0,8).join(" | ")||"(ว่าง)"}`),null}async function xn(){document.querySelectorAll("[data-netflow-captured]").forEach(t=>t.removeAttribute("data-netflow-captured")),document.documentElement.removeAttribute("data-nf-block-active");const e=document.createElement("script");if(e.textContent=`(function(){

...
@media (max-height: 600px) {
    ...
    `}function Ue(){K||(K=document.createElement("style"),K.id="netflow-overlay-styles",K.textContent=Lt(Z),document.head.appendChild(K))}function rt(t){t.innerHTML="",Y.forEach((e,r)=>{const a=document.createElement("div");a.className="nf-proc-row nf-proc-waiting",a.id=`nf-proc-${e.stepId}`,a.innerHTML=`
    ...
    `,A.appendChild(y);const g=document.createElement("div");g.className="nf-terminal",g.id="nf-terminal",rt(g),A.appendChild(g);const w=document.createElement("div");w.className="nf-engine-core",w.id="nf-engine-core";const M=document.createElement("div");M.className="nf-engine-frame",["nf-fc-tl","nf-fc-tr","nf-fc-bl","nf-fc-br"].forEach(f=>{const b=document.createElement("div");b.className=`nf-frame-corner ${f}`,M.appendChild(b)}),w.appendChild(M);const O="http://www.w3.org/2000/svg",$=document.createElementNS(O,"svg");$.setAttribute("class","nf-engine-waves"),$.setAttribute("viewBox","0 0 560 140"),$.setAttribute("preserveAspectRatio","none"),$.id="nf-engine-waves";for(let f=0;f<4;f++){const b=document.createElementNS(O,"path");b.setAttribute("fill","none"),b.setAttribute("stroke-width",f<2?"1.5":"1"),b.setAttribute("stroke",f<2?`rgba(${Z.rgb},${.14+f*.1})`:`rgba(${Z.accentRgb},${.1+(f-2)*.08})`),b.setAttribute("data-wave-idx",String(f)),$.appendChild(b)}w.appendChild($);const x=document.createElement("div");x.className="nf-engine-brand-inner",x.innerHTML=`

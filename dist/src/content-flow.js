...
@media (max-height: 600px) {
    ...
    `,P.appendChild(y);const f=document.createElement("div");f.className="nf-terminal",f.id="nf-terminal",it(f),P.appendChild(f);const w=document.createElement("div");w.className="nf-engine-core",w.id="nf-engine-core";const M=document.createElement("div");M.className="nf-engine-frame",["nf-fc-tl","nf-fc-tr","nf-fc-bl","nf-fc-br"].forEach(g=>{const v=document.createElement("div");v.className=`nf-frame-corner ${g}`,M.appendChild(v)}),w.appendChild(M);const F="http://www.w3.org/2000/svg",T=document.createElementNS(F,"svg");T.setAttribute("class","nf-engine-waves"),T.setAttribute("viewBox","0 0 560 140"),T.setAttribute("preserveAspectRatio","none"),T.id="nf-engine-waves";for(let g=0;g<4;g++){const v=document.createElementNS(F,"path");v.setAttribute("fill","none"),v.setAttribute("stroke-width",g<2?"1.5":"1"),v.setAttribute("stroke",g<2?`rgba(${Q.rgb},${.14+g*.1})`:`rgba(${Q.accentRgb},${.1+(g-2)*.08})`),v.setAttribute("data-wave-idx",String(g)),T.appendChild(v)}w.appendChild(T);const $=document.createElement("div");$.className="nf-engine-brand-inner",$.innerHTML=`
    ...
    `,w.appendChild($);const D=document.createElement("div");D.className="nf-engine-stats",D.id="nf-engine-stats",D.innerHTML=[["SCENES","nf-stat-scenes","1/1"],["ELAPSED","nf-stat-elapsed","00:00"],["STEP","nf-stat-step","0/0"],["STATUS","nf-stat-status","READY"],["PROGRESS","nf-stat-progress","—"]].map(([g,v,z])=>`<div class="nf-stat-item"><span class="nf-stat-label">${g}</span><span class="nf-stat-val" id="${v}">${z}</span></div>`).join(""),w.appendChild(D),P.appendChild(w),B.appendChild(P);const x=["nf-mod-tl","nf-mod-tr","nf-mod-bl","nf-mod-br"];ae.forEach((g,v)=>{const z=zt(g);z.classList.add(x[v]),z.id=`nf-mod-${g.id}`,B.appendChild(z)}),t.appendChild(B);for(let g=0;g<30;g++){const v=document.createElement("div");v.className="nf-particle",v.style.left=`${5+Math.random()*90}%`,v.style.bottom=`${Math.random()*40}%`,v.style.animationDuration=`${3+Math.random()*5}s`,v.style.animationDelay=`${Math.random()*4}s`;const z=.3+Math.random()*.4,q=.7+Math.random()*.3;v.style.background=`rgba(${Math.floor(te*q)}, ${Math.floor(ne*q)}, ${Math.floor(oe*q)}, ${z})`,v.style.width=`${1+Math.random()*2}px`,v.style.height=v.style.width,t.appendChild(v)}return t}function zt(t){const e=document.createElement("div");e.className="nf-module";const a=document.createElement("div");a.className="nf-mod-header",a.innerHTML=`

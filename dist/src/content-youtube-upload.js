(function(){"use strict";if(window.__NETFLOW_YOUTUBE_UPLOAD_LOADED__)console.log("[NetFlow YouTube] Already loaded — skipping duplicate injection");else{window.__NETFLOW_YOUTUBE_UPLOAD_LOADED__=!0,console.log("[NetFlow YouTube] Content script loaded on:",window.location.href);const a=e=>new Promise(n=>setTimeout(n,e)),_=(e,n)=>Math.floor(Math.random()*(n-e+1))+e,r=e=>{console.log(`[NetFlow YouTube] ${e}`);try{chrome.runtime.sendMessage({action:"FLOW_LOG",level:"info",msg:`[YT] ${e}`})}catch{}},v=e=>{console.warn(`[NetFlow YouTube] ⚠️ ${e}`);try{chrome.runtime.sendMessage({action:"FLOW_LOG",level:"warn",msg:`[YT] ⚠️ ${e}`})}catch{}},m=async(e,n=2e4,l=500)=>{const d=Date.now();for(;Date.now()-d<n;){const u=typeof e=="string"?document.querySelector(e):e();if(u)return u;await a(l)}return null},R=(e,n)=>{const l=[];return l.push(...Array.from(e.querySelectorAll(n))),e.querySelectorAll("*").forEach(d=>{d.shadowRoot&&l.push(...R(d.shadowRoot,n))}),l},f=e=>{e.focus(),e.click(),e.dispatchEvent(new MouseEvent("click",{bubbles:!0,cancelable:!0}))},x=(e,n="button, a, span, div, label, tp-yt-paper-item, ytcp-text, yt-formatted-string, tp-yt-paper-radio-button",l=!1)=>{const d=Array.from(document.querySelectorAll(n)),u=R(document,n),E=[...d,...u],b=new Set;for(const w of E){if(b.has(w))continue;b.add(w);const g=(w.textContent||"").replace(/\s+/g," ").trim(),S=(w.innerText||"").replace(/\s+/g," ").trim();for(const $ of e){const C=l?g===$:g.includes($),U=l?S===$:S.includes($);if(C||U){const B=w.getBoundingClientRect();if(B.width>0&&B.height>0)return w}}}return null},F=async(e,n)=>{if(e.focus(),e.click(),await a(200),document.execCommand("selectAll"),await a(100),document.execCommand("delete"),await a(200),Math.random()<.5)document.execCommand("insertText",!1,n),r(`[Paste] "${n.substring(0,40)}..."`);else{for(const d of n)document.execCommand("insertText",!1,d),await a(_(30,120));r(`[Type] "${n.substring(0,40)}..."`)}await a(300)},L=async(e,n)=>{e.focus(),e.click(),await a(200),e.select(),await a(100),e.value="",e.dispatchEvent(new Event("input",{bubbles:!0})),await a(200);for(const l of n)e.value+=l,e.dispatchEvent(new Event("input",{bubbles:!0})),await a(_(50,150));e.dispatchEvent(new Event("change",{bubbles:!0})),await a(200)};let q=null,T=null,I=[],O=0,A=null;const P={green:{rgb:"0, 255, 65",hex:"#00ff41",accentRgb:"0, 255, 180",accentHex:"#00ffb4",doneRgb:"34, 197, 94",doneHex:"#22c55e"},red:{rgb:"220, 38, 38",hex:"#dc2626",accentRgb:"251, 146, 60",accentHex:"#fb923c",doneRgb:"34, 197, 94",doneHex:"#22c55e"},blue:{rgb:"43, 125, 233",hex:"#2b7de9",accentRgb:"6, 182, 212",accentHex:"#06b6d4",doneRgb:"34, 197, 94",doneHex:"#22c55e"},yellow:{rgb:"234, 179, 8",hex:"#eab308",accentRgb:"245, 158, 11",accentHex:"#f59e0b",doneRgb:"34, 197, 94",doneHex:"#22c55e"},purple:{rgb:"139, 92, 246",hex:"#8b5cf6",accentRgb:"168, 85, 247",accentHex:"#a855f7",doneRgb:"34, 197, 94",doneHex:"#22c55e"}},K=e=>{const n=e.rgb,l=e.accentRgb,d=e.doneRgb,u=e.hex,E=e.doneHex,b=u.match(/^#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i),w=b?Math.max(parseInt(b[1],16),parseInt(b[2],16),parseInt(b[3],16),1):255,g=b?parseInt(b[1],16)/w:0,S=b?parseInt(b[2],16)/w:1,$=b?parseInt(b[3],16)/w:.25,C=U=>`${Math.round(g*U)}, ${Math.round(S*U)}, ${Math.round($*U)}`;return`
...
`},z=e=>{if(q)return;const n=P[e||""]||P.green;T||(T=document.createElement("style"),document.head.appendChild(T)),T.textContent=K(n),O=Date.now(),q=document.createElement("div"),q.id="netflow-youtube-hud",q.innerHTML=`

...
@media (max-height: 600px) {
    ...
    `}function Rt(){W||(W=document.createElement("style"),W.id="netflow-overlay-styles",W.textContent=ce(M),document.head.appendChild(W))}function de(e){e.innerHTML="",A.forEach((t,n)=>{const a=document.createElement("div");a.className="nf-proc-row nf-proc-waiting",a.id=`nf-proc-${t.stepId}`,a.innerHTML=`
            <span class="nf-proc-num">${n+1}</span>
            <span class="nf-proc-dot"></span>
            <span class="nf-proc-label">${t.label}</span>
            <span class="nf-proc-badge">(queued)</span>
        `,e.appendChild(a)})}function Tt(e,t){let s="";for(let k=0;k<20;k++){const b=k/20*Math.PI*2,w=(k+.2)/20*Math.PI*2,h=(k+.5)/20*Math.PI*2,o=(k+.8)/20*Math.PI*2,u=(k+1)/20*Math.PI*2;s+=`${k===0?"M":"L"}${(120+100*Math.cos(b)).toFixed(1)},${(120+100*Math.sin(b)).toFixed(1)} `,s+=`L${(120+100*Math.cos(w)).toFixed(1)},${(120+100*Math.sin(w)).toFixed(1)} `,s+=`L${(120+112*Math.cos(h)).toFixed(1)},${(120+112*Math.sin(h)).toFixed(1)} `,s+=`L${(120+100*Math.cos(o)).toFixed(1)},${(120+100*Math.sin(o)).toFixed(1)} `,s+=`L${(120+100*Math.cos(u)).toFixed(1)},${(120+100*Math.sin(u)).toFixed(1)} `}s+="Z";const c=14,p=72,d=62;let x="";for(let k=0;k<c;k++){const b=k/c*Math.PI*2,w=(k+.25)/c*Math.PI*2,h=(k+.75)/c*Math.PI*2,o=(k+1)/c*Math.PI*2;x+=`${k===0?"M":"L"}${(120+d*Math.cos(b)).toFixed(1)},${(120+d*Math.sin(b)).toFixed(1)} `,x+=`L${(120+p*Math.cos(w)).toFixed(1)},${(120+p*Math.sin(w)).toFixed(1)} `,x+=`L${(120+p*Math.cos(h)).toFixed(1)},${(120+p*Math.sin(h)).toFixed(1)} `,x+=`L${(120+d*Math.cos(o)).toFixed(1)},${(120+d*Math.sin(o)).toFixed(1)} `}return x+="Z",`<svg width="72" height="72" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
        ...
        <g class="nf-kinetic-mid">
            ...
            <circle cx="120" cy="120" r="${d}" fill="none" stroke="rgba(${t},0.15)" stroke-width="0.3" stroke-dasharray="2,3"/>
        </g>
    ...
    </svg>`}function pe(){const e=document.createElement("div");e.id="netflow-engine-overlay",D=document.createElement("canvas"),D.id="nf-matrix-canvas",e.appendChild(D);const t=document.createElement("div");t.className="nf-pat-plasma",e.appendChild(t);for(let E=1;E<=5;E++){const v=document.createElement("div");v.className=`nf-ambient-orb nf-orb-${E}`,e.appendChild(v)}const n=document.createElement("div");n.className="nf-pat-data",e.appendChild(n);const a=document.createElement("div");a.className="nf-pat-diag-a",e.appendChild(a);const r=document.createElement("div");r.className="nf-pat-diag-b",e.appendChild(r);const i=document.createElement("div");i.className="nf-pat-circuit",e.appendChild(i);const l=document.createElement("div");l.className="nf-pat-honeycomb",e.appendChild(l);const s=document.createElement("div");s.className="nf-pat-binary",e.appendChild(s);const c=document.createElement("div");c.className="nf-pat-crosshatch",e.appendChild(c);const p=document.createElement("div");p.className="nf-pat-diamond",e.appendChild(p);const d=document.createElement("div");d.className="nf-pat-wave-h",e.appendChild(d);const x=document.createElement("div");x.className="nf-pat-radar",e.appendChild(x);const k=document.createElement("div");k.className="nf-pat-ripple-1",e.appendChild(k);const b=document.createElement("div");b.className="nf-pat-ripple-2",e.appendChild(b);const w=document.createElement("div");w.className="nf-pat-techscan",e.appendChild(w);const h=document.createElement("div");h.className="nf-center-glow",e.appendChild(h);const o=document.createElement("div");o.className="nf-pat-noise",e.appendChild(o);const u=document.createElement("div");u.className="nf-crt-scanlines",e.appendChild(u);const C=document.createElement("div");C.className="nf-vignette",e.appendChild(C);for(let E=0;E<3;E++){const v=document.createElement("div");v.className="nf-pulse-ring",e.appendChild(v)}["nf-deco-tl","nf-deco-tr","nf-deco-bl","nf-deco-br"].forEach(E=>{const v=document.createElement("div");v.className=`nf-corner-deco ${E}`,e.appendChild(v)});const G=document.createElement("button");G.className="nf-stop-btn",G.innerHTML='<span class="nf-stop-icon"></span> หยุด',G.onclick=()=>{var E;window.__NETFLOW_STOP__=!0;try{yt("⛔ ผู้ใช้หยุดการทำงาน")}catch{}try{typeof chrome<"u"&&((E=chrome.runtime)!=null&&E.sendMessage)&&chrome.runtime.sendMessage({action:"AUTOMATION_STOPPED"})}catch{}},e.appendChild(G);const V=document.createElement("button");V.className="nf-close-btn",V.textContent="✕ ซ่อน",V.onclick=()=>wt(),e.appendChild(V);const _=document.createElement("div");_.className="nf-layout";const m=document.createElement("div");m.className="nf-core-monitor",m.id="nf-core-monitor";const z=document.createElement("div");z.className="nf-core-header",z.innerHTML=`
    ...
    `,t.appendChild(n),e.steps.forEach(r=>{const i=document.createElement("div");i.className="nf-step",i.id=`nf-step-${r.id}`;let l="";r.progress!==void 0&&(l=`
            ...
            ${l}
        `,t.appendChild(i)});const a=document.createElement("div");return a.className="nf-mod-progress",a.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${e.id}"></div>`,t.appendChild(a),t}function ge(){It=Date.now(),lt=setInterval(()=>{const e=Math.floor((Date.now()-It)/1e3),t=String(Math.floor(e/60)).padStart(2,"0"),n=String(e%60).padStart(2,"0"),a=document.getElementById("nf-timer");a&&(a.textContent=`${t}:${n}`);const r=document.getElementById("nf-stat-elapsed");r&&(r.textContent=`${t}:${n}`)},1e3)}function zt(){lt&&(clearInterval(lt),lt=null)}const ue=120,_t=160,Pt=.4;let Y=null,Nt=0,Ot=0,Bt=0,rt=[];function me(e,t){rt=[];for(let n=0;n<ue;n++){const a=Math.random();let r;a<.22?r=0:a<.4?r=1:a<.55?r=2:a<.68?r=3:a<.84?r=4:r=5;const i=Math.random()*e,l=Math.random()*t,s=50+Math.random()*220,c=Math.random()*Math.PI*2,p=(.003+Math.random()*.008)*(Math.random()>.5?1:-1);rt.push({x:r===0?Math.random()*e:i+Math.cos(c)*s,y:r===0?Math.random()*t:l+Math.sin(c)*s,vx:(Math.random()-.5)*Pt,vy:(Math.random()-.5)*Pt,radius:1.2+Math.random()*2.5,pulsePhase:Math.random()*Math.PI*2,pulseSpeed:.02+Math.random()*.03,motion:r,oCx:i,oCy:l,oRadius:s,oAngle:c,oSpeed:p})}}function he(){if(!D)return;const e=D;if(et=e.getContext("2d"),!et)return;const t=()=>{e.width=window.innerWidth,e.height=window.innerHeight,rt.length===0&&me(e.width,e.height)};t(),window.addEventListener("resize",t);let n=null,a=0,r=0,i=!1;function l(){if(!et||!D){nt=null;return}if(nt=requestAnimationFrame(l),i=!i,i)return;const s=et,c=D.width,p=D.height;s.fillStyle=`rgba(${P*.04|0},${N*.04|0},${O*.06|0},1)`,s.fillRect(0,0,c,p),(!n||a!==c||r!==p)&&(a=c,r=p,n=s.createRadialGradient(c*.5,p*.5,0,c*.5,p*.5,Math.max(c,p)*.6),n.addColorStop(0,`rgba(${P*.08|0},${N*.08|0},${O*.1|0},0.4)`),n.addColorStop(1,"rgba(0,0,0,0)")),s.fillStyle=n,s.fillRect(0,0,c,p);const d=rt,x=d.length,k=_t*_t;for(let h=0;h<x;h++){const o=d[h];if(o.pulsePhase+=o.pulseSpeed,o.motion===0)o.x+=o.vx,o.y+=o.vy,o.x<0?(o.x=0,o.vx=Math.abs(o.vx)*(.8+Math.random()*.4)):o.x>c&&(o.x=c,o.vx=-Math.abs(o.vx)*(.8+Math.random()*.4)),o.y<0?(o.y=0,o.vy=Math.abs(o.vy)*(.8+Math.random()*.4)):o.y>p&&(o.y=p,o.vy=-Math.abs(o.vy)*(.8+Math.random()*.4));else if(o.motion===1)o.oAngle+=o.oSpeed,o.x=o.oCx+Math.cos(o.oAngle)*o.oRadius,o.y=o.oCy+Math.sin(o.oAngle)*o.oRadius,o.oCx+=Math.sin(o.oAngle*.3)*.15,o.oCy+=Math.cos(o.oAngle*.3)*.15;else if(o.motion===2)o.oAngle+=o.oSpeed,o.x=o.oCx+Math.cos(o.oAngle)*o.oRadius,o.y=o.oCy+Math.sin(o.oAngle)*o.oRadius*.5,o.oCx+=Math.sin(o.oAngle*.2)*.1,o.oCy+=Math.cos(o.oAngle*.2)*.1;else if(o.motion===3){o.oAngle+=o.oSpeed;const u=o.oAngle,C=o.oRadius*.7;o.x=o.oCx+C*Math.cos(u),o.y=o.oCy+C*Math.sin(u)*Math.cos(u),o.oCx+=Math.sin(u*.15)*.12,o.oCy+=Math.cos(u*.15)*.12}else if(o.motion===4){o.oAngle+=o.oSpeed*1.2;const u=o.oRadius*(.5+.5*Math.abs(Math.sin(o.oAngle*.15)));o.x=o.oCx+Math.cos(o.oAngle)*u,o.y=o.oCy+Math.sin(o.oAngle)*u,o.oCx+=Math.sin(o.oAngle*.1)*.18,o.oCy+=Math.cos(o.oAngle*.1)*.18}else o.oAngle+=o.oSpeed,o.x+=o.vx*.8,o.y=o.oCy+Math.sin(o.oAngle+o.x*.008)*o.oRadius*.35,o.x<-30?o.x=c+30:o.x>c+30&&(o.x=-30),o.oCy+=Math.sin(o.oAngle*.1)*.08;if(o.motion>0){const u=o.oRadius+50;o.oCx<-u?o.oCx=c+u:o.oCx>c+u&&(o.oCx=-u),o.oCy<-u?o.oCy=p+u:o.oCy>p+u&&(o.oCy=-u)}}s.beginPath(),s.strokeStyle=`rgba(${P},${N},${O},0.06)`,s.lineWidth=.4;const b=new Path2D;for(let h=0;h<x;h++){const o=d[h];for(let u=h+1;u<x;u++){const C=d[u],G=o.x-C.x,V=o.y-C.y,_=G*G+V*V;_<k&&(1-_/k<.4?(s.moveTo(o.x,o.y),s.lineTo(C.x,C.y)):(b.moveTo(o.x,o.y),b.lineTo(C.x,C.y)))}}if(s.stroke(),s.strokeStyle=`rgba(${P},${N},${O},0.18)`,s.lineWidth=.8,s.stroke(b),!Y||Nt!==P||Ot!==N||Bt!==O){Y=document.createElement("canvas");const h=48;Y.width=h,Y.height=h;const o=Y.getContext("2d"),u=o.createRadialGradient(h/2,h/2,0,h/2,h/2,h/2);u.addColorStop(0,`rgba(${P},${N},${O},0.9)`),u.addColorStop(.3,`rgba(${P},${N},${O},0.35)`),u.addColorStop(1,`rgba(${P},${N},${O},0)`),o.fillStyle=u,o.fillRect(0,0,h,h),Nt=P,Ot=N,Bt=O}const w=Y;for(let h=0;h<x;h++){const o=d[h],u=.6+.4*Math.sin(o.pulsePhase),C=o.radius*5*(.8+u*.4);s.globalAlpha=.5+u*.4,s.drawImage(w,o.x-C/2,o.y-C/2,C,C)}s.globalAlpha=1,s.fillStyle="rgba(255,255,255,0.45)",s.beginPath();for(let h=0;h<x;h++){const o=d[h];if(o.radius>2){const u=.6+.4*Math.sin(o.pulsePhase),C=o.radius*(.8+u*.4)*.35;s.moveTo(o.x+C,o.y),s.arc(o.x,o.y,C,0,Math.PI*2)}}s.fill()}l()}function xe(){nt!==null&&(cancelAnimationFrame(nt),nt=null),D=null,et=null,rt=[]}let ot=null;const pt=560,be=140,Lt=pt/2,Ft=be/2,Dt=[];for(let e=0;e<=pt;e+=8){const t=Math.abs(e-Lt)/Lt;Dt.push(Math.pow(Math.min(1,t*1.6),.6))}const we=[0,1,2,3].map(e=>({amp:10+e*5,freq:(1.2+e*.35)*Math.PI*2/pt,off:e*.6,spd:.7+e*.12}));let bt=!1;function Gt(){if(tt=requestAnimationFrame(Gt),bt=!bt,bt)return;if(xt+=.07,!ot){const t=document.getElementById("nf-engine-waves");if(!t){tt=null;return}ot=Array.from(t.querySelectorAll("path"))}const e=[];for(let t=0;t<ot.length;t++){const n=we[t],a=xt*n.spd+n.off;e.length=0,e.push(`M 0 ${Ft}`);let r=0;for(let i=0;i<=pt;i+=8){const l=Ft+n.amp*Dt[r++]*Math.sin(i*n.freq+a);e.push(`L${i} ${l*10+.5|0}`)}ot[t].setAttribute("d",e.join(" "))}}function ye(){xt=0,Gt(),he(),ct=setInterval(()=>{const e=document.getElementById("nf-stat-freq"),t=document.getElementById("nf-stat-lat1"),n=document.getElementById("nf-stat-lat2"),a=document.getElementById("nf-stat-buf");e&&(e.textContent=`${(4.5+Math.random()*.5).toFixed(1)} GHz`),t&&(t.textContent=`${Math.floor(12+Math.random()*10)}ms`),n&&(n.textContent=`${Math.floor(12+Math.random()*10)}ms`),a&&(a.textContent=`${Math.floor(90+Math.random()*9)}%`)},2e3)}function Ht(){tt!==null&&(cancelAnimationFrame(tt),tt=null),ct&&(clearInterval(ct),ct=null),ot=null,xe()}function Wt(){let e=0;const t=A.filter(p=>p.status!=="skipped").length;for(const p of A){const d=document.getElementById(`nf-proc-${p.stepId}`);if(!d)continue;d.className="nf-proc-row";const x=d.querySelector(".nf-proc-badge");switch(p.status){case"done":d.classList.add("nf-proc-done"),x&&(x.textContent="✅ done"),e++;break;case"active":d.classList.add("nf-proc-active"),x&&(x.textContent=p.progress!==void 0&&p.progress>0?`⏳ ${p.progress}%`:"⏳ active");break;case"error":d.classList.add("nf-proc-error"),x&&(x.textContent="❌ error");break;case"skipped":d.classList.add("nf-proc-skipped"),x&&(x.textContent="— skip");break;default:d.classList.add("nf-proc-waiting"),x&&(x.textContent="(queued)")}}const n=A.findIndex(p=>p.status==="active"),a=n>=0?n+1:e>=t&&t>0?A.length:e,r=document.getElementById("nf-step-counter");r&&(r.textContent=`${a}/${A.length}`);const i=document.querySelector(".nf-core-title-val"),l=document.querySelector(".nf-status-dot");e>=t&&t>0?(i&&(i.textContent="COMPLETE",i.style.color=M.doneHex),l&&(l.style.background=M.doneHex,l.style.boxShadow=`0 0 8px rgba(${M.doneRgb},0.7)`)):A.some(d=>d.status==="error")?(i&&(i.textContent="ERROR",i.style.color="#f87171"),l&&(l.style.background="#ef4444",l.style.boxShadow="0 0 8px rgba(239,68,68,0.7)")):A.some(d=>d.status==="active")&&i&&(i.textContent="ACTIVE",i.style.color=M.hex,i.style.textShadow=`0 0 10px rgba(${M.rgb},0.5)`);const s=document.getElementById("nf-terminal"),c=s==null?void 0:s.querySelector(".nf-proc-active");c&&s&&c.scrollIntoView({behavior:"smooth",block:"center"})}function Vt(){I&&I.isConnected||(Rt(),I=document.createElement("button"),I.id="nf-toggle-btn",I.className="nf-toggle-visible",I.innerHTML=j?St:Mt,I.title="ซ่อน/แสดง Netflow Overlay",I.onclick=()=>wt(),document.body.appendChild(I))}function wt(){S&&(Vt(),j?(S.classList.remove("nf-hidden"),S.classList.add("nf-visible"),I&&(I.innerHTML=Mt),j=!1):(S.classList.remove("nf-visible"),S.classList.add("nf-hidden"),I&&(I.innerHTML=St),j=!0))}const Ut={red:"themes/theme-red.jpg",yellow:"themes/theme-yellow.jpg",blue:"themes/theme-blue.jpg",purple:"themes/theme-purple.jpg",green:"themes/theme-green.jpg"};function jt(){const e=document.getElementById("nf-core-monitor");if(!e)return;let t=Q;if(!t)try{t=localStorage.getItem("netflow_app_theme")||"green"}catch{t="green"}const n=Ut[t]||Ut.green;let a;try{a=chrome.runtime.getURL(n)}catch{a=`/${n}`}const r=M.rgb;e.style.backgroundImage=["linear-gradient(180deg, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.78) 45%, rgba(0,0,0,0.90) 100%)",`linear-gradient(180deg, rgba(${r},0.25) 0%, rgba(${r},0.12) 50%, rgba(${r},0.20) 100%)`,`url('${a}')`].join(", "),e.style.backgroundSize="auto, auto, 50%",e.style.backgroundPosition="center, center, center",e.style.backgroundRepeat="no-repeat, no-repeat, no-repeat",e.style.setProperty("--nf-bg-set","1"),e.style.border=`1.5px solid rgba(${r},0.45)`,e.style.boxShadow=`0 0 70px rgba(${r},0.22), 0 0 140px rgba(${r},0.1), inset 0 1px 0 rgba(${r},0.15)`}function ve(e=1){if(M=le(),Ct(),S&&S.isConnected){j&&wt();return}if(S&&!S.isConnected&&(S=null),W&&(W.remove(),W=null),Rt(),dt=e,A=At(e),e>1){const t=at.find(a=>a.id==="video");if(t){const a=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:"waiting",progress:0}];for(let r=2;r<=e;r++)a.push({id:`scene${r}-prompt`,label:`Scene ${r} Prompt`,status:"waiting"}),a.push({id:`scene${r}-gen`,label:`Scene ${r} Generate`,status:"waiting"}),a.push({id:`scene${r}-wait`,label:`Scene ${r} รอผล`,status:"waiting",progress:0});t.steps=a}const n=at.find(a=>a.id==="render");if(n){const a=n.steps.find(i=>i.id==="download");a&&(a.label="ดาวน์โหลด 720p");const r=n.steps.find(i=>i.id==="upscale");r&&(r.label="Full Video")}}S=pe(),document.body.appendChild(S),j=!1,Vt(),ge(),ye(),requestAnimationFrame(()=>jt())}function qt(){zt(),Ht(),j=!1,S&&(S.classList.add("nf-fade-out"),setTimeout(()=>{S==null||S.remove(),S=null},500)),I&&(I.remove(),I=null)}const $e={settings:"CONFIG","upload-char":"UPLOAD","upload-prod":"UPLOAD","img-prompt":"PROMPT","img-generate":"GENERATE","img-wait":"IMG WAIT",animate:"ANIMATE","vid-prompt":"VID PROMPT","vid-generate":"VID GEN","vid-wait":"VID WAIT",download:"DOWNLOAD",upscale:"UPSCALE",open:"OPENING"};function ke(e,t,n){const a=A.findIndex(d=>d.status==="active"),r=A.filter(d=>d.status==="done").length,i=A.length,l=a>=0?a+1:r>=i?i:r,s=document.getElementById("nf-stat-step");s&&(s.textContent=`${l}/${i}`);let c=1;for(const d of A)if(d.status==="active"||d.status==="done")if(d.stepId.startsWith("scene")){const x=d.stepId.match(/^scene(\d+)-/);x&&(c=Math.max(c,parseInt(x[1],10)))}else(d.stepId==="download"||d.stepId==="upscale"||d.stepId==="open")&&(c=dt);const p=document.getElementById("nf-stat-scenes");if(p&&(p.textContent=dt>1?`${c}/${dt}`:"1/1"),t==="active"){const d=document.getElementById("nf-stat-status"),x=$e[e]||e.toUpperCase();d&&(d.textContent=x)}else if(t==="done"&&r>=i){const d=document.getElementById("nf-stat-status");d&&(d.textContent="COMPLETE")}else if(t==="error"){const d=document.getElementById("nf-stat-status");d&&(d.textContent="ERROR")}if(n!==void 0&&n>0){const d=document.getElementById("nf-stat-progress");d&&(d.textContent=`${Math.min(100,n)}%`)}}function $(e,t,n){if(!S)return;for(const r of at)for(const i of r.steps)i.id===e&&(i.status=t,n!==void 0&&(i.progress=n));for(const r of A)r.stepId===e&&(r.status=t,n!==void 0&&(r.progress=n));const a=document.getElementById(`nf-step-${e}`);if(a&&(a.className="nf-step",t==="active"?a.classList.add("nf-step-active"):t==="done"?a.classList.add("nf-step-done"):t==="error"&&a.classList.add("nf-step-error")),ke(e,t,n),n!==void 0){const r=document.getElementById(`nf-bar-${e}`);r&&(r.style.width=`${Math.min(100,n)}%`)}Yt(),Wt()}function Ee(e=4e3){zt(),Ht(),Yt(),Wt(),setTimeout(()=>qt(),e)}function Yt(){for(const e of at){const t=e.steps.filter(c=>c.status!=="skipped").length,n=e.steps.filter(c=>c.status==="done").length,a=e.steps.some(c=>c.status==="active"),r=t>0?Math.round(n/t*100):0,i=document.getElementById(`nf-pct-${e.id}`);i&&(i.textContent=`${r}%`);const l=document.getElementById(`nf-modbar-${e.id}`);l&&(l.style.width=`${r}%`);const s=document.getElementById(`nf-mod-${e.id}`);s&&(s.classList.remove("nf-active","nf-done"),r>=100?s.classList.add("nf-done"):a&&s.classList.add("nf-active"))}}function yt(e){e.replace(/^\[Netflow AI\]\s*/,"")}const ft="7.1.0",it="[Netflow AI — Grok]";window.__NETFLOW_GROK_LOADED__&&console.log(`${it} Already loaded — skipping duplicate injection`),window.__NETFLOW_GROK_LOADED__=!0,(()=>{var r,i;const e=navigator.userAgent,t=((r=e.match(/Chrome\/(\d+)/))==null?void 0:r[1])||"?",n=/Mac/i.test(e)?"macOS":/Windows/i.test(e)?"Windows":"Linux",a=!!(typeof chrome<"u"&&((i=chrome==null?void 0:chrome.runtime)!=null&&i.id));console.log(`%c NetflowAI Grok v${ft} %c ${n} · Chrome ${t} %c ${a?"✅ Ready":"❌ No Runtime"}`,"background:#1d9bf0;color:#fff;font-weight:bold;padding:2px 6px;border-radius:4px 0 0 4px","background:#334155;color:#fff;padding:2px 6px",`background:${a?"#16a34a":"#dc2626"};color:#fff;padding:2px 6px;border-radius:0 4px 4px 0`)})();const g=e=>new Promise(t=>setTimeout(t,e)),f=e=>{console.log(`${it} ${e}`);try{yt(e)}catch{}try{chrome.runtime.sendMessage({action:"FLOW_LOG",level:"info",msg:e})}catch{}},y=e=>{console.warn(`${it} ⚠️ ${e}`);try{yt(`⚠️ ${e}`)}catch{}try{chrome.runtime.sendMessage({action:"FLOW_LOG",level:"warn",msg:`⚠️ ${e}`})}catch{}},gt=e=>{var t,n;try{if(!((t=chrome.runtime)!=null&&t.id)){console.warn(`${it} Extension context invalidated`);return}chrome.runtime.sendMessage(e)}catch(a){(n=a.message)!=null&&n.includes("Extension context invalidated")&&console.error(`${it} ❌ Extension context invalidated! Refresh this page.`)}},L=(e,t)=>{gt({type:"GROK_STATUS",status:e,detail:t})};let vt=!1;const ut=()=>vt,X=()=>{const e=document.querySelectorAll('[contenteditable="true"]');for(const n of e){const a=n,r=a.getBoundingClientRect();if(r.width>150&&r.bottom>window.innerHeight*.5)return a}const t=['[placeholder*="imagine" i]','[aria-label*="imagine" i]','[data-placeholder*="imagine" i]','textarea[placeholder*="imagine" i]',"textarea"];for(const n of t){const a=document.querySelector(n);if(a&&a.getBoundingClientRect().width>100)return a}for(const n of e){const a=n,r=a.getBoundingClientRect();if(r.width>100&&r.height>10)return a}return null},Xt=async(e=2e4)=>{f("⏳ Waiting for grok.com/imagine root page...");const t=Date.now();for(;Date.now()-t<e;){const n=window.location.href;if(/grok\.com\/imagine\/?$/.test(n)||/grok\.com\/imagine\/?(\?|#|$)/.test(n)){const r=X();if(r)return f(`✅ On imagine root, input found (${r.tagName})`),!0}const a=Math.round((Date.now()-t)/1e3);a%3===0&&a>0&&f(`Still waiting for imagine root... ${a}s`),await g(800)}return!1},R=(e,t)=>{const n=document.querySelectorAll(e),a=t.toLowerCase();for(const r of n)if((r.textContent||"").trim().toLowerCase().includes(a)&&r.offsetParent!==null)return r;return null},T=async e=>{var r,i;const t=e;(r=t.scrollIntoView)==null||r.call(t,{block:"center",behavior:"instant"}),await g(100),(i=t.focus)==null||i.call(t),t.click(),await g(50);const n=e.getBoundingClientRect(),a={bubbles:!0,cancelable:!0,clientX:n.left+n.width/2,clientY:n.top+n.height/2};e.dispatchEvent(new MouseEvent("pointerdown",a)),e.dispatchEvent(new MouseEvent("mousedown",a)),e.dispatchEvent(new MouseEvent("pointerup",a)),e.dispatchEvent(new MouseEvent("mouseup",a)),e.dispatchEvent(new MouseEvent("click",a))},Kt=async e=>{const t=e.getBoundingClientRect(),n={bubbles:!0,cancelable:!0,clientX:t.left+t.width/2,clientY:t.top+t.height/2};e.dispatchEvent(new MouseEvent("mouseenter",{...n,bubbles:!1})),e.dispatchEvent(new MouseEvent("mouseover",n)),e.dispatchEvent(new MouseEvent("mousemove",n)),e.dispatchEvent(new PointerEvent("pointerenter",{...n,bubbles:!1})),e.dispatchEvent(new PointerEvent("pointerover",n)),e.dispatchEvent(new PointerEvent("pointermove",n))},mt=async(e,t)=>{var a,r;if(e.focus(),e.click(),await g(150),e.getAttribute("contenteditable")==="true"||e.isContentEditable)e.textContent="",e.focus(),await g(100),document.execCommand("selectAll",!1),document.execCommand("delete",!1),document.execCommand("insertText",!1,t),e.dispatchEvent(new InputEvent("input",{bubbles:!0,data:t,inputType:"insertText"})),e.dispatchEvent(new Event("change",{bubbles:!0}));else{const i=e,l=((a=Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype,"value"))==null?void 0:a.set)||((r=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,"value"))==null?void 0:r.set);l?l.call(i,t):i.value=t,i.dispatchEvent(new Event("input",{bubbles:!0})),i.dispatchEvent(new Event("change",{bubbles:!0}));for(const s of t.slice(-3))i.dispatchEvent(new InputEvent("input",{bubbles:!0,data:s,inputType:"insertText"}))}await g(400)},Jt=(e,t)=>{var n;try{if(e.startsWith("http://")||e.startsWith("https://")||e.startsWith("blob:"))return y("base64ToFile received a URL, not base64 — use fetchImageAsFile instead"),null;let a="image/png",r;if(e.startsWith("data:")){const s=e.indexOf(",");if(s===-1)return y("Invalid data URL — no comma separator"),null;const c=e.substring(0,s);r=e.substring(s+1),a=((n=c.match(/:(.*?);/))==null?void 0:n[1])||"image/png"}else r=e;if(r=r.replace(/[\s\r\n]/g,""),!r||r.length<10)return y("base64 data too short or empty"),null;const i=atob(r),l=new Uint8Array(i.length);for(let s=0;s<i.length;s++)l[s]=i.charCodeAt(s);return new File([l],t,{type:a})}catch(a){return y("base64ToFile failed: "+a),null}},Ce=async(e,t)=>{try{f(`🌐 Fetching image from URL: ${e.substring(0,80)}...`);const n=await fetch(e,{mode:"cors"});if(!n.ok)return y(`Fetch failed: ${n.status} ${n.statusText}`),null;const a=await n.blob(),r=a.type||"image/png",i=new File([a],t,{type:r});return f(`✅ Fetched image: ${(i.size/1024).toFixed(1)} KB, type: ${r}`),i}catch(n){y("fetchImageAsFile failed: "+n);try{const a=document.createElement("img");a.crossOrigin="anonymous",a.src=e,await new Promise((s,c)=>{a.onload=()=>s(),a.onerror=()=>c(new Error("Image load failed")),setTimeout(()=>c(new Error("Image load timeout")),1e4)});const r=document.createElement("canvas");r.width=a.naturalWidth,r.height=a.naturalHeight;const i=r.getContext("2d");if(!i)return null;i.drawImage(a,0,0);const l=r.toDataURL("image/png");return Jt(l,t)}catch(a){return y("Canvas fallback also failed: "+a),null}}},Se=async(e,t)=>e.startsWith("http://")||e.startsWith("https://")||e.startsWith("blob:")?Ce(e,t):Jt(e,t),$t=()=>{var i,l,s;const e=X();if(e){const c=e.closest("form")||((l=(i=e.parentElement)==null?void 0:i.parentElement)==null?void 0:l.parentElement)||((s=e.parentElement)==null?void 0:s.parentElement)||e.parentElement;if(c){const p=e.getBoundingClientRect(),d=Array.from(c.querySelectorAll("button")),x=d.filter(k=>{const b=k.getBoundingClientRect();return Math.abs(b.top-p.top)<60&&b.left>=p.right-60});if(x.length>0)return x.sort((k,b)=>b.getBoundingClientRect().right-k.getBoundingClientRect().right),x[0];if(d.length>0)return d[d.length-1]}}const t=document.querySelector('button[type="submit"]');if(t)return t;const n=document.querySelectorAll("button");let a=null,r=0;for(const c of n){const p=c.getBoundingClientRect();p.bottom>window.innerHeight*.75&&p.right>r&&c.querySelector("svg")&&(a=c,r=p.right)}return a},Zt=async(e,t="image")=>{f(`📤 Injecting ${t} into Grok file input...`),L("uploading",`กำลังแนบรูป${t==="product"?"สินค้า":"ตัวละคร"}เข้า Grok...`);const n=await Se(e,`${t}_${Date.now()}.png`);if(!n)return y(`imageSourceToFile failed for ${t}`),!1;f(`📦 ${t} file ready: ${n.name} (${(n.size/1024).toFixed(1)} KB)`);const a=()=>{const l=document.querySelectorAll('input[type="file"]');for(const s of l){const c=s,p=c.accept||"";if(!p||p.includes("image")||p.includes("*"))return c}return l.length>0?l[0]:null};let r=a();if(!r){f("No file input found, searching for attach button...");const l=R('button, [role="button"], label',"Image")||document.querySelector('[aria-label*="image" i], [aria-label*="attach" i], [aria-label*="photo" i]');if(l){const s=HTMLInputElement.prototype,c=s.click;s.click=function(){if(this.type==="file"){r=this,f("Captured file input via button click intercept");return}return c.call(this)},l.click(),await g(300),s.click=c}r||(r=a())}if(!r){y("Could not find file input — trying drag-and-drop fallback");const l=X()||document.querySelector("main")||document.body,s=new DataTransfer;return s.items.add(n),l.dispatchEvent(new DragEvent("dragenter",{bubbles:!0,dataTransfer:s})),await g(100),l.dispatchEvent(new DragEvent("dragover",{bubbles:!0,dataTransfer:s})),await g(100),l.dispatchEvent(new DragEvent("drop",{bubbles:!0,dataTransfer:s})),await g(2e3),!1}const i=new DataTransfer;return i.items.add(n),r.files=i.files,r.dispatchEvent(new Event("change",{bubbles:!0})),r.dispatchEvent(new Event("input",{bubbles:!0})),f(`✅ File injected: ${n.name} (${(n.size/1024).toFixed(1)} KB)`),await g(2500),!0},Qt=()=>{const e=new Set;document.querySelectorAll("img").forEach(n=>{n.src&&n.getBoundingClientRect().width>50&&e.add(n.src)});const t=new Set;return document.querySelectorAll("video").forEach(n=>{n.src&&t.add(n.src),n.querySelectorAll("source").forEach(a=>{a.src&&t.add(a.src)})}),{imgs:e,videos:t,count:e.size+t.size}},Me=async(e,t=18e4)=>{f("⏳ Waiting for new content to appear...");const n=Date.now();for(;Date.now()-n<t;){if(ut())return!1;const a=Qt();let r=0;if(a.imgs.forEach(l=>{e.imgs.has(l)||r++}),a.videos.forEach(l=>{e.videos.has(l)||r++}),r>0)return f(`✅ Found ${r} new content item(s)`),await g(3e3),!0;const i=Math.round((Date.now()-n)/1e3);i%10===0&&i>0&&f(`⏳ Still waiting... ${i}s`),await g(2e3)}return y("Timed out waiting for new content"),!1},Ie=(e=80,t=80)=>{const a=(document.querySelector("main")||document.body).querySelectorAll("img"),r=[];for(const i of a){const l=i.getBoundingClientRect();if(l.width<e||l.height<t||l.top<0||l.bottom>window.innerHeight+200)continue;const s=i.getAttribute("src")||"";s.includes("avatar")||s.includes("icon")||s.includes("logo")||s.includes("favicon")||s.includes("profile")||s.includes("emoji")||r.push(i)}return r.sort((i,l)=>{const s=i.getBoundingClientRect(),c=l.getBoundingClientRect();return c.width*c.height-s.width*s.height}),r},Ae=async(e=45e3)=>{f("🖼️ Looking for a generated image to hover...");const t=Date.now();let n=0;for(;Date.now()-t<e;){n++;const a=n<=3?120:n<=6?80:50,r=Ie(a,a);if(r.length>0){const l=r[0],s=l.closest("a, div[class], article, li, [data-testid]")||l;f(`Found ${r.length} image(s) (attempt ${n}), hovering on first one`),l.scrollIntoView({block:"center",behavior:"instant"}),await g(300),await Kt(s),await g(600),await Kt(l),await g(800);const c=l.getBoundingClientRect();for(let p=0;p<3;p++){const d=c.left+c.width*(.3+p*.2),x=c.top+c.height*.5;l.dispatchEvent(new MouseEvent("mousemove",{bubbles:!0,clientX:d,clientY:x})),s.dispatchEvent(new MouseEvent("mousemove",{bubbles:!0,clientX:d,clientY:x})),await g(200)}return await g(1e3),s}const i=Math.round((Date.now()-t)/1e3);i%5===0&&f(`⏳ Still looking for generated image... ${i}s`),await g(2e3)}return y("No suitable images found after "+Math.round(e/1e3)+"s"),null},te=()=>{const e='button, a, div[role="button"], div[role="menuitem"], span, li, [class*="option"], [class*="action"], [class*="menu-item"]';let t=R(e,"Make video");if(t||(t=R(e,"Animate Image"),t))return t;if(t=R(e,"Animate"),t){const a=(t.textContent||"").trim();if(a.length<80&&/animate/i.test(a))return t}if(t=R(e,"Turn an image to a video"),t)return t.closest('button, a, div[role="button"], li, [class*="option"]')||t;const n=document.querySelectorAll("*");for(const a of n){if(a.children.length>5)continue;const r=(a.textContent||"").trim();if(r.length>0&&r.length<60&&/animate\s*image|make\s*video/i.test(r)){if(a.offsetParent===null)continue;return a}}return null},Re=async(e=8)=>{f('🎬 Looking for "Make video" / "Animate Image" button...'),L("animating","กำลังหาปุ่ม Make video...");const t=Date.now(),n=6e4;for(let a=0;a<e&&Date.now()-t<n;a++){const r=te();if(r)return f(`Found "Make video" button directly (attempt ${a})`),await T(r),await g(2e3),st()||K()||await g(2e3),!0;if(a>=1&&(f(`Trying hover strategy (attempt ${a})...`),await Ae(1e4))){await g(1500);const l=te();if(l)return f("Found button after hover"),await T(l),await g(3e3),!0}await g(3e3)}return y('Could not find "Make video" button'),!1},st=()=>/\/(post|video)\//i.test(window.location.href),K=()=>{var t;const e=document.querySelectorAll("video");for(const n of e){if(!(n.src||((t=n.querySelector("source[src]"))==null?void 0:t.getAttribute("src"))||""))continue;const r=n.getBoundingClientRect();if(r.width>200&&r.height>200)return n}return null},ee=()=>{const e=document.body.textContent||"";if(/generating\s*\d+%/i.test(e))return!0;const t=document.querySelectorAll('[role="progressbar"], progress');for(const a of t)if(a.getBoundingClientRect().width>50)return!0;return!!R("button","Cancel")},ne=()=>{const e=document.querySelectorAll('[role="progressbar"]');for(const r of e){const i=r.getAttribute("aria-valuenow");if(i)return parseFloat(i)}const n=(document.body.textContent||"").match(/generating\s*(\d+)%/i);if(n)return parseInt(n[1],10);const a=document.querySelectorAll('[class*="progress"] > div, [class*="bar"]');for(const r of a){const i=r.style.width;if(i&&i.endsWith("%")){const l=parseFloat(i);if(l>0&&l<=100)return l}}return-1},Te=async(e=6e5)=>{f("⏳ Waiting for video to be ready (up to 10 min)...");const t=Date.now();for(;Date.now()-t<e;){if(ut())return!1;if(ee()){const r=ne();if(r>=0){const i=Math.round((Date.now()-t)/1e3);i%10===0&&f(`⏳ Video generating: ${r}% (${i}s)`);try{$("vid-wait","active",r)}catch{}}await g(3e3);continue}const n=K();if(n){if(n.readyState>=2)return f("✅ Video ready (readyState >= 2)"),!0;if(await g(2e3),n.readyState>=2)return f("✅ Video ready after brief wait"),!0}if(R("button, a","Download"))return f("✅ Download button found — video likely ready"),!0;await g(3e3)}return y("⏰ Timed out waiting for video"),!1},ae=()=>{if(!st())return y("getVideoUrl called but not on /post/ page"),null;const e=K();if(e!=null&&e.src)return e.src;if(e){const a=e.querySelector("source[src]");if(a)return a.src}const t=document.querySelector('a[download], a[href*="download"]');if(t!=null&&t.href)return t.href;const n=R("button, a","Download");return n!=null&&n.href?n.href:window.location.href},ze=()=>{let e=R('button, a, div[role="button"], [role="menuitem"]',"Extend video");return e||(e=document.querySelector('[aria-label*="Extend" i]'),e&&e.offsetParent!==null)?e:null},_e=()=>{const e=K();if(!e)return null;const t=e.getBoundingClientRect(),a=Array.from(document.querySelectorAll("button")).filter(r=>{const i=r.getBoundingClientRect();return!(i.width<10||i.width>60||i.height<10||i.height>60||i.left<t.right-60||i.top<t.top-50||i.bottom>t.bottom+50||!r.querySelector("svg")&&!/⋯|\.\.\.|\u22EF|\u2026|more/i.test(r.textContent||""))});return a.length===0?null:(a.sort((r,i)=>i.getBoundingClientRect().bottom-r.getBoundingClientRect().bottom),a[0])},re=async(e=6e4)=>{f("🔄 Looking for Extend video button...");const t=Date.now();for(;Date.now()-t<e;){if(ut())return!1;if(!st())return y("Not on /post/ page — aborting extend"),!1;const n=ze();if(n)return f('Found direct "Extend video" button'),await T(n),await g(2e3),!0;const a=_e();if(a){f("Found ⋯ menu button, clicking..."),await T(a),await g(1e3);for(let r=0;r<5;r++){const i=R('[role="menuitem"], [role="option"], button, a, div[class*="menu"], li',"Extend video");if(i)return f('Found "Extend video" in dropdown'),await T(i),await g(2e3),!0;await g(500)}document.dispatchEvent(new KeyboardEvent("keydown",{key:"Escape",bubbles:!0})),await g(500)}await g(3e3)}return y("Could not find Extend video option"),!1},oe=async(e=15e3)=>{f("🔍 Looking for Extend prompt input...");const t=Date.now();for(;Date.now()-t<e;){const n=X();if(n)return n;const a=document.querySelector("textarea");if(a&&a.offsetParent!==null)return a;await g(1e3)}return null},ie=async(e,t,n=6e5)=>{var i;f(`⏳ Waiting for extended video (Scene ${t})...`);const a=Date.now(),r=`scene${t}-wait`;for(;Date.now()-a<n;){if(ut())return!1;if(ee()){const c=ne();if(c>=0)try{$(r,"active",c)}catch{}await g(3e3);continue}const l=K();if(l){const c=l.src||((i=l.querySelector("source"))==null?void 0:i.getAttribute("src"))||"";if(c&&c!==e)return f(`✅ Extended video ready (Scene ${t}) — src changed`),!0}if(!st())return y("Left /post/ page during extend wait"),!1;const s=Math.round((Date.now()-a)/1e3);s%15===0&&s>0&&f(`⏳ Still waiting for extend... ${s}s`),await g(3e3)}return y(`⏰ Timed out waiting for extended video (Scene ${t})`),!1},Pe=async(e,t)=>{var k;const n=e+1,a=`scene${n}-prompt`,r=`scene${n}-gen`,i=`scene${n}-wait`;if(f(`═══ Scene ${n}: Starting Extend ═══`),!st())return y(`Not on /post/ page — cannot extend for Scene ${n}`),!1;const l=K(),s=(l==null?void 0:l.src)||((k=l==null?void 0:l.querySelector("source"))==null?void 0:k.getAttribute("src"))||null;try{$(a,"active")}catch{}if(f(`Scene ${n}: Clicking Extend...`),!await re(6e4)){y(`Failed to click Extend for Scene ${n}`);try{$(a,"error")}catch{}return!1}try{$(a,"done")}catch{}try{$(r,"active")}catch{}f(`Scene ${n}: Typing script...`);const p=await oe(15e3);if(!p){y(`No prompt input for Scene ${n}`);try{$(r,"error")}catch{}return!1}await mt(p,t),await g(500);const d=$t();d?await T(d):p.dispatchEvent(new KeyboardEvent("keydown",{key:"Enter",bubbles:!0})),await g(2e3);try{$(r,"done")}catch{}try{$(i,"active",0)}catch{}if(f(`Scene ${n}: Waiting for video generation...`),!await ie(s,n,6e5)){y(`Scene ${n}: First attempt failed, retrying...`);const b=t.substring(0,Math.min(t.length,200));for(let w=0;w<2;w++){if(!await re(3e4))continue;const o=await oe(1e4);if(!o)continue;await mt(o,b),await g(500);const u=$t();if(u?await T(u):o.dispatchEvent(new KeyboardEvent("keydown",{key:"Enter",bubbles:!0})),await g(2e3),await ie(s,n,6e5)){try{$(i,"done",100)}catch{}return!0}}try{$(i,"error")}catch{}return!1}try{$(i,"done",100)}catch{}return!0},Ne=e=>{const t=(e||"").toLowerCase();return/fitness|gym|sport|workout|protein/i.test(t)?"in a modern gym setting with equipment and mirrors":/beauty|skincare|makeup|cream|serum/i.test(t)?"in a spa-like vanity setting with soft lighting and mirror":/perfume|fragrance|cologne|scent/i.test(t)?"in a luxury setting with golden bokeh and soft focus":/fashion|clothing|dress|shirt|shoes/i.test(t)?"in an editorial fashion setting with dramatic lighting":/food|snack|drink|coffee|tea|restaurant/i.test(t)?"in a stylish kitchen or café setting with warm lighting":/tech|phone|laptop|gadget|computer/i.test(t)?"in a clean modern workspace with ambient lighting":/home|furniture|decor|kitchen|bed/i.test(t)?"in a beautifully designed home interior":/pet|dog|cat|animal/i.test(t)?"in a warm, pet-friendly environment":/car|vehicle|auto|motor/i.test(t)?"in a sleek showroom or scenic road setting":/baby|kid|child|toy/i.test(t)?"in a safe, colorful, child-friendly space":"in a professional studio setting with clean background"},Oe=async e=>{const{imagePrompt:t,videoPrompt:n,characterImage:a,productImage:r,aspectRatio:i,sceneCount:l,sceneScripts:s,productName:c,theme:p,resolution:d,duration:x}=e;try{p&&se(p),ve(l,"grok")}catch(m){f(`⚠️ Overlay error: ${m.message}`)}if(f("Step 1/8: Waiting for Grok Imagine page..."),L("step_1","รอหน้า Grok Imagine..."),!await Xt(25e3)&&!/grok\.com\/imagine/i.test(window.location.href)&&(f("Navigating to /imagine..."),window.location.href="https://grok.com/imagine",await g(3e3),!await Xt(15e3)))return y("Could not reach Grok Imagine page"),{success:!1};f("Step 2/8: Uploading images..."),L("step_2","กำลังแนบรูป...");try{$("upload-char","active")}catch{}a&&(await Zt(a,"character")?f("✅ Character image uploaded"):y("Character image upload failed (continuing)"));try{$("upload-char","done")}catch{}try{$("upload-prod","active")}catch{}r&&(await Zt(r,"product")?f("✅ Product image uploaded"):y("Product image upload failed (continuing)"));try{$("upload-prod","done")}catch{}f("Step 3/8: Typing image prompt..."),L("step_3","กำลังใส่ prompt...");try{$("img-prompt","active")}catch{}let b=n||t||"";if(c){const m=Ne(c);m&&!b.includes(m)&&(b=b+" "+m)}if(b.length>500&&(b=b.substring(0,500)),!b)return y("No prompt text provided"),{success:!1};let w=X();if(w||(await g(2e3),w=X()),!w)return y("Could not find prompt input"),{success:!1};await mt(w,b),await g(500),(w.textContent||w.value||"").trim().length===0&&(f("Retrying prompt entry..."),await mt(w,b),await g(500));try{$("img-prompt","done")}catch{}f("Step 4/8: Clicking Generate..."),L("step_4","กด Generate...");try{$("img-generate","active")}catch{}const o=Qt(),u=$t();u?await T(u):w.dispatchEvent(new KeyboardEvent("keydown",{key:"Enter",bubbles:!0})),await g(2e3);try{$("img-generate","done")}catch{}f("Step 5/8: Waiting for image generation..."),L("step_5","กำลังรอสร้างภาพ...");try{$("img-wait","active",0)}catch{}if(!await Me(o,3e5)){y("No new content after 5 minutes");try{$("img-wait","error")}catch{}return{success:!1}}await g(3e3);try{$("img-wait","done",100)}catch{}f('Step 6/8: Clicking "Make video"...'),L("step_6","กำลังคลิก Make video...");try{$("animate","active")}catch{}if(!await Re(8)){y('Could not find/click "Make video" button');try{$("animate","error")}catch{}return{success:!1}}try{$("animate","done")}catch{}f("Step 7/8: Setting aspect ratio..."),L("step_7","ตั้ง Aspect Ratio...");try{$("vid-prompt","active")}catch{}if(i){const m=document.querySelector('button[aria-label="Aspect Ratio"]');if(m){const z=(m.textContent||"").trim();if(z.includes(i))f(`Aspect ratio already ${i} — skipping`);else{f(`Opening Aspect Ratio dropdown (current: ${z})...`),await T(m),await g(800);const U=document.querySelectorAll('[role="menuitemradio"], [role="menuitem"], [role="option"], [data-radix-collection-item]');let B=!1;for(const F of U){const J=(F.textContent||"").trim();if(J===i||J.replace(/\s/g,"")===i.replace(/\s/g,"")){f(`Selecting aspect ratio: "${J}"`),await T(F),B=!0;break}}if(!B){const F=R('[role="menuitemradio"], [role="menuitem"], [role="option"], div, span',i);F?(await T(F),f(`Fallback: selected ${i}`)):y(`Could not find aspect ratio option: ${i}`)}await g(600)}}else{const z=R('button, div[role="button"], span',i);z?(await T(z),f(`Direct click aspect ratio: ${i}`),await g(600)):y("Could not find Aspect Ratio trigger button")}}if(d){const m=R('button, div[role="button"], span, [class*="option"]',d);m?(f(`Setting resolution: ${d}`),await T(m),await g(600)):f(`Resolution button "${d}" not found — may already be set`)}if(x){const m=R('button, div[role="button"], span, [class*="option"]',x);m?(f(`Setting duration: ${x}`),await T(m),await g(600)):f(`Duration button "${x}" not found — may already be set`)}try{$("vid-prompt","done")}catch{}f("Step 8/8: Waiting for video generation..."),L("step_8","กำลังสร้างวิดีโอฉาก 1...");try{$("vid-wait","active",0)}catch{}if(!await Te(6e5)){y("Video generation timed out");try{$("vid-wait","error")}catch{}return{success:!1}}try{$("vid-wait","done",100)}catch{}if(f("✅ Scene 1 video ready"),l>1)for(let m=1;m<l;m++){const z=m+1;f(`═══ Starting Scene ${z} ═══`);const U=s[m]||s[0]||n;if(!await Pe(m,U))return y(`Scene ${z} extend failed — using Scene 1 result`),{success:!0,videoUrl:ae()||void 0};f(`✅ Scene ${z} complete`)}const _=ae();return f(`🎬 Pipeline complete! Video URL: ${(_==null?void 0:_.substring(0,80))||"N/A"}`),{success:!0,videoUrl:_||void 0}};async function Be(e){f("═══ Grok Automation Started ═══"),f(`Engine: Grok v${ft} | Scenes: ${e.sceneCount||1} | Orientation: ${e.orientation||"vertical"}`);const t=Math.min(3,Math.max(1,e.sceneCount||1));let n=[];if(e.videoScenePrompts&&e.videoScenePrompts.length>0)n=e.videoScenePrompts;else if(e.videoPrompt){const r=e.videoPrompt.split(/\n{2,}/).map(i=>i.trim()).filter(i=>i.length>0);n=Array.from({length:t},(i,l)=>r[l]||r[0]||e.videoPrompt)}const a={imagePrompt:e.imagePrompt||e.videoPrompt||"",videoPrompt:e.videoPrompt||e.imagePrompt||"",characterImage:e.characterImage,productImage:e.productImage,aspectRatio:e.grokAspectRatio||(e.orientation==="horizontal"?"16:9":"9:16"),sceneCount:t,sceneScripts:n,productName:e.productName,autoPostTikTok:e.autoPostTikTok,theme:e.theme,resolution:e.grokResolution||"480p",duration:e.grokDuration||"6s"};try{const r=await Oe(a);if(r.success){gt({type:"VIDEO_GENERATION_COMPLETE",videoUrl:r.videoUrl,sceneCount:t,source:"grok"});try{$("complete","done")}catch{}try{Ee(8e3)}catch{}return{success:!0,message:`วิดีโอสำเร็จ (${t} ฉาก)`,step:"complete"}}else return gt({type:"VIDEO_GENERATION_ERROR",error:"Pipeline failed",source:"grok"}),{success:!1,message:"Grok pipeline failed — ลองใหม่อีกครั้ง",step:"pipeline-error"}}catch(r){return y(`Pipeline crashed: ${r.message}`),gt({type:"VIDEO_GENERATION_ERROR",error:r.message,source:"grok"}),{success:!1,message:`Error: ${r.message}`,step:"error"}}}chrome.runtime.onMessage.addListener((e,t,n)=>{if(e!=null&&e.action){if(e.action==="PING"){n({status:"ready",engine:"grok",version:ft,url:window.location.href});return}if(e.action==="STOP_AUTOMATION"){vt=!0,f("⛔ ผู้ใช้สั่งหยุด automation");try{qt()}catch{}n({success:!0,message:"Stopped"});return}if(e.action==="GENERATE_IMAGE")return vt=!1,Be(e).then(a=>n(a),a=>n({success:!1,message:`Error: ${a.message}`,step:"error"})),!0}}),f(`✅ Grok content script v${ft} loaded on: ${window.location.href}`)})();

...
@media (max-height: 600px) {
    ...
    `,T.appendChild(V);const U=document.createElement("div");U.className="nf-terminal",U.id="nf-terminal",Me(U),T.appendChild(U);const B=document.createElement("div");B.className="nf-engine-core",B.id="nf-engine-core";const ee=document.createElement("div");ee.className="nf-engine-frame",["nf-fc-tl","nf-fc-tr","nf-fc-bl","nf-fc-br"].forEach(u=>{const m=document.createElement("div");m.className=`nf-frame-corner ${u}`,ee.appendChild(m)}),B.appendChild(ee);const de="http://www.w3.org/2000/svg",H=document.createElementNS(de,"svg");H.setAttribute("class","nf-engine-waves"),H.setAttribute("viewBox","0 0 560 140"),H.setAttribute("preserveAspectRatio","none"),H.id="nf-engine-waves";for(let u=0;u<4;u++){const m=document.createElementNS(de,"path");m.setAttribute("fill","none"),m.setAttribute("stroke-width",u<2?"1.5":"1"),m.setAttribute("stroke",u<2?`rgba(${k.rgb},${.14+u*.1})`:`rgba(${k.accentRgb},${.1+(u-2)*.08})`),m.setAttribute("data-wave-idx",String(u)),H.appendChild(m)}B.appendChild(H);const te=document.createElement("div");te.className="nf-engine-brand-inner",te.innerHTML=`
    ...
    `,B.appendChild(te);const W=document.createElement("div");W.className="nf-engine-stats",W.id="nf-engine-stats",W.innerHTML=[["SCENES","nf-stat-scenes","1/1"],["ELAPSED","nf-stat-elapsed","00:00"],["STEP","nf-stat-step","0/0"],["STATUS","nf-stat-status","READY"],["PROGRESS","nf-stat-progress","—"]].map(([u,m,O])=>`<div class="nf-stat-item"><span class="nf-stat-label">${u}</span><span class="nf-stat-val" id="${m}">${O}</span></div>`).join(""),B.appendChild(W),T.appendChild(B),_.appendChild(T);const Pe=["nf-mod-tl","nf-mod-tr","nf-mod-bl","nf-mod-br"];I.forEach((u,m)=>{const O=_e(u);O.classList.add(Pe[m]),O.id=`nf-mod-${u.id}`,_.appendChild(O)}),t.appendChild(_);for(let u=0;u<30;u++){const m=document.createElement("div");m.className="nf-particle",m.style.left=`${5+Math.random()*90}%`,m.style.bottom=`${Math.random()*40}%`,m.style.animationDuration=`${3+Math.random()*5}s`,m.style.animationDelay=`${Math.random()*4}s`;const O=.3+Math.random()*.4,ne=.7+Math.random()*.3;m.style.background=`rgba(${Math.floor(E*ne)}, ${Math.floor(S*ne)}, ${Math.floor(z*ne)}, ${O})`,m.style.width=`${1+Math.random()*2}px`,m.style.height=m.style.width,t.appendChild(m)}return t}function _e(t){const e=document.createElement("div");e.className="nf-module";const a=document.createElement("div");a.className="nf-mod-header",a.innerHTML=`

...
@media (max-height: 600px) {
    ...
    `}function ke(){A||(A=document.createElement("style"),A.id="netflow-overlay-styles",A.textContent=Le(v),document.head.appendChild(A))}function Me(t){t.innerHTML="",w.forEach((e,a)=>{const o=document.createElement("div");o.className="nf-proc-row nf-proc-waiting",o.id=`nf-proc-${e.stepId}`,o.innerHTML=`
            <span class="nf-proc-num">${a+1}</span>
            <span class="nf-proc-dot"></span>
            <span class="nf-proc-label">${e.label}</span>
            <span class="nf-proc-badge">(queued)</span>
        `,t.appendChild(o)})}function Ce(){const t=document.getElementById("nf-terminal");if(!t)return;Me(t);const e=document.getElementById("nf-step-counter");e&&(e.textContent=`0/${w.length}`)}function ce(t,e){let i="";for(let b=0;b<20;b++){const k=b/20*Math.PI*2,u=(b+.2)/20*Math.PI*2,c=(b+.5)/20*Math.PI*2,n=(b+.8)/20*Math.PI*2,f=(b+1)/20*Math.PI*2;i+=`${b===0?"M":"L"}${(120+100*Math.cos(k)).toFixed(1)},${(120+100*Math.sin(k)).toFixed(1)} `,i+=`L${(120+100*Math.cos(u)).toFixed(1)},${(120+100*Math.sin(u)).toFixed(1)} `,i+=`L${(120+112*Math.cos(c)).toFixed(1)},${(120+112*Math.sin(c)).toFixed(1)} `,i+=`L${(120+100*Math.cos(n)).toFixed(1)},${(120+100*Math.sin(n)).toFixed(1)} `,i+=`L${(120+100*Math.cos(f)).toFixed(1)},${(120+100*Math.sin(f)).toFixed(1)} `}i+="Z";const p=14,s=72,x=62;let d="";for(let b=0;b<p;b++){const k=b/p*Math.PI*2,u=(b+.25)/p*Math.PI*2,c=(b+.75)/p*Math.PI*2,n=(b+1)/p*Math.PI*2;d+=`${b===0?"M":"L"}${(120+x*Math.cos(k)).toFixed(1)},${(120+x*Math.sin(k)).toFixed(1)} `,d+=`L${(120+s*Math.cos(u)).toFixed(1)},${(120+s*Math.sin(u)).toFixed(1)} `,d+=`L${(120+s*Math.cos(c)).toFixed(1)},${(120+s*Math.sin(c)).toFixed(1)} `,d+=`L${(120+x*Math.cos(n)).toFixed(1)},${(120+x*Math.sin(n)).toFixed(1)} `}return d+="Z",`<svg width="72" height="72" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
    ...
    </svg>`}function Re(){const t=document.createElement("div");t.id="netflow-engine-overlay",L=document.createElement("canvas"),L.id="nf-matrix-canvas",t.appendChild(L);const e=document.createElement("div");e.className="nf-pat-plasma",t.appendChild(e);for(let h=1;h<=5;h++){const m=document.createElement("div");m.className=`nf-ambient-orb nf-orb-${h}`,t.appendChild(m)}const a=document.createElement("div");a.className="nf-pat-data",t.appendChild(a);const o=document.createElement("div");o.className="nf-pat-diag-a",t.appendChild(o);const r=document.createElement("div");r.className="nf-pat-diag-b",t.appendChild(r);const l=document.createElement("div");l.className="nf-pat-circuit",t.appendChild(l);const g=document.createElement("div");g.className="nf-pat-honeycomb",t.appendChild(g);const i=document.createElement("div");i.className="nf-pat-binary",t.appendChild(i);const p=document.createElement("div");p.className="nf-pat-crosshatch",t.appendChild(p);const s=document.createElement("div");s.className="nf-pat-diamond",t.appendChild(s);const x=document.createElement("div");x.className="nf-pat-wave-h",t.appendChild(x);const d=document.createElement("div");d.className="nf-pat-radar",t.appendChild(d);const b=document.createElement("div");b.className="nf-pat-ripple-1",t.appendChild(b);const k=document.createElement("div");k.className="nf-pat-ripple-2",t.appendChild(k);const u=document.createElement("div");u.className="nf-pat-techscan",t.appendChild(u);const c=document.createElement("div");c.className="nf-center-glow",t.appendChild(c);const n=document.createElement("div");n.className="nf-pat-noise",t.appendChild(n);const f=document.createElement("div");f.className="nf-crt-scanlines",t.appendChild(f);const y=document.createElement("div");y.className="nf-vignette",t.appendChild(y);for(let h=0;h<3;h++){const m=document.createElement("div");m.className="nf-pulse-ring",t.appendChild(m)}["nf-deco-tl","nf-deco-tr","nf-deco-bl","nf-deco-br"].forEach(h=>{const m=document.createElement("div");m.className=`nf-corner-deco ${h}`,t.appendChild(m)});const P=document.createElement("button");P.className="nf-stop-btn",P.innerHTML='<span class="nf-stop-icon"></span> หยุด',P.style.cssText="position:absolute !important;top:14px !important;right:110px !important;z-index:2147483646 !important;cursor:pointer !important;pointer-events:auto !important;background:rgba(255,60,60,0.08) !important;border:1px solid rgba(255,60,60,0.25) !important;border-radius:8px !important;color:rgba(255,100,100,0.8) !important;font-size:13px !important;padding:6px 14px !important;font-family:inherit !important;display:flex !important;align-items:center !important;gap:6px !important;",P.onclick=()=>{var h;window.__NETFLOW_STOP__=!0;try{R("⛔ ผู้ใช้หยุดการทำงาน")}catch{}try{typeof chrome<"u"&&((h=chrome.runtime)!=null&&h.sendMessage)&&chrome.runtime.sendMessage({action:"AUTOMATION_STOPPED"})}catch{}},t.appendChild(P);const B=document.createElement("div");B.className="nf-layout";const I=document.createElement("div");I.className="nf-core-monitor",I.id="nf-core-monitor";const V=document.createElement("div");V.className="nf-core-header",V.innerHTML=`
    ...
    `,I.appendChild(V);const U=document.createElement("div");U.className="nf-terminal",U.id="nf-terminal",Me(U),I.appendChild(U);const _=document.createElement("div");_.className="nf-engine-core",_.id="nf-engine-core";const Q=document.createElement("div");Q.className="nf-engine-frame",["nf-fc-tl","nf-fc-tr","nf-fc-bl","nf-fc-br"].forEach(h=>{const m=document.createElement("div");m.className=`nf-frame-corner ${h}`,Q.appendChild(m)}),_.appendChild(Q);const le="http://www.w3.org/2000/svg",O=document.createElementNS(le,"svg");O.setAttribute("class","nf-engine-waves"),O.setAttribute("viewBox","0 0 560 140"),O.setAttribute("preserveAspectRatio","none"),O.id="nf-engine-waves";for(let h=0;h<4;h++){const m=document.createElementNS(le,"path");m.setAttribute("fill","none"),m.setAttribute("stroke-width",h<2?"1.5":"1"),m.setAttribute("stroke",h<2?`rgba(${v.rgb},${.14+h*.1})`:`rgba(${v.accentRgb},${.1+(h-2)*.08})`),m.setAttribute("data-wave-idx",String(h)),O.appendChild(m)}_.appendChild(O);const ee=document.createElement("div");ee.className="nf-engine-brand-inner",ee.innerHTML=`
        <div class="nf-brand-gear-icon nf-brand-gear-left">
            ${ce(v.rgb,v.accentRgb)}
        </div>
        <div class="nf-brand-inner-text">NETFLOW AI ENGINE V1.0</div>
        <div class="nf-brand-gear-icon nf-brand-gear-right">
            ${ce(v.rgb,v.accentRgb)}
        </div>
    `,_.appendChild(ee);const Y=document.createElement("div");Y.className="nf-engine-stats",Y.id="nf-engine-stats",Y.innerHTML=[["SCENES","nf-stat-scenes","1/1"],["ELAPSED","nf-stat-elapsed","00:00"],["STEP","nf-stat-step","0/0"],["STATUS","nf-stat-status","READY"],["PROGRESS","nf-stat-progress","—"]].map(([h,m,H])=>`<div class="nf-stat-item"><span class="nf-stat-label">${h}</span><span class="nf-stat-val" id="${m}">${H}</span></div>`).join(""),_.appendChild(Y),I.appendChild(_),B.appendChild(I);const Pe=["nf-mod-tl","nf-mod-tr","nf-mod-bl","nf-mod-br"];z.forEach((h,m)=>{const H=Be(h);H.classList.add(Pe[m]),H.id=`nf-mod-${h.id}`,B.appendChild(H)}),t.appendChild(B);for(let h=0;h<30;h++){const m=document.createElement("div");m.className="nf-particle",m.style.left=`${5+Math.random()*90}%`,m.style.bottom=`${Math.random()*40}%`,m.style.animationDuration=`${3+Math.random()*5}s`,m.style.animationDelay=`${Math.random()*4}s`;const H=.3+Math.random()*.4,te=.7+Math.random()*.3;m.style.background=`rgba(${Math.floor(C*te)}, ${Math.floor(E*te)}, ${Math.floor(S*te)}, ${H})`,m.style.width=`${1+Math.random()*2}px`,m.style.height=m.style.width,t.appendChild(m)}return t}function Be(t){const e=document.createElement("div");e.className="nf-module";const a=document.createElement("div");a.className="nf-mod-header",a.innerHTML=`
    ...
    `,e.appendChild(a),t.steps.forEach(r=>{const l=document.createElement("div");l.className="nf-step",l.id=`nf-step-${r.id}`;let g="";r.progress!==void 0&&(g=`
            ...
            ${g}
        `,e.appendChild(l)});const o=document.createElement("div");return o.className="nf-mod-progress",o.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${t.id}"></div>`,e.appendChild(o),e}function _e(){de=Date.now(),setInterval(()=>{const t=Math.floor((Date.now()-de)/1e3),e=String(Math.floor(t/60)).padStart(2,"0"),a=String(t%60).padStart(2,"0"),o=document.getElementById("nf-timer");o&&(o.textContent=`${e}:${a}`);const r=document.getElementById("nf-stat-elapsed");r&&(r.textContent=`${e}:${a}`)},1e3)}const He=120,fe=160,ge=.4;let G=null,xe=0,me=0,be=0,J=[];function Oe(t,e){J=[];for(let a=0;a<He;a++){const o=Math.random();let r;o<.22?r=0:o<.4?r=1:o<.55?r=2:o<.68?r=3:o<.84?r=4:r=5;const l=Math.random()*t,g=Math.random()*e,i=50+Math.random()*220,p=Math.random()*Math.PI*2,s=(.003+Math.random()*.008)*(Math.random()>.5?1:-1);J.push({x:r===0?Math.random()*t:l+Math.cos(p)*i,y:r===0?Math.random()*e:g+Math.sin(p)*i,vx:(Math.random()-.5)*ge,vy:(Math.random()-.5)*ge,radius:1.2+Math.random()*2.5,pulsePhase:Math.random()*Math.PI*2,pulseSpeed:.02+Math.random()*.03,motion:r,oCx:l,oCy:g,oRadius:i,oAngle:p,oSpeed:s})}}function Ge(){if(!L)return;const t=L;if(W=t.getContext("2d"),!W)return;const e=()=>{t.width=window.innerWidth,t.height=window.innerHeight,J.length===0&&Oe(t.width,t.height)};e(),window.addEventListener("resize",e);let a=null,o=0,r=0,l=!1;function g(){if(!W||!L||(requestAnimationFrame(g),l=!l,l))return;const i=W,p=L.width,s=L.height;i.fillStyle=`rgba(${C*.04|0},${E*.04|0},${S*.06|0},1)`,i.fillRect(0,0,p,s),(!a||o!==p||r!==s)&&(o=p,r=s,a=i.createRadialGradient(p*.5,s*.5,0,p*.5,s*.5,Math.max(p,s)*.6),a.addColorStop(0,`rgba(${C*.08|0},${E*.08|0},${S*.1|0},0.4)`),a.addColorStop(1,"rgba(0,0,0,0)")),i.fillStyle=a,i.fillRect(0,0,p,s);const x=J,d=x.length,b=fe*fe;for(let c=0;c<d;c++){const n=x[c];if(n.pulsePhase+=n.pulseSpeed,n.motion===0)n.x+=n.vx,n.y+=n.vy,n.x<0?(n.x=0,n.vx=Math.abs(n.vx)*(.8+Math.random()*.4)):n.x>p&&(n.x=p,n.vx=-Math.abs(n.vx)*(.8+Math.random()*.4)),n.y<0?(n.y=0,n.vy=Math.abs(n.vy)*(.8+Math.random()*.4)):n.y>s&&(n.y=s,n.vy=-Math.abs(n.vy)*(.8+Math.random()*.4));else if(n.motion===1)n.oAngle+=n.oSpeed,n.x=n.oCx+Math.cos(n.oAngle)*n.oRadius,n.y=n.oCy+Math.sin(n.oAngle)*n.oRadius,n.oCx+=Math.sin(n.oAngle*.3)*.15,n.oCy+=Math.cos(n.oAngle*.3)*.15;else if(n.motion===2)n.oAngle+=n.oSpeed,n.x=n.oCx+Math.cos(n.oAngle)*n.oRadius,n.y=n.oCy+Math.sin(n.oAngle)*n.oRadius*.5,n.oCx+=Math.sin(n.oAngle*.2)*.1,n.oCy+=Math.cos(n.oAngle*.2)*.1;else if(n.motion===3){n.oAngle+=n.oSpeed;const f=n.oAngle,y=n.oRadius*.7;n.x=n.oCx+y*Math.cos(f),n.y=n.oCy+y*Math.sin(f)*Math.cos(f),n.oCx+=Math.sin(f*.15)*.12,n.oCy+=Math.cos(f*.15)*.12}else if(n.motion===4){n.oAngle+=n.oSpeed*1.2;const f=n.oRadius*(.5+.5*Math.abs(Math.sin(n.oAngle*.15)));n.x=n.oCx+Math.cos(n.oAngle)*f,n.y=n.oCy+Math.sin(n.oAngle)*f,n.oCx+=Math.sin(n.oAngle*.1)*.18,n.oCy+=Math.cos(n.oAngle*.1)*.18}else n.oAngle+=n.oSpeed,n.x+=n.vx*.8,n.y=n.oCy+Math.sin(n.oAngle+n.x*.008)*n.oRadius*.35,n.x<-30?n.x=p+30:n.x>p+30&&(n.x=-30),n.oCy+=Math.sin(n.oAngle*.1)*.08;if(n.motion>0){const f=n.oRadius+50;n.oCx<-f?n.oCx=p+f:n.oCx>p+f&&(n.oCx=-f),n.oCy<-f?n.oCy=s+f:n.oCy>s+f&&(n.oCy=-f)}}i.beginPath(),i.strokeStyle=`rgba(${C},${E},${S},0.06)`,i.lineWidth=.4;const k=new Path2D;for(let c=0;c<d;c++){const n=x[c];for(let f=c+1;f<d;f++){const y=x[f],P=n.x-y.x,B=n.y-y.y,I=P*P+B*B;I<b&&(1-I/b<.4?(i.moveTo(n.x,n.y),i.lineTo(y.x,y.y)):(k.moveTo(n.x,n.y),k.lineTo(y.x,y.y)))}}if(i.stroke(),i.strokeStyle=`rgba(${C},${E},${S},0.18)`,i.lineWidth=.8,i.stroke(k),!G||xe!==C||me!==E||be!==S){G=document.createElement("canvas");const c=48;G.width=c,G.height=c;const n=G.getContext("2d"),f=n.createRadialGradient(c/2,c/2,0,c/2,c/2,c/2);f.addColorStop(0,`rgba(${C},${E},${S},0.9)`),f.addColorStop(.3,`rgba(${C},${E},${S},0.35)`),f.addColorStop(1,`rgba(${C},${E},${S},0)`),n.fillStyle=f,n.fillRect(0,0,c,c),xe=C,me=E,be=S}const u=G;for(let c=0;c<d;c++){const n=x[c],f=.6+.4*Math.sin(n.pulsePhase),y=n.radius*5*(.8+f*.4);i.globalAlpha=.5+f*.4,i.drawImage(u,n.x-y/2,n.y-y/2,y,y)}i.globalAlpha=1,i.fillStyle="rgba(255,255,255,0.45)",i.beginPath();for(let c=0;c<d;c++){const n=x[c];if(n.radius>2){const f=.6+.4*Math.sin(n.pulsePhase),y=n.radius*(.8+f*.4)*.35;i.moveTo(n.x+y,n.y),i.arc(n.x,n.y,y,0,Math.PI*2)}}i.fill()}g()}let q=null;const Z=560,Fe=140,he=Z/2,ue=Fe/2,Ee=[];for(let t=0;t<=Z;t+=8){const e=Math.abs(t-he)/he;Ee.push(Math.pow(Math.min(1,e*1.6),.6))}const De=[0,1,2,3].map(t=>({amp:10+t*5,freq:(1.2+t*.35)*Math.PI*2/Z,off:t*.6,spd:.7+t*.12}));let ne=!1;function Se(){if(requestAnimationFrame(Se),ne=!ne,ne)return;if(ae+=.07,!q){const e=document.getElementById("nf-engine-waves");if(!e)return;q=Array.from(e.querySelectorAll("path"))}const t=[];for(let e=0;e<q.length;e++){const a=De[e],o=ae*a.spd+a.off;t.length=0,t.push(`M 0 ${ue}`);let r=0;for(let l=0;l<=Z;l+=8){const g=ue+a.amp*Ee[r++]*Math.sin(l*a.freq+o);t.push(`L${l} ${g*10+.5|0}`)}q[e].setAttribute("d",t.join(" "))}}function je(){ae=0,Se(),Ge(),setInterval(()=>{const t=document.getElementById("nf-stat-freq"),e=document.getElementById("nf-stat-lat1"),a=document.getElementById("nf-stat-lat2"),o=document.getElementById("nf-stat-buf");t&&(t.textContent=`${(4.5+Math.random()*.5).toFixed(1)} GHz`),e&&(e.textContent=`${Math.floor(12+Math.random()*10)}ms`),a&&(a.textContent=`${Math.floor(12+Math.random()*10)}ms`),o&&(o.textContent=`${Math.floor(90+Math.random()*9)}%`)},2e3)}function oe(){let t=0;const e=w.filter(s=>s.status!=="skipped").length;for(const s of w){const x=document.getElementById(`nf-proc-${s.stepId}`);if(!x)continue;x.className="nf-proc-row";const d=x.querySelector(".nf-proc-badge");switch(s.status){case"done":x.classList.add("nf-proc-done"),d&&(d.textContent="✅ done"),t++;break;case"active":x.classList.add("nf-proc-active"),d&&(d.textContent=s.progress!==void 0&&s.progress>0?`⏳ ${s.progress}%`:"⏳ active");break;case"error":x.classList.add("nf-proc-error"),d&&(d.textContent="❌ error");break;case"skipped":x.classList.add("nf-proc-skipped"),d&&(d.textContent="— skip");break;default:x.classList.add("nf-proc-waiting"),d&&(d.textContent="(queued)")}}const a=w.findIndex(s=>s.status==="active"),o=a>=0?a+1:t>=e&&e>0?w.length:t,r=document.getElementById("nf-step-counter");r&&(r.textContent=`${o}/${w.length}`);const l=document.querySelector(".nf-core-title-val"),g=document.querySelector(".nf-status-dot");t>=e&&e>0?(l&&(l.textContent="COMPLETE",l.style.color=v.doneHex),g&&(g.style.background=v.doneHex,g.style.boxShadow=`0 0 8px rgba(${v.doneRgb},0.7)`)):w.some(x=>x.status==="error")?(l&&(l.textContent="ERROR",l.style.color="#f87171"),g&&(g.style.background="#ef4444",g.style.boxShadow="0 0 8px rgba(239,68,68,0.7)")):w.some(x=>x.status==="active")&&l&&(l.textContent="ACTIVE",l.style.color=v.hex,l.style.textShadow=`0 0 10px rgba(${v.rgb},0.5)`);const i=document.getElementById("nf-terminal"),p=i==null?void 0:i.querySelector(".nf-proc-active");p&&i&&p.scrollIntoView({behavior:"smooth",block:"center"})}function ze(){M&&M.isConnected||(ke(),M=document.createElement("button"),M.id="nf-toggle-btn",M.className="nf-toggle-visible",M.innerHTML=F?$e:ve,M.title="ซ่อน/แสดง Netflow Overlay",M.style.cssText="position:fixed !important;top:20px !important;right:20px !important;z-index:2147483647 !important;width:48px !important;height:48px !important;border-radius:50% !important;border:2px solid rgba(255,255,255,0.5) !important;background:rgba(0,0,0,0.85) !important;color:#fff !important;font-size:23px !important;cursor:pointer !important;display:flex !important;align-items:center !important;justify-content:center !important;pointer-events:auto !important;",M.onclick=()=>Ie(),document.body.appendChild(M))}function Ie(){$&&(ze(),F?($.classList.remove("nf-hidden"),$.classList.add("nf-visible"),$.style.opacity="1",$.style.pointerEvents="auto",M&&(M.innerHTML=ve),F=!1):($.classList.remove("nf-visible"),$.classList.add("nf-hidden"),$.style.opacity="0",$.style.pointerEvents="none",M&&(M.innerHTML=$e),F=!0))}const we={red:"themes/theme-red.jpg",yellow:"themes/theme-yellow.jpg",blue:"themes/theme-blue.jpg",purple:"themes/theme-purple.jpg",green:"themes/theme-green.jpg"};function Te(){const t=document.getElementById("nf-core-monitor");if(!t)return;let e=j;if(!e)try{e=localStorage.getItem("netflow_app_theme")||"green"}catch{e="green"}const a=we[e]||we.green;let o;try{o=chrome.runtime.getURL(a)}catch{o=`/${a}`}const r=v.rgb;t.style.backgroundImage=["linear-gradient(180deg, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.78) 45%, rgba(0,0,0,0.90) 100%)",`linear-gradient(180deg, rgba(${r},0.25) 0%, rgba(${r},0.12) 50%, rgba(${r},0.20) 100%)`,`url('${o}')`].join(", "),t.style.backgroundSize="auto, auto, 50%",t.style.backgroundPosition="center, center, center",t.style.backgroundRepeat="no-repeat, no-repeat, no-repeat",t.style.setProperty("--nf-bg-set","1"),t.style.border=`1.5px solid rgba(${r},0.45)`,t.style.boxShadow=`0 0 70px rgba(${r},0.22), 0 0 140px rgba(${r},0.1), inset 0 1px 0 rgba(${r},0.15)`}function Ve(t=1){if(v=Ae(),ye(),$&&$.isConnected){for(const e of z)for(const a of e.steps)a.status="waiting",a.progress=a.progress!==void 0?0:void 0;D=t,w=K(t),Ce();for(const e of z)re(e);ie(),oe(),F&&Ie();return}$&&!$.isConnected&&($=null),A&&(A.remove(),A=null),ke();for(const e of z)for(const a of e.steps)a.status="waiting",a.progress=a.progress!==void 0?0:void 0;if(D=t,w=K(t),t>1){const e=z.find(o=>o.id==="video");if(e){const o=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:"waiting",progress:0}];for(let r=2;r<=t;r++)o.push({id:`scene${r}-prompt`,label:`Scene ${r} Prompt`,status:"waiting"}),o.push({id:`scene${r}-gen`,label:`Scene ${r} Generate`,status:"waiting"}),o.push({id:`scene${r}-wait`,label:`Scene ${r} รอผล`,status:"waiting",progress:0});e.steps=o}const a=z.find(o=>o.id==="render");if(a){const o=a.steps.find(l=>l.id==="download");o&&(o.label="ดาวน์โหลด 720p");const r=a.steps.find(l=>l.id==="upscale");r&&(r.label="Full Video")}}$=Re(),$.style.cssText="position:fixed !important;inset:0 !important;z-index:2147483647 !important;pointer-events:auto !important;opacity:1 !important;",document.body.appendChild($),$.classList.add("nf-visible"),F=!1,ze(),_e(),je(),requestAnimationFrame(()=>Te())}const Ue={settings:"SETTINGS","upload-char":"UPLOAD","upload-prod":"UPLOAD","img-prompt":"PROMPT","img-generate":"GENERATE","img-wait":"IMG WAIT",animate:"ANIMATE","vid-prompt":"VID PROMPT","vid-generate":"VID GEN","vid-wait":"VID WAIT",download:"DOWNLOAD",upscale:"UPSCALE",open:"OPENING"};function Ye(t,e,a){const o=w.findIndex(d=>d.status==="active"),r=w.filter(d=>d.status==="done").length,l=w.length,g=o>=0?o+1:r>=l?l:r,i=document.getElementById("nf-stat-step");i&&(i.textContent=`${g}/${l}`);let p=1;for(const d of w)if(d.status==="active"||d.status==="done")if(d.stepId.startsWith("scene")){const b=d.stepId.match(/^scene(\d+)-/);b&&(p=Math.max(p,parseInt(b[1],10)))}else(d.stepId==="download"||d.stepId==="upscale"||d.stepId==="open")&&(p=D);const s=document.getElementById("nf-stat-scenes");if(s&&(s.textContent=D>1?`${p}/${D}`:"1/1"),e==="active"){const d=document.getElementById("nf-stat-status"),b=Ue[t]||t.toUpperCase();d&&(d.textContent=b)}else if(e==="done"&&r>=l){const d=document.getElementById("nf-stat-status");d&&(d.textContent="COMPLETE")}else if(e==="error"){const d=document.getElementById("nf-stat-status");d&&(d.textContent="ERROR")}const x=document.getElementById("nf-stat-progress");x&&e==="active"&&(x.textContent="—")}function T(t,e,a){if(!$)return;for(const r of z)for(const l of r.steps)l.id===t&&(l.status=e);for(const r of w)r.stepId===t&&(r.status=e);const o=document.getElementById(`nf-step-${t}`);o&&(o.className="nf-step",e==="active"?o.classList.add("nf-step-active"):e==="done"?o.classList.add("nf-step-done"):e==="error"&&o.classList.add("nf-step-error")),Ye(t,e),ie(),oe()}function ie(){for(const t of z){const e=t.steps.filter(p=>p.status!=="skipped").length,a=t.steps.filter(p=>p.status==="done").length,o=t.steps.some(p=>p.status==="active"),r=e>0?Math.round(a/e*100):0,l=document.getElementById(`nf-pct-${t.id}`);l&&(l.textContent=`${r}%`);const g=document.getElementById(`nf-modbar-${t.id}`);g&&(g.style.width=`${r}%`);const i=document.getElementById(`nf-mod-${t.id}`);i&&(i.classList.remove("nf-active","nf-done"),r>=100?i.classList.add("nf-done"):o&&i.classList.add("nf-active"))}}function We(t){var o,r,l,g;D=t;const e=new Map;for(const i of w)e.set(i.stepId,{status:i.status,progress:i.progress});w=K(t);for(const i of w){const p=e.get(i.stepId);p&&(i.status=p.status,p.progress!==void 0&&(i.progress=p.progress))}Ce();{const i=z.find(p=>p.id==="video");if(i){const p=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:((o=i.steps.find(s=>s.id==="animate"))==null?void 0:o.status)||"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:((r=i.steps.find(s=>s.id==="vid-prompt"))==null?void 0:r.status)||"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:((l=i.steps.find(s=>s.id==="vid-generate"))==null?void 0:l.status)||"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:((g=i.steps.find(s=>s.id==="vid-wait"))==null?void 0:g.status)||"waiting",progress:0}];for(let s=2;s<=t;s++)p.push({id:`scene${s}-prompt`,label:`Scene ${s} Prompt`,status:"waiting"}),p.push({id:`scene${s}-gen`,label:`Scene ${s} Generate`,status:"waiting"}),p.push({id:`scene${s}-wait`,label:`Scene ${s} รอผล`,status:"waiting",progress:0});i.steps=p,re(i)}}const a=z.find(i=>i.id==="render");if(a&&t>1){const i=a.steps.find(s=>s.id==="download");i&&(i.label="ดาวน์โหลด 720p");const p=a.steps.find(s=>s.id==="upscale");p&&(p.label="Full Video"),re(a)}ie(),oe()}function re(t){const e=document.getElementById(`nf-mod-${t.id}`);if(!e)return;e.querySelectorAll(".nf-step, .nf-mod-progress").forEach(r=>r.remove()),t.steps.forEach(r=>{const l=document.createElement("div");l.className="nf-step",l.id=`nf-step-${r.id}`;let g="";r.progress!==void 0&&(g=`
                <div class="nf-progress-bar">
                    <div class="nf-progress-fill" id="nf-bar-${r.id}" style="width: 0%"></div>
                </div>
            `),l.innerHTML=`
            ...
            <span class="nf-step-label">${r.label}</span>
            ${g}
        `,e.appendChild(l)});const o=document.createElement("div");o.className="nf-mod-progress",o.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${t.id}"></div>`,e.appendChild(o)}function R(t){t.replace(/^\[Netflow AI\]\s*/,"")}window.chrome={runtime:{sendMessage:()=>{},lastError:null},storage:{local:{get:(t,e)=>e==null?void 0:e({}),set:()=>{}}}};const se=new URLSearchParams(location.search).get("theme")||"red";Ne(se);We(2);Ve();setTimeout(()=>{R("🚀 Overlay preview started — theme: "+se),T("settings","active")},400);setTimeout(()=>{T("settings","done"),T("upload","active"),R("✅ Settings configured")},1600);setTimeout(()=>{T("upload","done"),T("prompt","active"),R("✅ Images uploaded (2 refs)")},3e3);setTimeout(()=>{T("prompt","done"),T("generate","active"),R("✅ Prompt injected"),R("⏳ Waiting for Scene 1 generation...")},4500);setTimeout(()=>{T("generate","done"),T("scene2","active"),R("✅ Scene 1 generated"),R("⏳ Generating Scene 2...")},7e3);const pe=document.createElement("div");pe.style.cssText=`
...
`;const qe=["red","blue","green","yellow","purple"],X={red:"#ef4444",blue:"#3b82f6",green:"#00ff41",yellow:"#eab308",purple:"#8b5cf6"};qe.forEach(t=>{const e=document.createElement("button");e.textContent=t,e.style.cssText=`
        ...
        ${t===se?"opacity:1; box-shadow: 0 0 8px "+X[t]:"opacity:0.5"};
    `,e.onclick=()=>location.search="?theme="+t,pe.appendChild(e)});document.body.appendChild(pe);

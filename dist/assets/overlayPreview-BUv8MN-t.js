...
@media (max-height: 600px) {
    ...
    `,B.appendChild(Z);const U=document.createElement("div");U.className="nf-terminal",U.id="nf-terminal",Ce(U),B.appendChild(U);const _=document.createElement("div");_.className="nf-engine-core",_.id="nf-engine-core";const Q=document.createElement("div");Q.className="nf-engine-frame",["nf-fc-tl","nf-fc-tr","nf-fc-bl","nf-fc-br"].forEach(b=>{const x=document.createElement("div");x.className=`nf-frame-corner ${b}`,Q.appendChild(x)}),_.appendChild(Q);const pe="http://www.w3.org/2000/svg",F=document.createElementNS(pe,"svg");F.setAttribute("class","nf-engine-waves"),F.setAttribute("viewBox","0 0 560 140"),F.setAttribute("preserveAspectRatio","none"),F.id="nf-engine-waves";for(let b=0;b<4;b++){const x=document.createElementNS(pe,"path");x.setAttribute("fill","none"),x.setAttribute("stroke-width",b<2?"1.5":"1"),x.setAttribute("stroke",b<2?`rgba(${$.rgb},${.14+b*.1})`:`rgba(${$.accentRgb},${.1+(b-2)*.08})`),x.setAttribute("data-wave-idx",String(b)),F.appendChild(x)}_.appendChild(F);const ee=document.createElement("div");ee.className="nf-engine-brand-inner",ee.innerHTML=`
    ...
    `,_.appendChild(ee);const Y=document.createElement("div");Y.className="nf-engine-stats",Y.id="nf-engine-stats",Y.innerHTML=[["SCENES","nf-stat-scenes","1/1"],["ELAPSED","nf-stat-elapsed","00:00"],["STEP","nf-stat-step","0/0"],["STATUS","nf-stat-status","READY"],["PROGRESS","nf-stat-progress","—"]].map(([b,x,H])=>`<div class="nf-stat-item"><span class="nf-stat-label">${b}</span><span class="nf-stat-val" id="${x}">${H}</span></div>`).join(""),_.appendChild(Y),B.appendChild(_),R.appendChild(B);const Pe=["nf-mod-tl","nf-mod-tr","nf-mod-bl","nf-mod-br"];O.forEach((b,x)=>{const H=_e(b);H.classList.add(Pe[x]),H.id=`nf-mod-${b.id}`,R.appendChild(H)}),t.appendChild(R);for(let b=0;b<30;b++){const x=document.createElement("div");x.className="nf-particle",x.style.left=`${5+Math.random()*90}%`,x.style.bottom=`${Math.random()*40}%`,x.style.animationDuration=`${3+Math.random()*5}s`,x.style.animationDelay=`${Math.random()*4}s`;const H=.3+Math.random()*.4,te=.7+Math.random()*.3;x.style.background=`rgba(${Math.floor(M*te)}, ${Math.floor(E*te)}, ${Math.floor(S*te)}, ${H})`,x.style.width=`${1+Math.random()*2}px`,x.style.height=x.style.width,t.appendChild(x)}return t}function _e(t){const e=document.createElement("div");e.className="nf-module";const a=document.createElement("div");a.className="nf-mod-header",a.innerHTML=`
    ...
    `,e.appendChild(a),t.steps.forEach(r=>{const l=document.createElement("div");l.className="nf-step",l.id=`nf-step-${r.id}`;let g="";r.progress!==void 0&&(g=`
            ...
            ${g}
        `,e.appendChild(l)});const o=document.createElement("div");return o.className="nf-mod-progress",o.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${t.id}"></div>`,e.appendChild(o),e}function He(){le=Date.now(),setInterval(()=>{const t=Math.floor((Date.now()-le)/1e3),e=String(Math.floor(t/60)).padStart(2,"0"),a=String(t%60).padStart(2,"0"),o=document.getElementById("nf-timer");o&&(o.textContent=`${e}:${a}`);const r=document.getElementById("nf-stat-elapsed");r&&(r.textContent=`${e}:${a}`)},1e3)}const Oe=120,ce=160,fe=.4;let G=null,ge=0,xe=0,me=0,K=[];function Fe(t,e){K=[];for(let a=0;a<Oe;a++){const o=Math.random();let r;o<.22?r=0:o<.4?r=1:o<.55?r=2:o<.68?r=3:o<.84?r=4:r=5;const l=Math.random()*t,g=Math.random()*e,i=50+Math.random()*220,p=Math.random()*Math.PI*2,s=(.003+Math.random()*.008)*(Math.random()>.5?1:-1);K.push({x:r===0?Math.random()*t:l+Math.cos(p)*i,y:r===0?Math.random()*e:g+Math.sin(p)*i,vx:(Math.random()-.5)*fe,vy:(Math.random()-.5)*fe,radius:1.2+Math.random()*2.5,pulsePhase:Math.random()*Math.PI*2,pulseSpeed:.02+Math.random()*.03,motion:r,oCx:l,oCy:g,oRadius:i,oAngle:p,oSpeed:s})}}function Ge(){if(!P)return;const t=P;if(W=t.getContext("2d"),!W)return;const e=()=>{t.width=window.innerWidth,t.height=window.innerHeight,K.length===0&&Fe(t.width,t.height)};e(),window.addEventListener("resize",e);let a=null,o=0,r=0,l=!1;function g(){if(!W||!P||(requestAnimationFrame(g),l=!l,l))return;const i=W,p=P.width,s=P.height;i.fillStyle=`rgba(${M*.04|0},${E*.04|0},${S*.06|0},1)`,i.fillRect(0,0,p,s),(!a||o!==p||r!==s)&&(o=p,r=s,a=i.createRadialGradient(p*.5,s*.5,0,p*.5,s*.5,Math.max(p,s)*.6),a.addColorStop(0,`rgba(${M*.08|0},${E*.08|0},${S*.1|0},0.4)`),a.addColorStop(1,"rgba(0,0,0,0)")),i.fillStyle=a,i.fillRect(0,0,p,s);const d=K,m=d.length,h=ce*ce;for(let c=0;c<m;c++){const n=d[c];if(n.pulsePhase+=n.pulseSpeed,n.motion===0)n.x+=n.vx,n.y+=n.vy,n.x<0?(n.x=0,n.vx=Math.abs(n.vx)*(.8+Math.random()*.4)):n.x>p&&(n.x=p,n.vx=-Math.abs(n.vx)*(.8+Math.random()*.4)),n.y<0?(n.y=0,n.vy=Math.abs(n.vy)*(.8+Math.random()*.4)):n.y>s&&(n.y=s,n.vy=-Math.abs(n.vy)*(.8+Math.random()*.4));else if(n.motion===1)n.oAngle+=n.oSpeed,n.x=n.oCx+Math.cos(n.oAngle)*n.oRadius,n.y=n.oCy+Math.sin(n.oAngle)*n.oRadius,n.oCx+=Math.sin(n.oAngle*.3)*.15,n.oCy+=Math.cos(n.oAngle*.3)*.15;else if(n.motion===2)n.oAngle+=n.oSpeed,n.x=n.oCx+Math.cos(n.oAngle)*n.oRadius,n.y=n.oCy+Math.sin(n.oAngle)*n.oRadius*.5,n.oCx+=Math.sin(n.oAngle*.2)*.1,n.oCy+=Math.cos(n.oAngle*.2)*.1;else if(n.motion===3){n.oAngle+=n.oSpeed;const f=n.oAngle,w=n.oRadius*.7;n.x=n.oCx+w*Math.cos(f),n.y=n.oCy+w*Math.sin(f)*Math.cos(f),n.oCx+=Math.sin(f*.15)*.12,n.oCy+=Math.cos(f*.15)*.12}else if(n.motion===4){n.oAngle+=n.oSpeed*1.2;const f=n.oRadius*(.5+.5*Math.abs(Math.sin(n.oAngle*.15)));n.x=n.oCx+Math.cos(n.oAngle)*f,n.y=n.oCy+Math.sin(n.oAngle)*f,n.oCx+=Math.sin(n.oAngle*.1)*.18,n.oCy+=Math.cos(n.oAngle*.1)*.18}else n.oAngle+=n.oSpeed,n.x+=n.vx*.8,n.y=n.oCy+Math.sin(n.oAngle+n.x*.008)*n.oRadius*.35,n.x<-30?n.x=p+30:n.x>p+30&&(n.x=-30),n.oCy+=Math.sin(n.oAngle*.1)*.08;if(n.motion>0){const f=n.oRadius+50;n.oCx<-f?n.oCx=p+f:n.oCx>p+f&&(n.oCx=-f),n.oCy<-f?n.oCy=s+f:n.oCy>s+f&&(n.oCy=-f)}}i.beginPath(),i.strokeStyle=`rgba(${M},${E},${S},0.06)`,i.lineWidth=.4;const v=new Path2D;for(let c=0;c<m;c++){const n=d[c];for(let f=c+1;f<m;f++){const w=d[f],A=n.x-w.x,L=n.y-w.y,R=A*A+L*L;R<h&&(1-R/h<.4?(i.moveTo(n.x,n.y),i.lineTo(w.x,w.y)):(v.moveTo(n.x,n.y),v.lineTo(w.x,w.y)))}}if(i.stroke(),i.strokeStyle=`rgba(${M},${E},${S},0.18)`,i.lineWidth=.8,i.stroke(v),!G||ge!==M||xe!==E||me!==S){G=document.createElement("canvas");const c=48;G.width=c,G.height=c;const n=G.getContext("2d"),f=n.createRadialGradient(c/2,c/2,0,c/2,c/2,c/2);f.addColorStop(0,`rgba(${M},${E},${S},0.9)`),f.addColorStop(.3,`rgba(${M},${E},${S},0.35)`),f.addColorStop(1,`rgba(${M},${E},${S},0)`),n.fillStyle=f,n.fillRect(0,0,c,c),ge=M,xe=E,me=S}const u=G;for(let c=0;c<m;c++){const n=d[c],f=.6+.4*Math.sin(n.pulsePhase),w=n.radius*5*(.8+f*.4);i.globalAlpha=.5+f*.4,i.drawImage(u,n.x-w/2,n.y-w/2,w,w)}i.globalAlpha=1,i.fillStyle="rgba(255,255,255,0.45)",i.beginPath();for(let c=0;c<m;c++){const n=d[c];if(n.radius>2){const f=.6+.4*Math.sin(n.pulsePhase),w=n.radius*(.8+f*.4)*.35;i.moveTo(n.x+w,n.y),i.arc(n.x,n.y,w,0,Math.PI*2)}}i.fill()}g()}let q=null;const J=560,De=140,be=J/2,he=De/2,Me=[];for(let t=0;t<=J;t+=8){const e=Math.abs(t-be)/be;Me.push(Math.pow(Math.min(1,e*1.6),.6))}const je=[0,1,2,3].map(t=>({amp:10+t*5,freq:(1.2+t*.35)*Math.PI*2/J,off:t*.6,spd:.7+t*.12}));let ne=!1;function Ee(){if(requestAnimationFrame(Ee),ne=!ne,ne)return;if(ae+=.07,!q){const e=document.getElementById("nf-engine-waves");if(!e)return;q=Array.from(e.querySelectorAll("path"))}const t=[];for(let e=0;e<q.length;e++){const a=je[e],o=ae*a.spd+a.off;t.length=0,t.push(`M 0 ${he}`);let r=0;for(let l=0;l<=J;l+=8){const g=he+a.amp*Me[r++]*Math.sin(l*a.freq+o);t.push(`L${l} ${g*10+.5|0}`)}q[e].setAttribute("d",t.join(" "))}}function Ve(){ae=0,Ee(),Ge(),setInterval(()=>{const t=document.getElementById("nf-stat-freq"),e=document.getElementById("nf-stat-lat1"),a=document.getElementById("nf-stat-lat2"),o=document.getElementById("nf-stat-buf");t&&(t.textContent=`${(4.5+Math.random()*.5).toFixed(1)} GHz`),e&&(e.textContent=`${Math.floor(12+Math.random()*10)}ms`),a&&(a.textContent=`${Math.floor(12+Math.random()*10)}ms`),o&&(o.textContent=`${Math.floor(90+Math.random()*9)}%`)},2e3)}function Se(){let t=0;const e=y.filter(s=>s.status!=="skipped").length;for(const s of y){const d=document.getElementById(`nf-proc-${s.stepId}`);if(!d)continue;d.className="nf-proc-row";const m=d.querySelector(".nf-proc-badge");switch(s.status){case"done":d.classList.add("nf-proc-done"),m&&(m.textContent="✅ done"),t++;break;case"active":d.classList.add("nf-proc-active"),m&&(m.textContent=s.progress!==void 0&&s.progress>0?`⏳ ${s.progress}%`:"⏳ active");break;case"error":d.classList.add("nf-proc-error"),m&&(m.textContent="❌ error");break;case"skipped":d.classList.add("nf-proc-skipped"),m&&(m.textContent="— skip");break;default:d.classList.add("nf-proc-waiting"),m&&(m.textContent="(queued)")}}const a=y.findIndex(s=>s.status==="active"),o=a>=0?a+1:t>=e&&e>0?y.length:t,r=document.getElementById("nf-step-counter");r&&(r.textContent=`${o}/${y.length}`);const l=document.querySelector(".nf-core-title-val"),g=document.querySelector(".nf-status-dot");t>=e&&e>0?(l&&(l.textContent="COMPLETE",l.style.color=$.doneHex),g&&(g.style.background=$.doneHex,g.style.boxShadow=`0 0 8px rgba(${$.doneRgb},0.7)`)):y.some(d=>d.status==="error")?(l&&(l.textContent="ERROR",l.style.color="#f87171"),g&&(g.style.background="#ef4444",g.style.boxShadow="0 0 8px rgba(239,68,68,0.7)")):y.some(d=>d.status==="active")&&l&&(l.textContent="ACTIVE",l.style.color=$.hex,l.style.textShadow=`0 0 10px rgba(${$.rgb},0.5)`);const i=document.getElementById("nf-terminal"),p=i==null?void 0:i.querySelector(".nf-proc-active");p&&i&&p.scrollIntoView({behavior:"smooth",block:"center"})}function ze(){k&&k.isConnected||(ke(),k=document.createElement("button"),k.id="nf-toggle-btn",k.className="nf-toggle-visible",k.innerHTML=D?$e:ve,k.title="ซ่อน/แสดง Netflow Overlay",k.onclick=()=>oe(),document.body.appendChild(k))}function oe(){C&&(ze(),D?(C.classList.remove("nf-hidden"),C.classList.add("nf-visible"),k&&(k.innerHTML=ve),D=!1):(C.classList.remove("nf-visible"),C.classList.add("nf-hidden"),k&&(k.innerHTML=$e),D=!0))}const ue={red:"themes/theme-red.jpg",yellow:"themes/theme-yellow.jpg",blue:"themes/theme-blue.jpg",purple:"themes/theme-purple.jpg",green:"themes/theme-green.jpg"};function Ie(){const t=document.getElementById("nf-core-monitor");if(!t)return;let e=j;if(!e)try{e=localStorage.getItem("netflow_app_theme")||"green"}catch{e="green"}const a=ue[e]||ue.green;let o;try{o=chrome.runtime.getURL(a)}catch{o=`/${a}`}const r=$.rgb;t.style.backgroundImage=["linear-gradient(180deg, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.78) 45%, rgba(0,0,0,0.90) 100%)",`linear-gradient(180deg, rgba(${r},0.25) 0%, rgba(${r},0.12) 50%, rgba(${r},0.20) 100%)`,`url('${o}')`].join(", "),t.style.backgroundSize="auto, auto, 50%",t.style.backgroundPosition="center, center, center",t.style.backgroundRepeat="no-repeat, no-repeat, no-repeat",t.style.setProperty("--nf-bg-set","1"),t.style.border=`1.5px solid rgba(${r},0.45)`,t.style.boxShadow=`0 0 70px rgba(${r},0.22), 0 0 140px rgba(${r},0.1), inset 0 1px 0 rgba(${r},0.15)`}function Ue(t=1){if($=Ae(),ye(),C&&C.isConnected){D&&oe();return}if(C&&!C.isConnected&&(C=null),T&&(T.remove(),T=null),ke(),V=t,y=re(t),t>1){const e=O.find(o=>o.id==="video");if(e){const o=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:"waiting",progress:0}];for(let r=2;r<=t;r++)o.push({id:`scene${r}-prompt`,label:`Scene ${r} Prompt`,status:"waiting"}),o.push({id:`scene${r}-gen`,label:`Scene ${r} Generate`,status:"waiting"}),o.push({id:`scene${r}-wait`,label:`Scene ${r} รอผล`,status:"waiting",progress:0});e.steps=o}const a=O.find(o=>o.id==="render");if(a){const o=a.steps.find(l=>l.id==="download");o&&(o.label="ดาวน์โหลด 720p");const r=a.steps.find(l=>l.id==="upscale");r&&(r.label="Full Video")}}C=Be(),document.body.appendChild(C),D=!1,ze(),He(),Ve(),requestAnimationFrame(()=>Ie())}const Ye={settings:"CONFIG","upload-char":"UPLOAD","upload-prod":"UPLOAD","img-prompt":"PROMPT","img-generate":"GENERATE","img-wait":"IMG WAIT",animate:"ANIMATE","vid-prompt":"VID PROMPT","vid-generate":"VID GEN","vid-wait":"VID WAIT",download:"DOWNLOAD",upscale:"UPSCALE",open:"OPENING"};function We(t,e,a){const o=y.findIndex(d=>d.status==="active"),r=y.filter(d=>d.status==="done").length,l=y.length,g=o>=0?o+1:r>=l?l:r,i=document.getElementById("nf-stat-step");i&&(i.textContent=`${g}/${l}`);let p=1;for(const d of y)if(d.status==="active"||d.status==="done")if(d.stepId.startsWith("scene")){const m=d.stepId.match(/^scene(\d+)-/);m&&(p=Math.max(p,parseInt(m[1],10)))}else(d.stepId==="download"||d.stepId==="upscale"||d.stepId==="open")&&(p=V);const s=document.getElementById("nf-stat-scenes");if(s&&(s.textContent=V>1?`${p}/${V}`:"1/1"),e==="active"){const d=document.getElementById("nf-stat-status"),m=Ye[t]||t.toUpperCase();d&&(d.textContent=m)}else if(e==="done"&&r>=l){const d=document.getElementById("nf-stat-status");d&&(d.textContent="COMPLETE")}else if(e==="error"){const d=document.getElementById("nf-stat-status");d&&(d.textContent="ERROR")}}function z(t,e,a){if(!C)return;for(const r of O)for(const l of r.steps)l.id===t&&(l.status=e);for(const r of y)r.stepId===t&&(r.status=e);const o=document.getElementById(`nf-step-${t}`);o&&(o.className="nf-step",e==="active"?o.classList.add("nf-step-active"):e==="done"?o.classList.add("nf-step-done"):e==="error"&&o.classList.add("nf-step-error")),We(t,e),Te(),Se()}function Te(){for(const t of O){const e=t.steps.filter(p=>p.status!=="skipped").length,a=t.steps.filter(p=>p.status==="done").length,o=t.steps.some(p=>p.status==="active"),r=e>0?Math.round(a/e*100):0,l=document.getElementById(`nf-pct-${t.id}`);l&&(l.textContent=`${r}%`);const g=document.getElementById(`nf-modbar-${t.id}`);g&&(g.style.width=`${r}%`);const i=document.getElementById(`nf-mod-${t.id}`);i&&(i.classList.remove("nf-active","nf-done"),r>=100?i.classList.add("nf-done"):o&&i.classList.add("nf-active"))}}function qe(t){var o,r,l,g;V=t;const e=new Map;for(const i of y)e.set(i.stepId,{status:i.status,progress:i.progress});y=re(t);for(const i of y){const p=e.get(i.stepId);p&&(i.status=p.status,p.progress!==void 0&&(i.progress=p.progress))}Re();{const i=O.find(p=>p.id==="video");if(i){const p=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:((o=i.steps.find(s=>s.id==="animate"))==null?void 0:o.status)||"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:((r=i.steps.find(s=>s.id==="vid-prompt"))==null?void 0:r.status)||"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:((l=i.steps.find(s=>s.id==="vid-generate"))==null?void 0:l.status)||"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:((g=i.steps.find(s=>s.id==="vid-wait"))==null?void 0:g.status)||"waiting",progress:0}];for(let s=2;s<=t;s++)p.push({id:`scene${s}-prompt`,label:`Scene ${s} Prompt`,status:"waiting"}),p.push({id:`scene${s}-gen`,label:`Scene ${s} Generate`,status:"waiting"}),p.push({id:`scene${s}-wait`,label:`Scene ${s} รอผล`,status:"waiting",progress:0});i.steps=p,we(i)}}const a=O.find(i=>i.id==="render");if(a&&t>1){const i=a.steps.find(s=>s.id==="download");i&&(i.label="ดาวน์โหลด 720p");const p=a.steps.find(s=>s.id==="upscale");p&&(p.label="Full Video"),we(a)}Te(),Se()}function we(t){const e=document.getElementById(`nf-mod-${t.id}`);if(!e)return;e.querySelectorAll(".nf-step, .nf-mod-progress").forEach(r=>r.remove()),t.steps.forEach(r=>{const l=document.createElement("div");l.className="nf-step",l.id=`nf-step-${r.id}`;let g="";r.progress!==void 0&&(g=`
                <div class="nf-progress-bar">
                    <div class="nf-progress-fill" id="nf-bar-${r.id}" style="width: 0%"></div>
                </div>
            `),l.innerHTML=`
...
`;const Xe=["red","blue","green","yellow","purple"],X={red:"#ef4444",blue:"#3b82f6",green:"#00ff41",yellow:"#eab308",purple:"#8b5cf6"};Xe.forEach(t=>{const e=document.createElement("button");e.textContent=t,e.style.cssText=`
        ...
        ${t===ie?"opacity:1; box-shadow: 0 0 8px "+X[t]:"opacity:0.5"};

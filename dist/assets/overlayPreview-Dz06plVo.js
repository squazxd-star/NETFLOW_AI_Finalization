...
.nf-proc-skipped {
    ...
    `}function pe(){B||(B=document.createElement("style"),B.id="netflow-overlay-styles",B.textContent=ye(S),document.head.appendChild(B))}function fe(n){n.innerHTML="",E.forEach((e,r)=>{const o=document.createElement("div");o.className="nf-proc-row nf-proc-waiting",o.id=`nf-proc-${e.stepId}`,o.innerHTML=`
            <span class="nf-proc-num">${r+1}</span>
            <span class="nf-proc-dot"></span>
            <span class="nf-proc-label">${e.label}</span>
            <span class="nf-proc-badge">(queued)</span>
        `,n.appendChild(o)})}function we(){const n=document.getElementById("nf-terminal");if(!n)return;fe(n);const e=document.getElementById("nf-step-counter");e&&(e.textContent=`0/${E.length}`)}function ve(n,e){let a="";for(let c=0;c<32;c++){const m=c/32*Math.PI*2,h=(c+.2)/32*Math.PI*2,u=(c+.5)/32*Math.PI*2,w=(c+.8)/32*Math.PI*2,v=(c+1)/32*Math.PI*2;a+=`${c===0?"M":"L"}${(120+104*Math.cos(m)).toFixed(1)},${(120+104*Math.sin(m)).toFixed(1)} `,a+=`L${(120+104*Math.cos(h)).toFixed(1)},${(120+104*Math.sin(h)).toFixed(1)} `,a+=`L${(120+116*Math.cos(u)).toFixed(1)},${(120+116*Math.sin(u)).toFixed(1)} `,a+=`L${(120+104*Math.cos(w)).toFixed(1)},${(120+104*Math.sin(w)).toFixed(1)} `,a+=`L${(120+104*Math.cos(v)).toFixed(1)},${(120+104*Math.sin(v)).toFixed(1)} `}a+="Z";const s=24,d=100,$=90;let M="";for(let c=0;c<s;c++){const m=c/s*Math.PI*2,h=(c+.25)/s*Math.PI*2,u=(c+.75)/s*Math.PI*2,w=(c+1)/s*Math.PI*2;M+=`${c===0?"M":"L"}${(120+$*Math.cos(m)).toFixed(1)},${(120+$*Math.sin(m)).toFixed(1)} `,M+=`L${(120+d*Math.cos(h)).toFixed(1)},${(120+d*Math.sin(h)).toFixed(1)} `,M+=`L${(120+d*Math.cos(u)).toFixed(1)},${(120+d*Math.sin(u)).toFixed(1)} `,M+=`L${(120+$*Math.cos(w)).toFixed(1)},${(120+$*Math.sin(w)).toFixed(1)} `}M+="Z";let C="";for(let c=0;c<64;c++){const m=c/64*Math.PI*2,h=c%4===0?117:119,u=c%4===0?124:122,w=c%4===0?.8:.4,v=c%4===0?.7:.35;C+=`<line x1="${(120+h*Math.cos(m)).toFixed(1)}" y1="${(120+h*Math.sin(m)).toFixed(1)}" x2="${(120+u*Math.cos(m)).toFixed(1)}" y2="${(120+u*Math.sin(m)).toFixed(1)}" stroke="rgba(${n},${v})" stroke-width="${w}"/>`}const x=26,t=78,p=68;let y="";for(let c=0;c<x;c++){const m=c/x*Math.PI*2,h=(c+.2)/x*Math.PI*2,u=(c+.5)/x*Math.PI*2,w=(c+.8)/x*Math.PI*2,v=(c+1)/x*Math.PI*2;y+=`${c===0?"M":"L"}${(120+p*Math.cos(m)).toFixed(1)},${(120+p*Math.sin(m)).toFixed(1)} `,y+=`L${(120+p*Math.cos(h)).toFixed(1)},${(120+p*Math.sin(h)).toFixed(1)} `,y+=`L${(120+t*Math.cos(u)).toFixed(1)},${(120+t*Math.sin(u)).toFixed(1)} `,y+=`L${(120+p*Math.cos(w)).toFixed(1)},${(120+p*Math.sin(w)).toFixed(1)} `,y+=`L${(120+p*Math.cos(v)).toFixed(1)},${(120+p*Math.sin(v)).toFixed(1)} `}y+="Z";let g="";for(let c=0;c<48;c++){const m=c/48*Math.PI*2,h=c%4===0?79:80,u=c%4===0?85:83,w=c%4===0?.6:.3,v=c%4===0?.6:.3;g+=`<line x1="${(120+h*Math.cos(m)).toFixed(1)}" y1="${(120+h*Math.sin(m)).toFixed(1)}" x2="${(120+u*Math.cos(m)).toFixed(1)}" y2="${(120+u*Math.sin(m)).toFixed(1)}" stroke="rgba(${e},${v})" stroke-width="${w}"/>`}function b(c,m,h,u,w){let v="";for(let F=0;F<h;F++){const ne=F/h*Math.PI*2,ae=(F+.25)/h*Math.PI*2,oe=(F+.75)/h*Math.PI*2,re=(F+1)/h*Math.PI*2;v+=`${F===0?"M":"L"}${(c+w*Math.cos(ne)).toFixed(1)},${(m+w*Math.sin(ne)).toFixed(1)} `,v+=`L${(c+u*Math.cos(ae)).toFixed(1)},${(m+u*Math.sin(ae)).toFixed(1)} `,v+=`L${(c+u*Math.cos(oe)).toFixed(1)},${(m+u*Math.sin(oe)).toFixed(1)} `,v+=`L${(c+w*Math.cos(re)).toFixed(1)},${(m+w*Math.sin(re)).toFixed(1)} `}return v+"Z"}const k=42,P=[],U=b(120,120,14,18,13);P.push(`<g class="nf-kinetic-sub" style="transform-origin:120px 120px">
...
`);let O="";for(let c=0;c<8;c++){const m=c/8*Math.PI*2,h=120+10*Math.cos(m),u=120+10*Math.sin(m),w=120+(k-10)*Math.cos(m),v=120+(k-10)*Math.sin(m);O+=`<line x1="${h.toFixed(1)}" y1="${u.toFixed(1)}" x2="${w.toFixed(1)}" y2="${v.toFixed(1)}" stroke="rgba(${e},0.25)" stroke-width="0.5"/>`}return`<svg width="80" height="80" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
        ...
        <g class="nf-kinetic-inner">
            <path d="${y}" fill="none" stroke="url(#nfKGrad3)" stroke-width="1.1" stroke-linejoin="bevel"/>
    ...
    </svg>`}function ke(){const n=document.createElement("div");n.id="netflow-engine-overlay",G=document.createElement("canvas"),G.id="nf-matrix-canvas",n.appendChild(G);const e=document.createElement("div");e.className="nf-pat-data",n.appendChild(e);const r=document.createElement("div");r.className="nf-center-glow",n.appendChild(r);const o=document.createElement("div");o.className="nf-pat-noise",n.appendChild(o);const i=document.createElement("div");i.className="nf-vignette",n.appendChild(i);for(let g=0;g<3;g++){const b=document.createElement("div");b.className="nf-pulse-ring",n.appendChild(b)}["nf-deco-tl","nf-deco-tr","nf-deco-bl","nf-deco-br"].forEach(g=>{const b=document.createElement("div");b.className=`nf-corner-deco ${g}`,n.appendChild(b)});const f=document.createElement("button");f.className="nf-close-btn",f.textContent="✕ ซ่อน",f.onclick=()=>Q(),n.appendChild(f);const l=document.createElement("div");l.className="nf-layout";const a=document.createElement("div");a.className="nf-core-monitor",a.id="nf-core-monitor";const s=document.createElement("div");s.className="nf-core-header",s.innerHTML=`
    ...
    `,a.appendChild(s);const d=document.createElement("div");d.className="nf-terminal",d.id="nf-terminal",fe(d),a.appendChild(d);const $=document.createElement("div");$.className="nf-engine-core",$.id="nf-engine-core";const M=document.createElement("div");M.className="nf-engine-frame",["nf-fc-tl","nf-fc-tr","nf-fc-bl","nf-fc-br"].forEach(g=>{const b=document.createElement("div");b.className=`nf-frame-corner ${g}`,M.appendChild(b)}),$.appendChild(M);const C="http://www.w3.org/2000/svg",x=document.createElementNS(C,"svg");x.setAttribute("class","nf-engine-waves"),x.setAttribute("viewBox","0 0 560 140"),x.setAttribute("preserveAspectRatio","none"),x.id="nf-engine-waves";for(let g=0;g<6;g++){const b=document.createElementNS(C,"path");b.setAttribute("fill","none"),b.setAttribute("stroke-width",g<3?"1.5":"1"),b.setAttribute("stroke",g<3?`rgba(${S.rgb},${.12+g*.08})`:`rgba(${S.accentRgb},${.08+(g-3)*.06})`),b.setAttribute("data-wave-idx",String(g)),x.appendChild(b)}$.appendChild(x);const t=document.createElement("div");t.className="nf-engine-brand-inner",t.innerHTML=`
        <div class="nf-brand-gear-icon">
            ${ve(S.rgb,S.accentRgb)}
        </div>
        <div class="nf-brand-inner-text">NETFLOW AI ENGINE V1.0</div>
    `,$.appendChild(t);const p=document.createElement("div");p.className="nf-engine-stats",p.id="nf-engine-stats",p.innerHTML=[["SCENES","nf-stat-scenes","1/1"],["ELAPSED","nf-stat-elapsed","00:00"],["STEP","nf-stat-step","0/0"],["STATUS","nf-stat-status","READY"],["PROGRESS","nf-stat-progress","—"]].map(([g,b,k])=>`<div class="nf-stat-item"><span class="nf-stat-label">${g}</span><span class="nf-stat-val" id="${b}">${k}</span></div>`).join(""),$.appendChild(p),a.appendChild($),l.appendChild(a);const y=["nf-mod-tl","nf-mod-tr","nf-mod-bl","nf-mod-br"];j.forEach((g,b)=>{const k=Me(g);k.classList.add(y[b]),k.id=`nf-mod-${g.id}`,l.appendChild(k)}),n.appendChild(l);for(let g=0;g<30;g++){const b=document.createElement("div");b.className="nf-particle",b.style.left=`${5+Math.random()*90}%`,b.style.bottom=`${Math.random()*40}%`,b.style.animationDuration=`${3+Math.random()*5}s`,b.style.animationDelay=`${Math.random()*4}s`;const k=.3+Math.random()*.4,P=.7+Math.random()*.3;b.style.background=`rgba(${Math.floor(L*P)}, ${Math.floor(T*P)}, ${Math.floor(R*P)}, ${k})`,b.style.width=`${1+Math.random()*2}px`,b.style.height=b.style.width,n.appendChild(b)}return n}function Me(n){const e=document.createElement("div");e.className="nf-module";const r=document.createElement("div");r.className="nf-mod-header",r.innerHTML=`
        <div class="nf-mod-title">${n.title}</div>
        <span class="nf-mod-pct" id="nf-pct-${n.id}">0%</span>
    `,e.appendChild(r),n.steps.forEach(i=>{const f=document.createElement("div");f.className="nf-step",f.id=`nf-step-${i.id}`;let l="";i.progress!==void 0&&(l=`
            ...
            ${l}
        `,e.appendChild(f)});const o=document.createElement("div");return o.className="nf-mod-progress",o.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${n.id}"></div>`,e.appendChild(o),e}function Ce(){ie=Date.now(),setInterval(()=>{const n=Math.floor((Date.now()-ie)/1e3),e=String(Math.floor(n/60)).padStart(2,"0"),r=String(n%60).padStart(2,"0"),o=document.getElementById("nf-timer");o&&(o.textContent=`${e}:${r}`);const i=document.getElementById("nf-stat-elapsed");i&&(i.textContent=`${e}:${r}`)},1e3)}const Ee=220,K=210,se=.4;let X=[];function Ie(n,e){X=[];for(let r=0;r<Ee;r++){const o=Math.random();let i;o<.22?i=0:o<.4?i=1:o<.55?i=2:o<.68?i=3:o<.84?i=4:i=5;const f=Math.random()*n,l=Math.random()*e,a=50+Math.random()*220,s=Math.random()*Math.PI*2,d=(.003+Math.random()*.008)*(Math.random()>.5?1:-1);X.push({x:i===0?Math.random()*n:f+Math.cos(s)*a,y:i===0?Math.random()*e:l+Math.sin(s)*a,vx:(Math.random()-.5)*se,vy:(Math.random()-.5)*se,radius:1.2+Math.random()*2.5,pulsePhase:Math.random()*Math.PI*2,pulseSpeed:.02+Math.random()*.03,motion:i,oCx:f,oCy:l,oRadius:a,oAngle:s,oSpeed:d})}}function Se(){if(!G)return;const n=G;if(Y=n.getContext("2d"),!Y)return;const e=()=>{n.width=window.innerWidth,n.height=window.innerHeight,X.length===0&&Ie(n.width,n.height)};e(),window.addEventListener("resize",e);let r=null,o=0,i=0;function f(){if(!Y||!G)return;requestAnimationFrame(f);const l=Y,a=G.width,s=G.height;l.fillStyle=`rgba(${L*.04|0},${T*.04|0},${R*.06|0},1)`,l.fillRect(0,0,a,s),(!r||o!==a||i!==s)&&(o=a,i=s,r=l.createRadialGradient(a*.5,s*.5,0,a*.5,s*.5,Math.max(a,s)*.6),r.addColorStop(0,`rgba(${L*.08|0},${T*.08|0},${R*.1|0},0.4)`),r.addColorStop(1,"rgba(0,0,0,0)")),l.fillStyle=r,l.fillRect(0,0,a,s);const d=X,$=d.length,M=K*K;for(let x=0;x<$;x++){const t=d[x];if(t.pulsePhase+=t.pulseSpeed,t.motion===0)t.x+=t.vx,t.y+=t.vy,t.x<0?(t.x=0,t.vx=Math.abs(t.vx)*(.8+Math.random()*.4)):t.x>a&&(t.x=a,t.vx=-Math.abs(t.vx)*(.8+Math.random()*.4)),t.y<0?(t.y=0,t.vy=Math.abs(t.vy)*(.8+Math.random()*.4)):t.y>s&&(t.y=s,t.vy=-Math.abs(t.vy)*(.8+Math.random()*.4));else if(t.motion===1)t.oAngle+=t.oSpeed,t.x=t.oCx+Math.cos(t.oAngle)*t.oRadius,t.y=t.oCy+Math.sin(t.oAngle)*t.oRadius,t.oCx+=Math.sin(t.oAngle*.3)*.15,t.oCy+=Math.cos(t.oAngle*.3)*.15;else if(t.motion===2)t.oAngle+=t.oSpeed,t.x=t.oCx+Math.cos(t.oAngle)*t.oRadius,t.y=t.oCy+Math.sin(t.oAngle)*t.oRadius*.5,t.oCx+=Math.sin(t.oAngle*.2)*.1,t.oCy+=Math.cos(t.oAngle*.2)*.1;else if(t.motion===3){t.oAngle+=t.oSpeed;const p=t.oAngle,y=t.oRadius*.7;t.x=t.oCx+y*Math.cos(p),t.y=t.oCy+y*Math.sin(p)*Math.cos(p),t.oCx+=Math.sin(p*.15)*.12,t.oCy+=Math.cos(p*.15)*.12}else if(t.motion===4){t.oAngle+=t.oSpeed*1.2;const p=t.oRadius*(.5+.5*Math.abs(Math.sin(t.oAngle*.15)));t.x=t.oCx+Math.cos(t.oAngle)*p,t.y=t.oCy+Math.sin(t.oAngle)*p,t.oCx+=Math.sin(t.oAngle*.1)*.18,t.oCy+=Math.cos(t.oAngle*.1)*.18}else t.oAngle+=t.oSpeed,t.x+=t.vx*.8,t.y=t.oCy+Math.sin(t.oAngle+t.x*.008)*t.oRadius*.35,t.x<-30?t.x=a+30:t.x>a+30&&(t.x=-30),t.oCy+=Math.sin(t.oAngle*.1)*.08;if(t.motion>0){const p=t.oRadius+50;t.oCx<-p?t.oCx=a+p:t.oCx>a+p&&(t.oCx=-p),t.oCy<-p?t.oCy=s+p:t.oCy>s+p&&(t.oCy=-p)}}const C=5;for(let x=0;x<C;x++){const t=x/C,p=(x+1)/C,y=((t+p)*.5*.35).toFixed(3);l.beginPath(),l.strokeStyle=`rgba(${L},${T},${R},${y})`,l.lineWidth=(t+p)*.5*1.2;for(let g=0;g<$;g++){const b=d[g];for(let k=g+1;k<$;k++){const P=d[k],U=b.x-P.x,V=b.y-P.y,O=U*U+V*V;if(O<M){const c=1-Math.sqrt(O)/K;c>=t&&c<p&&(l.moveTo(b.x,b.y),l.lineTo(P.x,P.y))}}}l.stroke()}l.save(),l.shadowColor=`rgba(${L},${T},${R},0.8)`,l.shadowBlur=25;for(let x=0;x<$;x++){const t=d[x],p=.6+.4*Math.sin(t.pulsePhase),y=t.radius*(.8+p*.4),g=L+(255-L)*.7*p|0,b=T+(255-T)*.7*p|0,k=R+(255-R)*.7*p|0;l.beginPath(),l.arc(t.x,t.y,y,0,Math.PI*2),l.fillStyle=`rgba(${g},${b},${k},${(.6+p*.4).toFixed(2)})`,l.fill()}l.restore(),l.fillStyle="rgba(255,255,255,0.45)",l.beginPath();for(let x=0;x<$;x++){const t=d[x];if(t.radius>2){const p=.6+.4*Math.sin(t.pulsePhase),y=t.radius*(.8+p*.4)*.35;l.moveTo(t.x+y,t.y),l.arc(t.x,t.y,y,0,Math.PI*2)}}l.fill(),l.fillStyle=`rgba(${L},${T},${R},0.08)`;for(let x=0;x<6;x++)l.fillRect(Math.random()*a,Math.random()*s,1,1)}f()}function ge(){requestAnimationFrame(ge),J+=.035;const n=document.getElementById("nf-engine-waves");if(!n)return;const e=560,r=140,o=e/2;n.querySelectorAll("path").forEach(f=>{const l=parseInt(f.getAttribute("data-wave-idx")||"0",10),a=10+l*5,s=1.2+l*.35,d=l*.6,$=.7+l*.12;let M=`M 0 ${r/2}`;for(let C=0;C<=e;C+=3){const x=Math.abs(C-o)/o,t=Math.pow(Math.min(1,x*1.6),.6),p=r/2+a*t*Math.sin(C/e*s*Math.PI*2+J*$+d);M+=` L ${C} ${Math.round(p*10)/10}`}f.setAttribute("d",M)})}function Pe(){J=0,ge(),Se(),setInterval(()=>{const n=document.getElementById("nf-stat-freq"),e=document.getElementById("nf-stat-lat1"),r=document.getElementById("nf-stat-lat2"),o=document.getElementById("nf-stat-buf");n&&(n.textContent=`${(4.5+Math.random()*.5).toFixed(1)} GHz`),e&&(e.textContent=`${Math.floor(12+Math.random()*10)}ms`),r&&(r.textContent=`${Math.floor(12+Math.random()*10)}ms`),o&&(o.textContent=`${Math.floor(90+Math.random()*9)}%`)},2e3)}function be(){let n=0;const e=E.filter(a=>a.status!=="skipped").length;for(const a of E){const s=document.getElementById(`nf-proc-${a.stepId}`);if(!s)continue;s.className="nf-proc-row";const d=s.querySelector(".nf-proc-badge");switch(a.status){case"done":s.classList.add("nf-proc-done"),d&&(d.textContent="✅ done"),n++;break;case"active":s.classList.add("nf-proc-active"),d&&(d.textContent=a.progress!==void 0&&a.progress>0?`⏳ ${a.progress}%`:"⏳ active");break;case"error":s.classList.add("nf-proc-error"),d&&(d.textContent="❌ error");break;case"skipped":s.classList.add("nf-proc-skipped"),d&&(d.textContent="— skip");break;default:s.classList.add("nf-proc-waiting"),d&&(d.textContent="(queued)")}}const r=document.getElementById("nf-step-counter");r&&(r.textContent=`${n}/${E.length}`);const o=document.querySelector(".nf-core-title-val"),i=document.querySelector(".nf-status-dot");n>=e&&e>0?(o&&(o.textContent="COMPLETE",o.style.color=S.doneHex),i&&(i.style.background=S.doneHex,i.style.boxShadow=`0 0 8px rgba(${S.doneRgb},0.7)`)):E.some(s=>s.status==="error")?(o&&(o.textContent="ERROR",o.style.color="#f87171"),i&&(i.style.background="#ef4444",i.style.boxShadow="0 0 8px rgba(239,68,68,0.7)")):E.some(s=>s.status==="active")&&o&&(o.textContent="ACTIVE",o.style.color=S.hex,o.style.textShadow=`0 0 10px rgba(${S.rgb},0.5)`);const f=document.getElementById("nf-terminal"),l=f==null?void 0:f.querySelector(".nf-proc-active");l&&f&&l.scrollIntoView({behavior:"smooth",block:"center"})}function xe(){I||(pe(),I=document.createElement("button"),I.id="nf-toggle-btn",I.className="nf-toggle-hidden",I.innerHTML="⚡",I.title="เปิด Netflow Overlay",I.onclick=()=>Q(),document.body.appendChild(I))}function Q(){A&&(xe(),_?(A.classList.remove("nf-hidden"),A.classList.add("nf-visible"),I&&(I.classList.remove("nf-toggle-visible"),I.classList.add("nf-toggle-hidden")),_=!1):(A.classList.remove("nf-visible"),A.classList.add("nf-hidden"),I&&(I.classList.remove("nf-toggle-hidden"),I.classList.add("nf-toggle-visible")),_=!0))}const le={red:"themes/theme-red.jpg",yellow:"themes/theme-yellow.jpg",blue:"themes/theme-blue.jpg",purple:"themes/theme-purple.jpg",green:"themes/theme-green.jpg"};function me(){const n=document.getElementById("nf-core-monitor");if(!n)return;let e=D;if(!e)try{e=localStorage.getItem("netflow_app_theme")||"red"}catch{e="red"}const r=le[e]||le.red;let o;try{o=chrome.runtime.getURL(r)}catch{o=`/${r}`}const i=S.rgb;n.style.backgroundImage=["linear-gradient(180deg, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.78) 45%, rgba(0,0,0,0.90) 100%)",`linear-gradient(180deg, rgba(${i},0.25) 0%, rgba(${i},0.12) 50%, rgba(${i},0.20) 100%)`,`url('${o}')`].join(", "),n.style.backgroundSize="auto, auto, cover",n.style.backgroundPosition="center, center, center",n.style.backgroundRepeat="no-repeat, no-repeat, no-repeat",n.style.setProperty("--nf-bg-set","1"),n.style.border=`1.5px solid rgba(${i},0.45)`,n.style.boxShadow=`0 0 70px rgba(${i},0.22), 0 0 140px rgba(${i},0.1), inset 0 1px 0 rgba(${i},0.15)`}function Fe(){if(S=$e(),ce(),A){_&&Q();return}B&&(B.remove(),B=null),pe(),W=1,E=Z(1),A=ke(),document.body.appendChild(A),_=!1,xe(),Ce(),Pe(),requestAnimationFrame(()=>me())}const Le={settings:"CONFIG","upload-char":"UPLOAD","upload-prod":"UPLOAD","img-prompt":"PROMPT","img-generate":"GENERATE","img-wait":"IMG WAIT",animate:"ANIMATE","vid-prompt":"VID PROMPT","vid-generate":"VID GEN","vid-wait":"VID WAIT",download:"DOWNLOAD",upscale:"UPSCALE",open:"OPENING"};function Te(n,e,r){const o=E.filter(a=>a.status==="done").length,i=E.length,f=document.getElementById("nf-stat-step");f&&(f.textContent=`${o}/${i}`);const l=document.getElementById("nf-stat-scenes");if(l&&(l.textContent=W>1?`1/${W}`:"1/1"),e==="active"){const a=document.getElementById("nf-stat-status"),s=Le[n]||n.toUpperCase();a&&(a.textContent=s)}else if(e==="done"&&o>=i){const a=document.getElementById("nf-stat-status");a&&(a.textContent="COMPLETE")}else if(e==="error"){const a=document.getElementById("nf-stat-status");a&&(a.textContent="ERROR")}}function N(n,e,r){if(!A)return;for(const i of j)for(const f of i.steps)f.id===n&&(f.status=e);for(const i of E)i.stepId===n&&(i.status=e);const o=document.getElementById(`nf-step-${n}`);o&&(o.className="nf-step",e==="active"?o.classList.add("nf-step-active"):e==="done"?o.classList.add("nf-step-done"):e==="error"&&o.classList.add("nf-step-error")),Te(n,e),he(),be()}function he(){for(const n of j){const e=n.steps.filter(s=>s.status!=="skipped").length,r=n.steps.filter(s=>s.status==="done").length,o=n.steps.some(s=>s.status==="active"),i=e>0?Math.round(r/e*100):0,f=document.getElementById(`nf-pct-${n.id}`);f&&(f.textContent=`${i}%`);const l=document.getElementById(`nf-modbar-${n.id}`);l&&(l.style.width=`${i}%`);const a=document.getElementById(`nf-mod-${n.id}`);a&&(a.classList.remove("nf-active","nf-done"),i>=100?a.classList.add("nf-done"):o&&a.classList.add("nf-active"))}}function Re(n){var o,i,f,l;W=n;const e=new Map;for(const a of E)e.set(a.stepId,{status:a.status,progress:a.progress});E=Z(n);for(const a of E){const s=e.get(a.stepId);s&&(a.status=s.status,s.progress!==void 0&&(a.progress=s.progress))}we();{const a=j.find(s=>s.id==="video");if(a){const s=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:((o=a.steps.find(d=>d.id==="animate"))==null?void 0:o.status)||"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:((i=a.steps.find(d=>d.id==="vid-prompt"))==null?void 0:i.status)||"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:((f=a.steps.find(d=>d.id==="vid-generate"))==null?void 0:f.status)||"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:((l=a.steps.find(d=>d.id==="vid-wait"))==null?void 0:l.status)||"waiting",progress:0}];for(let d=2;d<=n;d++)s.push({id:`scene${d}-prompt`,label:`Scene ${d} Prompt`,status:"waiting"}),s.push({id:`scene${d}-gen`,label:`Scene ${d} Generate`,status:"waiting"}),s.push({id:`scene${d}-wait`,label:`Scene ${d} รอผล`,status:"waiting",progress:0});a.steps=s,de(a)}}const r=j.find(a=>a.id==="render");if(r&&n>1){const a=r.steps.find(d=>d.id==="download");a&&(a.label="ดาวน์โหลด 720p");const s=r.steps.find(d=>d.id==="upscale");s&&(s.label="Full Video"),de(r)}he(),be()}function de(n){const e=document.getElementById(`nf-mod-${n.id}`);if(!e)return;e.querySelectorAll(".nf-step, .nf-mod-progress").forEach(i=>i.remove()),n.steps.forEach(i=>{const f=document.createElement("div");f.className="nf-step",f.id=`nf-step-${i.id}`;let l="";i.progress!==void 0&&(l=`
                <div class="nf-progress-bar">
                    <div class="nf-progress-fill" id="nf-bar-${i.id}" style="width: 0%"></div>
                </div>
            `),f.innerHTML=`
            ...
            <span class="nf-step-label">${i.label}</span>
            ${l}
        `,e.appendChild(f)});const o=document.createElement("div");o.className="nf-mod-progress",o.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${n.id}"></div>`,e.appendChild(o)}function H(n){n.replace(/^\[Netflow AI\]\s*/,"")}window.chrome={runtime:{sendMessage:()=>{},lastError:null},storage:{local:{get:(n,e)=>e==null?void 0:e({}),set:()=>{}}}};const ee=new URLSearchParams(location.search).get("theme")||"red";ue(ee);Re(2);Fe();setTimeout(()=>{H("🚀 Overlay preview started — theme: "+ee),N("settings","active")},400);setTimeout(()=>{N("settings","done"),N("upload","active"),H("✅ Settings configured")},1600);setTimeout(()=>{N("upload","done"),N("prompt","active"),H("✅ Images uploaded (2 refs)")},3e3);setTimeout(()=>{N("prompt","done"),N("generate","active"),H("✅ Prompt injected"),H("⏳ Waiting for Scene 1 generation...")},4500);setTimeout(()=>{N("generate","done"),N("scene2","active"),H("✅ Scene 1 generated"),H("⏳ Generating Scene 2...")},7e3);const te=document.createElement("div");te.style.cssText=`

...
@media (max-height: 600px) {
    ...
    `,I.appendChild(V);const U=document.createElement("div");U.className="nf-terminal",U.id="nf-terminal",Me(U),I.appendChild(U);const _=document.createElement("div");_.className="nf-engine-core",_.id="nf-engine-core";const Q=document.createElement("div");Q.className="nf-engine-frame",["nf-fc-tl","nf-fc-tr","nf-fc-bl","nf-fc-br"].forEach(b=>{const x=document.createElement("div");x.className=`nf-frame-corner ${b}`,Q.appendChild(x)}),_.appendChild(Q);const le="http://www.w3.org/2000/svg",O=document.createElementNS(le,"svg");O.setAttribute("class","nf-engine-waves"),O.setAttribute("viewBox","0 0 560 140"),O.setAttribute("preserveAspectRatio","none"),O.id="nf-engine-waves";for(let b=0;b<4;b++){const x=document.createElementNS(le,"path");x.setAttribute("fill","none"),x.setAttribute("stroke-width",b<2?"1.5":"1"),x.setAttribute("stroke",b<2?`rgba(${v.rgb},${.14+b*.1})`:`rgba(${v.accentRgb},${.1+(b-2)*.08})`),x.setAttribute("data-wave-idx",String(b)),O.appendChild(x)}_.appendChild(O);const ee=document.createElement("div");ee.className="nf-engine-brand-inner",ee.innerHTML=`
    ...
    `,_.appendChild(ee);const Y=document.createElement("div");Y.className="nf-engine-stats",Y.id="nf-engine-stats",Y.innerHTML=[["SCENES","nf-stat-scenes","1/1"],["ELAPSED","nf-stat-elapsed","00:00"],["STEP","nf-stat-step","0/0"],["STATUS","nf-stat-status","READY"],["PROGRESS","nf-stat-progress","—"]].map(([b,x,H])=>`<div class="nf-stat-item"><span class="nf-stat-label">${b}</span><span class="nf-stat-val" id="${x}">${H}</span></div>`).join(""),_.appendChild(Y),I.appendChild(_),B.appendChild(I);const Pe=["nf-mod-tl","nf-mod-tr","nf-mod-bl","nf-mod-br"];z.forEach((b,x)=>{const H=Be(b);H.classList.add(Pe[x]),H.id=`nf-mod-${b.id}`,B.appendChild(H)}),t.appendChild(B);for(let b=0;b<30;b++){const x=document.createElement("div");x.className="nf-particle",x.style.left=`${5+Math.random()*90}%`,x.style.bottom=`${Math.random()*40}%`,x.style.animationDuration=`${3+Math.random()*5}s`,x.style.animationDelay=`${Math.random()*4}s`;const H=.3+Math.random()*.4,te=.7+Math.random()*.3;x.style.background=`rgba(${Math.floor(C*te)}, ${Math.floor(E*te)}, ${Math.floor(S*te)}, ${H})`,x.style.width=`${1+Math.random()*2}px`,x.style.height=x.style.width,t.appendChild(x)}return t}function Be(t){const e=document.createElement("div");e.className="nf-module";const a=document.createElement("div");a.className="nf-mod-header",a.innerHTML=`

import"./modulepreload-polyfill-B5Qt9EMX.js";const O={green:{rgb:"0, 255, 65",hex:"#00ff41",accentRgb:"0, 255, 180",accentHex:"#00ffb4",doneRgb:"34, 197, 94",doneHex:"#22c55e"},red:{rgb:"220, 38, 38",hex:"#dc2626",accentRgb:"251, 146, 60",accentHex:"#fb923c",doneRgb:"34, 197, 94",doneHex:"#22c55e"},blue:{rgb:"43, 125, 233",hex:"#2b7de9",accentRgb:"6, 182, 212",accentHex:"#06b6d4",doneRgb:"34, 197, 94",doneHex:"#22c55e"},yellow:{rgb:"234, 179, 8",hex:"#eab308",accentRgb:"245, 158, 11",accentHex:"#f59e0b",doneRgb:"34, 197, 94",doneHex:"#22c55e"},purple:{rgb:"139, 92, 246",hex:"#8b5cf6",accentRgb:"168, 85, 247",accentHex:"#a855f7",doneRgb:"34, 197, 94",doneHex:"#22c55e"}};let C=O.green,K=null;function Pe(t){O[t]&&(K=t,C=O[t],we(),requestAnimationFrame(()=>Ie()))}function Te(){if(K&&O[K])return O[K];try{const t=localStorage.getItem("netflow_app_theme");if(t&&O[t])return O[t]}catch{}return O.green}let L=0,R=255,A=65;function we(){const t=C.hex.match(/^#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i);t&&(L=parseInt(t[1],16),R=parseInt(t[2],16),A=parseInt(t[3],16))}let P=null,S=null,_=null,de=0,re=0,X=!1,D=null,J=null,ee=1,E=[];function oe(t){const e=[{stepId:"upload-char",label:"อัปโหลดภาพตัวละคร",status:"waiting"},{stepId:"upload-prod",label:"อัปโหลดภาพสินค้า",status:"waiting"},{stepId:"img-prompt",label:"ใส่คำสั่งสร้างภาพ",status:"waiting"},{stepId:"img-generate",label:"สั่งสร้างภาพ",status:"waiting"},{stepId:"img-wait",label:"รอผลลัพธ์การสร้างภาพ",status:"waiting",progress:0},{stepId:"animate",label:"แปลงเป็นภาพเคลื่อนไหว",status:"waiting"}];if(t<=1)e.push({stepId:"vid-prompt",label:"ใส่คำสั่งสร้างวิดีโอ",status:"waiting"},{stepId:"vid-generate",label:"สั่งสร้างวิดีโอ",status:"waiting"},{stepId:"vid-wait",label:"กำลังสร้างวิดีโอ",status:"waiting",progress:0},{stepId:"download",label:"ดาวน์โหลดวิดีโอ",status:"waiting"},{stepId:"upscale",label:"อัปสเกลความละเอียด",status:"waiting",progress:0},{stepId:"open",label:"เปิดดูตัวอย่างวิดีโอ",status:"waiting"});else{e.push({stepId:"vid-prompt",label:"ฉาก 1 — ใส่คำสั่ง",status:"waiting"},{stepId:"vid-generate",label:"ฉาก 1 — สั่งสร้าง",status:"waiting"},{stepId:"vid-wait",label:"ฉาก 1 — กำลังสร้าง",status:"waiting",progress:0});for(let a=2;a<=t;a++)e.push({stepId:`scene${a}-prompt`,label:`ฉาก ${a} — ใส่คำสั่ง`,status:"waiting"},{stepId:`scene${a}-gen`,label:`ฉาก ${a} — สั่งสร้าง`,status:"waiting"},{stepId:`scene${a}-wait`,label:`ฉาก ${a} — กำลังสร้าง`,status:"waiting",progress:0});e.push({stepId:"download",label:"ดาวน์โหลดวิดีโอ",status:"waiting"},{stepId:"upscale",label:"อัปสเกลความละเอียด",status:"waiting",progress:0},{stepId:"open",label:"เปิดดูตัวอย่างวิดีโอ",status:"waiting"})}return e}const U=[{id:"ingest",title:"ASSET_INGEST",steps:[{id:"settings",label:"กำหนดค่าเริ่มต้น",status:"waiting"},{id:"upload-char",label:"อัปโหลดภาพตัวละคร",status:"waiting"},{id:"upload-prod",label:"อัปโหลดภาพสินค้า",status:"waiting"}]},{id:"image",title:"AI_IMAGE_SYNTHESIS",steps:[{id:"img-prompt",label:"ใส่คำสั่งสร้างภาพ",status:"waiting"},{id:"img-generate",label:"สั่งสร้างภาพ",status:"waiting"},{id:"img-wait",label:"รอผลลัพธ์การสร้างภาพ",status:"waiting",progress:0}]},{id:"video",title:"VIDEO_PRODUCTION",steps:[{id:"animate",label:"แปลงเป็นภาพเคลื่อนไหว",status:"waiting"},{id:"vid-prompt",label:"ใส่คำสั่งสร้างวิดีโอ",status:"waiting"},{id:"vid-generate",label:"สั่งสร้างวิดีโอ",status:"waiting"},{id:"vid-wait",label:"กำลังสร้างวิดีโอ",status:"waiting",progress:0}]},{id:"render",title:"FINAL_RENDER_OUTPUT",steps:[{id:"download",label:"ดาวน์โหลดวิดีโอ",status:"waiting"},{id:"upscale",label:"อัปสเกลความละเอียด",status:"waiting",progress:0},{id:"open",label:"เปิดดูตัวอย่างวิดีโอ",status:"waiting"}]}];E=oe(1);function ze(t){const e=t.rgb,a=t.accentRgb,r=t.doneRgb,i=t.hex,c=t.accentHex,g=t.doneHex,o=(()=>{const p=i.match(/^#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i);if(!p)return"#4ade80";const n=f=>Math.min(255,f+80);return`#${[1,2,3].map(f=>n(parseInt(p[f],16)).toString(16).padStart(2,"0")).join("")}`})(),s=(()=>{const p=g.match(/^#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i);if(!p)return"#4ade80";const n=f=>Math.min(255,f+60);return`#${[1,2,3].map(f=>n(parseInt(p[f],16)).toString(16).padStart(2,"0")).join("")}`})(),l=i.match(/^#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i),k=l?Math.max(parseInt(l[1],16),parseInt(l[2],16),parseInt(l[3],16),1):255,M=l?parseInt(l[1],16)/k:0,B=l?parseInt(l[2],16)/k:1,I=l?parseInt(l[3],16)/k:.25,v=p=>`${Math.round(M*p)}, ${Math.round(B*p)}, ${Math.round(I*p)}`;return`
/* ─── Google Font ─── */
...
.nf-step-done .nf-progress-fill {
    background: linear-gradient(90deg, ${g}, ${s});
}
...
.nf-proc-skipped {
    ...
    `}function ye(){_||(_=document.createElement("style"),_.id="netflow-overlay-styles",_.textContent=ze(C),document.head.appendChild(_))}function ve(t){t.innerHTML="",E.forEach((e,a)=>{const r=document.createElement("div");r.className="nf-proc-row nf-proc-waiting",r.id=`nf-proc-${e.stepId}`,r.innerHTML=`
            <span class="nf-proc-num">${a+1}</span>
            <span class="nf-proc-dot"></span>
            <span class="nf-proc-label">${e.label}</span>
            <span class="nf-proc-badge">(queued)</span>
        `,t.appendChild(r)})}function Ne(){const t=document.getElementById("nf-terminal");if(!t)return;ve(t);const e=document.getElementById("nf-step-counter");e&&(e.textContent=`0/${E.length}`)}function pe(t,e){let o="";for(let d=0;d<32;d++){const b=d/32*Math.PI*2,h=(d+.2)/32*Math.PI*2,m=(d+.5)/32*Math.PI*2,$=(d+.8)/32*Math.PI*2,w=(d+1)/32*Math.PI*2;o+=`${d===0?"M":"L"}${(120+104*Math.cos(b)).toFixed(1)},${(120+104*Math.sin(b)).toFixed(1)} `,o+=`L${(120+104*Math.cos(h)).toFixed(1)},${(120+104*Math.sin(h)).toFixed(1)} `,o+=`L${(120+116*Math.cos(m)).toFixed(1)},${(120+116*Math.sin(m)).toFixed(1)} `,o+=`L${(120+104*Math.cos($)).toFixed(1)},${(120+104*Math.sin($)).toFixed(1)} `,o+=`L${(120+104*Math.cos(w)).toFixed(1)},${(120+104*Math.sin(w)).toFixed(1)} `}o+="Z";const s=24,l=100,k=90;let M="";for(let d=0;d<s;d++){const b=d/s*Math.PI*2,h=(d+.25)/s*Math.PI*2,m=(d+.75)/s*Math.PI*2,$=(d+1)/s*Math.PI*2;M+=`${d===0?"M":"L"}${(120+k*Math.cos(b)).toFixed(1)},${(120+k*Math.sin(b)).toFixed(1)} `,M+=`L${(120+l*Math.cos(h)).toFixed(1)},${(120+l*Math.sin(h)).toFixed(1)} `,M+=`L${(120+l*Math.cos(m)).toFixed(1)},${(120+l*Math.sin(m)).toFixed(1)} `,M+=`L${(120+k*Math.cos($)).toFixed(1)},${(120+k*Math.sin($)).toFixed(1)} `}M+="Z";let B="";for(let d=0;d<64;d++){const b=d/64*Math.PI*2,h=d%4===0?117:119,m=d%4===0?124:122,$=d%4===0?.8:.4,w=d%4===0?.7:.35;B+=`<line x1="${(120+h*Math.cos(b)).toFixed(1)}" y1="${(120+h*Math.sin(b)).toFixed(1)}" x2="${(120+m*Math.cos(b)).toFixed(1)}" y2="${(120+m*Math.sin(b)).toFixed(1)}" stroke="rgba(${t},${w})" stroke-width="${$}"/>`}const I=26,v=78,p=68;let n="";for(let d=0;d<I;d++){const b=d/I*Math.PI*2,h=(d+.2)/I*Math.PI*2,m=(d+.5)/I*Math.PI*2,$=(d+.8)/I*Math.PI*2,w=(d+1)/I*Math.PI*2;n+=`${d===0?"M":"L"}${(120+p*Math.cos(b)).toFixed(1)},${(120+p*Math.sin(b)).toFixed(1)} `,n+=`L${(120+p*Math.cos(h)).toFixed(1)},${(120+p*Math.sin(h)).toFixed(1)} `,n+=`L${(120+v*Math.cos(m)).toFixed(1)},${(120+v*Math.sin(m)).toFixed(1)} `,n+=`L${(120+p*Math.cos($)).toFixed(1)},${(120+p*Math.sin($)).toFixed(1)} `,n+=`L${(120+p*Math.cos(w)).toFixed(1)},${(120+p*Math.sin(w)).toFixed(1)} `}n+="Z";let f="";for(let d=0;d<48;d++){const b=d/48*Math.PI*2,h=d%4===0?79:80,m=d%4===0?85:83,$=d%4===0?.6:.3,w=d%4===0?.6:.3;f+=`<line x1="${(120+h*Math.cos(b)).toFixed(1)}" y1="${(120+h*Math.sin(b)).toFixed(1)}" x2="${(120+m*Math.cos(b)).toFixed(1)}" y2="${(120+m*Math.sin(b)).toFixed(1)}" stroke="rgba(${e},${w})" stroke-width="${$}"/>`}function y(d,b,h,m,$){let w="";for(let F=0;F<h;F++){const u=F/h*Math.PI*2,x=(F+.25)/h*Math.PI*2,G=(F+.75)/h*Math.PI*2,q=(F+1)/h*Math.PI*2;w+=`${F===0?"M":"L"}${(d+$*Math.cos(u)).toFixed(1)},${(b+$*Math.sin(u)).toFixed(1)} `,w+=`L${(d+m*Math.cos(x)).toFixed(1)},${(b+m*Math.sin(x)).toFixed(1)} `,w+=`L${(d+m*Math.cos(G)).toFixed(1)},${(b+m*Math.sin(G)).toFixed(1)} `,w+=`L${(d+$*Math.cos(q)).toFixed(1)},${(b+$*Math.sin(q)).toFixed(1)} `}return w+"Z"}const T=42,z=[],N=y(120,120,14,18,13);z.push(`<g class="nf-kinetic-sub" style="transform-origin:120px 120px">
        <path d="${N}" fill="none" stroke="rgba(${t},0.8)" stroke-width="1.2"/>
    ...
    </g>`);for(let d=0;d<8;d++){const b=d/8*Math.PI*2,h=120+T*Math.cos(b),m=120+T*Math.sin(b),w=y(h,m,10,14,10),F=d%2===0?"":"animation-direction:reverse;";z.push(`<g class="nf-kinetic-sub" style="transform-origin:${h.toFixed(1)}px ${m.toFixed(1)}px;${F}">
            <path d="${w}" fill="none" stroke="rgba(${e},0.6)" stroke-width="0.9"/>
            <circle cx="${h.toFixed(1)}" cy="${m.toFixed(1)}" r="7" fill="none" stroke="rgba(${t},0.4)" stroke-width="0.5" stroke-dasharray="1.5,1.5"/>
            <circle cx="${h.toFixed(1)}" cy="${m.toFixed(1)}" r="2.5" fill="rgba(${t},0.6)"/>
        </g>`)}const V=z.join(`
`);let j="";for(let d=0;d<8;d++){const b=d/8*Math.PI*2,h=120+10*Math.cos(b),m=120+10*Math.sin(b),$=120+(T-10)*Math.cos(b),w=120+(T-10)*Math.sin(b);j+=`<line x1="${h.toFixed(1)}" y1="${m.toFixed(1)}" x2="${$.toFixed(1)}" y2="${w.toFixed(1)}" stroke="rgba(${e},0.25)" stroke-width="0.5"/>`}return`<svg width="80" height="80" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
    ...
    </svg>`}function Fe(){const t=document.createElement("div");t.id="netflow-engine-overlay",D=document.createElement("canvas"),D.id="nf-matrix-canvas",t.appendChild(D);const e=document.createElement("div");e.className="nf-pat-plasma",t.appendChild(e);for(let u=1;u<=5;u++){const x=document.createElement("div");x.className=`nf-ambient-orb nf-orb-${u}`,t.appendChild(x)}const a=document.createElement("div");a.className="nf-pat-data",t.appendChild(a);const r=document.createElement("div");r.className="nf-pat-diag-a",t.appendChild(r);const i=document.createElement("div");i.className="nf-pat-diag-b",t.appendChild(i);const c=document.createElement("div");c.className="nf-pat-circuit",t.appendChild(c);const g=document.createElement("div");g.className="nf-pat-honeycomb",t.appendChild(g);const o=document.createElement("div");o.className="nf-pat-binary",t.appendChild(o);const s=document.createElement("div");s.className="nf-pat-crosshatch",t.appendChild(s);const l=document.createElement("div");l.className="nf-pat-diamond",t.appendChild(l);const k=document.createElement("div");k.className="nf-pat-wave-h",t.appendChild(k);const M=document.createElement("div");M.className="nf-pat-radar",t.appendChild(M);const B=document.createElement("div");B.className="nf-pat-ripple-1",t.appendChild(B);const I=document.createElement("div");I.className="nf-pat-ripple-2",t.appendChild(I);const v=document.createElement("div");v.className="nf-pat-techscan",t.appendChild(v);const p=document.createElement("div");p.className="nf-center-glow",t.appendChild(p);const n=document.createElement("div");n.className="nf-pat-noise",t.appendChild(n);const f=document.createElement("div");f.className="nf-crt-scanlines",t.appendChild(f);const y=document.createElement("div");y.className="nf-vignette",t.appendChild(y);for(let u=0;u<3;u++){const x=document.createElement("div");x.className="nf-pulse-ring",t.appendChild(x)}["nf-deco-tl","nf-deco-tr","nf-deco-bl","nf-deco-br"].forEach(u=>{const x=document.createElement("div");x.className=`nf-corner-deco ${u}`,t.appendChild(x)});const T=document.createElement("button");T.className="nf-close-btn",T.textContent="✕ ซ่อน",T.onclick=()=>ie(),t.appendChild(T);const z=document.createElement("div");z.className="nf-layout";const N=document.createElement("div");N.className="nf-core-monitor",N.id="nf-core-monitor";const V=document.createElement("div");V.className="nf-core-header",V.innerHTML=`
    ...
    `,N.appendChild(V);const j=document.createElement("div");j.className="nf-terminal",j.id="nf-terminal",ve(j),N.appendChild(j);const d=document.createElement("div");d.className="nf-engine-core",d.id="nf-engine-core";const b=document.createElement("div");b.className="nf-engine-frame",["nf-fc-tl","nf-fc-tr","nf-fc-bl","nf-fc-br"].forEach(u=>{const x=document.createElement("div");x.className=`nf-frame-corner ${u}`,b.appendChild(x)}),d.appendChild(b);const h="http://www.w3.org/2000/svg",m=document.createElementNS(h,"svg");m.setAttribute("class","nf-engine-waves"),m.setAttribute("viewBox","0 0 560 140"),m.setAttribute("preserveAspectRatio","none"),m.id="nf-engine-waves";for(let u=0;u<4;u++){const x=document.createElementNS(h,"path");x.setAttribute("fill","none"),x.setAttribute("stroke-width",u<2?"1.5":"1"),x.setAttribute("stroke",u<2?`rgba(${C.rgb},${.14+u*.1})`:`rgba(${C.accentRgb},${.1+(u-2)*.08})`),x.setAttribute("data-wave-idx",String(u)),m.appendChild(x)}d.appendChild(m);const $=document.createElement("div");$.className="nf-engine-brand-inner",$.innerHTML=`
        <div class="nf-brand-gear-icon nf-brand-gear-left">
            ${pe(C.rgb,C.accentRgb)}
        </div>
        <div class="nf-brand-inner-text">NETFLOW AI ENGINE V1.0</div>
        <div class="nf-brand-gear-icon nf-brand-gear-right">
            ${pe(C.rgb,C.accentRgb)}
        </div>
    `,d.appendChild($);const w=document.createElement("div");w.className="nf-engine-stats",w.id="nf-engine-stats",w.innerHTML=[["SCENES","nf-stat-scenes","1/1"],["ELAPSED","nf-stat-elapsed","00:00"],["STEP","nf-stat-step","0/0"],["STATUS","nf-stat-status","READY"],["PROGRESS","nf-stat-progress","—"]].map(([u,x,G])=>`<div class="nf-stat-item"><span class="nf-stat-label">${u}</span><span class="nf-stat-val" id="${x}">${G}</span></div>`).join(""),d.appendChild(w),N.appendChild(d),z.appendChild(N);const F=["nf-mod-tl","nf-mod-tr","nf-mod-bl","nf-mod-br"];U.forEach((u,x)=>{const G=Le(u);G.classList.add(F[x]),G.id=`nf-mod-${u.id}`,z.appendChild(G)}),t.appendChild(z);for(let u=0;u<30;u++){const x=document.createElement("div");x.className="nf-particle",x.style.left=`${5+Math.random()*90}%`,x.style.bottom=`${Math.random()*40}%`,x.style.animationDuration=`${3+Math.random()*5}s`,x.style.animationDelay=`${Math.random()*4}s`;const G=.3+Math.random()*.4,q=.7+Math.random()*.3;x.style.background=`rgba(${Math.floor(L*q)}, ${Math.floor(R*q)}, ${Math.floor(A*q)}, ${G})`,x.style.width=`${1+Math.random()*2}px`,x.style.height=x.style.width,t.appendChild(x)}return t}function Le(t){const e=document.createElement("div");e.className="nf-module";const a=document.createElement("div");a.className="nf-mod-header",a.innerHTML=`
        <div class="nf-mod-title">${t.title}</div>
        <span class="nf-mod-pct" id="nf-pct-${t.id}">0%</span>
    `,e.appendChild(a),t.steps.forEach(i=>{const c=document.createElement("div");c.className="nf-step",c.id=`nf-step-${i.id}`;let g="";i.progress!==void 0&&(g=`
            ...
            ${g}
        `,e.appendChild(c)});const r=document.createElement("div");return r.className="nf-mod-progress",r.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${t.id}"></div>`,e.appendChild(r),e}function Re(){de=Date.now(),setInterval(()=>{const t=Math.floor((Date.now()-de)/1e3),e=String(Math.floor(t/60)).padStart(2,"0"),a=String(t%60).padStart(2,"0"),r=document.getElementById("nf-timer");r&&(r.textContent=`${e}:${a}`);const i=document.getElementById("nf-stat-elapsed");i&&(i.textContent=`${e}:${a}`)},1e3)}const Ae=120,ce=160,fe=.4;let W=null,ge=0,be=0,xe=0,te=[];function Be(t,e){te=[];for(let a=0;a<Ae;a++){const r=Math.random();let i;r<.22?i=0:r<.4?i=1:r<.55?i=2:r<.68?i=3:r<.84?i=4:i=5;const c=Math.random()*t,g=Math.random()*e,o=50+Math.random()*220,s=Math.random()*Math.PI*2,l=(.003+Math.random()*.008)*(Math.random()>.5?1:-1);te.push({x:i===0?Math.random()*t:c+Math.cos(s)*o,y:i===0?Math.random()*e:g+Math.sin(s)*o,vx:(Math.random()-.5)*fe,vy:(Math.random()-.5)*fe,radius:1.2+Math.random()*2.5,pulsePhase:Math.random()*Math.PI*2,pulseSpeed:.02+Math.random()*.03,motion:i,oCx:c,oCy:g,oRadius:o,oAngle:s,oSpeed:l})}}function Ge(){if(!D)return;const t=D;if(J=t.getContext("2d"),!J)return;const e=()=>{t.width=window.innerWidth,t.height=window.innerHeight,te.length===0&&Be(t.width,t.height)};e(),window.addEventListener("resize",e);let a=null,r=0,i=0,c=!1;function g(){if(!J||!D||(requestAnimationFrame(g),c=!c,c))return;const o=J,s=D.width,l=D.height;o.fillStyle=`rgba(${L*.04|0},${R*.04|0},${A*.06|0},1)`,o.fillRect(0,0,s,l),(!a||r!==s||i!==l)&&(r=s,i=l,a=o.createRadialGradient(s*.5,l*.5,0,s*.5,l*.5,Math.max(s,l)*.6),a.addColorStop(0,`rgba(${L*.08|0},${R*.08|0},${A*.1|0},0.4)`),a.addColorStop(1,"rgba(0,0,0,0)")),o.fillStyle=a,o.fillRect(0,0,s,l);const k=te,M=k.length,B=ce*ce;for(let p=0;p<M;p++){const n=k[p];if(n.pulsePhase+=n.pulseSpeed,n.motion===0)n.x+=n.vx,n.y+=n.vy,n.x<0?(n.x=0,n.vx=Math.abs(n.vx)*(.8+Math.random()*.4)):n.x>s&&(n.x=s,n.vx=-Math.abs(n.vx)*(.8+Math.random()*.4)),n.y<0?(n.y=0,n.vy=Math.abs(n.vy)*(.8+Math.random()*.4)):n.y>l&&(n.y=l,n.vy=-Math.abs(n.vy)*(.8+Math.random()*.4));else if(n.motion===1)n.oAngle+=n.oSpeed,n.x=n.oCx+Math.cos(n.oAngle)*n.oRadius,n.y=n.oCy+Math.sin(n.oAngle)*n.oRadius,n.oCx+=Math.sin(n.oAngle*.3)*.15,n.oCy+=Math.cos(n.oAngle*.3)*.15;else if(n.motion===2)n.oAngle+=n.oSpeed,n.x=n.oCx+Math.cos(n.oAngle)*n.oRadius,n.y=n.oCy+Math.sin(n.oAngle)*n.oRadius*.5,n.oCx+=Math.sin(n.oAngle*.2)*.1,n.oCy+=Math.cos(n.oAngle*.2)*.1;else if(n.motion===3){n.oAngle+=n.oSpeed;const f=n.oAngle,y=n.oRadius*.7;n.x=n.oCx+y*Math.cos(f),n.y=n.oCy+y*Math.sin(f)*Math.cos(f),n.oCx+=Math.sin(f*.15)*.12,n.oCy+=Math.cos(f*.15)*.12}else if(n.motion===4){n.oAngle+=n.oSpeed*1.2;const f=n.oRadius*(.5+.5*Math.abs(Math.sin(n.oAngle*.15)));n.x=n.oCx+Math.cos(n.oAngle)*f,n.y=n.oCy+Math.sin(n.oAngle)*f,n.oCx+=Math.sin(n.oAngle*.1)*.18,n.oCy+=Math.cos(n.oAngle*.1)*.18}else n.oAngle+=n.oSpeed,n.x+=n.vx*.8,n.y=n.oCy+Math.sin(n.oAngle+n.x*.008)*n.oRadius*.35,n.x<-30?n.x=s+30:n.x>s+30&&(n.x=-30),n.oCy+=Math.sin(n.oAngle*.1)*.08;if(n.motion>0){const f=n.oRadius+50;n.oCx<-f?n.oCx=s+f:n.oCx>s+f&&(n.oCx=-f),n.oCy<-f?n.oCy=l+f:n.oCy>l+f&&(n.oCy=-f)}}o.beginPath(),o.strokeStyle=`rgba(${L},${R},${A},0.06)`,o.lineWidth=.4;const I=new Path2D;for(let p=0;p<M;p++){const n=k[p];for(let f=p+1;f<M;f++){const y=k[f],T=n.x-y.x,z=n.y-y.y,N=T*T+z*z;N<B&&(1-N/B<.4?(o.moveTo(n.x,n.y),o.lineTo(y.x,y.y)):(I.moveTo(n.x,n.y),I.lineTo(y.x,y.y)))}}if(o.stroke(),o.strokeStyle=`rgba(${L},${R},${A},0.18)`,o.lineWidth=.8,o.stroke(I),!W||ge!==L||be!==R||xe!==A){W=document.createElement("canvas");const p=48;W.width=p,W.height=p;const n=W.getContext("2d"),f=n.createRadialGradient(p/2,p/2,0,p/2,p/2,p/2);f.addColorStop(0,`rgba(${L},${R},${A},0.9)`),f.addColorStop(.3,`rgba(${L},${R},${A},0.35)`),f.addColorStop(1,`rgba(${L},${R},${A},0)`),n.fillStyle=f,n.fillRect(0,0,p,p),ge=L,be=R,xe=A}const v=W;for(let p=0;p<M;p++){const n=k[p],f=.6+.4*Math.sin(n.pulsePhase),y=n.radius*5*(.8+f*.4);o.globalAlpha=.5+f*.4,o.drawImage(v,n.x-y/2,n.y-y/2,y,y)}o.globalAlpha=1,o.fillStyle="rgba(255,255,255,0.45)",o.beginPath();for(let p=0;p<M;p++){const n=k[p];if(n.radius>2){const f=.6+.4*Math.sin(n.pulsePhase),y=n.radius*(.8+f*.4)*.35;o.moveTo(n.x+y,n.y),o.arc(n.x,n.y,y,0,Math.PI*2)}}o.fill()}g()}let Z=null;const ne=560,He=140,me=ne/2,he=He/2,ke=[];for(let t=0;t<=ne;t+=8){const e=Math.abs(t-me)/me;ke.push(Math.pow(Math.min(1,e*1.6),.6))}const Oe=[0,1,2,3].map(t=>({amp:10+t*5,freq:(1.2+t*.35)*Math.PI*2/ne,off:t*.6,spd:.7+t*.12}));let ae=!1;function Me(){if(requestAnimationFrame(Me),ae=!ae,ae)return;if(re+=.07,!Z){const e=document.getElementById("nf-engine-waves");if(!e)return;Z=Array.from(e.querySelectorAll("path"))}const t=[];for(let e=0;e<Z.length;e++){const a=Oe[e],r=re*a.spd+a.off;t.length=0,t.push(`M 0 ${he}`);let i=0;for(let c=0;c<=ne;c+=8){const g=he+a.amp*ke[i++]*Math.sin(c*a.freq+r);t.push(`L${c} ${g*10+.5|0}`)}Z[e].setAttribute("d",t.join(" "))}}function _e(){re=0,Me(),Ge(),setInterval(()=>{const t=document.getElementById("nf-stat-freq"),e=document.getElementById("nf-stat-lat1"),a=document.getElementById("nf-stat-lat2"),r=document.getElementById("nf-stat-buf");t&&(t.textContent=`${(4.5+Math.random()*.5).toFixed(1)} GHz`),e&&(e.textContent=`${Math.floor(12+Math.random()*10)}ms`),a&&(a.textContent=`${Math.floor(12+Math.random()*10)}ms`),r&&(r.textContent=`${Math.floor(90+Math.random()*9)}%`)},2e3)}function Ce(){let t=0;const e=E.filter(o=>o.status!=="skipped").length;for(const o of E){const s=document.getElementById(`nf-proc-${o.stepId}`);if(!s)continue;s.className="nf-proc-row";const l=s.querySelector(".nf-proc-badge");switch(o.status){case"done":s.classList.add("nf-proc-done"),l&&(l.textContent="✅ done"),t++;break;case"active":s.classList.add("nf-proc-active"),l&&(l.textContent=o.progress!==void 0&&o.progress>0?`⏳ ${o.progress}%`:"⏳ active");break;case"error":s.classList.add("nf-proc-error"),l&&(l.textContent="❌ error");break;case"skipped":s.classList.add("nf-proc-skipped"),l&&(l.textContent="— skip");break;default:s.classList.add("nf-proc-waiting"),l&&(l.textContent="(queued)")}}const a=document.getElementById("nf-step-counter");a&&(a.textContent=`${t}/${E.length}`);const r=document.querySelector(".nf-core-title-val"),i=document.querySelector(".nf-status-dot");t>=e&&e>0?(r&&(r.textContent="COMPLETE",r.style.color=C.doneHex),i&&(i.style.background=C.doneHex,i.style.boxShadow=`0 0 8px rgba(${C.doneRgb},0.7)`)):E.some(s=>s.status==="error")?(r&&(r.textContent="ERROR",r.style.color="#f87171"),i&&(i.style.background="#ef4444",i.style.boxShadow="0 0 8px rgba(239,68,68,0.7)")):E.some(s=>s.status==="active")&&r&&(r.textContent="ACTIVE",r.style.color=C.hex,r.style.textShadow=`0 0 10px rgba(${C.rgb},0.5)`);const c=document.getElementById("nf-terminal"),g=c==null?void 0:c.querySelector(".nf-proc-active");g&&c&&g.scrollIntoView({behavior:"smooth",block:"center"})}function Ee(){S&&S.isConnected||(ye(),S=document.createElement("button"),S.id="nf-toggle-btn",S.className="nf-toggle-visible",S.innerHTML=X?"⚡":"✕",S.title="ซ่อน/แสดง Netflow Overlay",S.onclick=()=>ie(),document.body.appendChild(S))}function ie(){P&&(Ee(),X?(P.classList.remove("nf-hidden"),P.classList.add("nf-visible"),S&&(S.innerHTML="✕"),X=!1):(P.classList.remove("nf-visible"),P.classList.add("nf-hidden"),S&&(S.innerHTML="⚡"),X=!0))}const ue={red:"themes/theme-red.jpg",yellow:"themes/theme-yellow.jpg",blue:"themes/theme-blue.jpg",purple:"themes/theme-purple.jpg",green:"themes/theme-green.jpg"};function Ie(){const t=document.getElementById("nf-core-monitor");if(!t)return;let e=K;if(!e)try{e=localStorage.getItem("netflow_app_theme")||"red"}catch{e="red"}const a=ue[e]||ue.red;let r;try{r=chrome.runtime.getURL(a)}catch{r=`/${a}`}const i=C.rgb;t.style.backgroundImage=["linear-gradient(180deg, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.78) 45%, rgba(0,0,0,0.90) 100%)",`linear-gradient(180deg, rgba(${i},0.25) 0%, rgba(${i},0.12) 50%, rgba(${i},0.20) 100%)`,`url('${r}')`].join(", "),t.style.backgroundSize="auto, auto, cover",t.style.backgroundPosition="center, center, center",t.style.backgroundRepeat="no-repeat, no-repeat, no-repeat",t.style.setProperty("--nf-bg-set","1"),t.style.border=`1.5px solid rgba(${i},0.45)`,t.style.boxShadow=`0 0 70px rgba(${i},0.22), 0 0 140px rgba(${i},0.1), inset 0 1px 0 rgba(${i},0.15)`}function De(t=1){if(C=Te(),we(),P&&P.isConnected){X&&ie();return}if(P&&!P.isConnected&&(P=null),_&&(_.remove(),_=null),ye(),ee=t,E=oe(t),t>1){const e=U.find(r=>r.id==="video");if(e){const r=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:"waiting",progress:0}];for(let i=2;i<=t;i++)r.push({id:`scene${i}-prompt`,label:`Scene ${i} Prompt`,status:"waiting"}),r.push({id:`scene${i}-gen`,label:`Scene ${i} Generate`,status:"waiting"}),r.push({id:`scene${i}-wait`,label:`Scene ${i} รอผล`,status:"waiting",progress:0});e.steps=r}const a=U.find(r=>r.id==="render");if(a){const r=a.steps.find(c=>c.id==="download");r&&(r.label="ดาวน์โหลด 720p");const i=a.steps.find(c=>c.id==="upscale");i&&(i.label="Full Video")}}P=Fe(),document.body.appendChild(P),X=!1,Ee(),Re(),_e(),requestAnimationFrame(()=>Ie())}const je={settings:"CONFIG","upload-char":"UPLOAD","upload-prod":"UPLOAD","img-prompt":"PROMPT","img-generate":"GENERATE","img-wait":"IMG WAIT",animate:"ANIMATE","vid-prompt":"VID PROMPT","vid-generate":"VID GEN","vid-wait":"VID WAIT",download:"DOWNLOAD",upscale:"UPSCALE",open:"OPENING"};function Ue(t,e,a){const r=E.filter(o=>o.status==="done").length,i=E.length,c=document.getElementById("nf-stat-step");c&&(c.textContent=`${r}/${i}`);const g=document.getElementById("nf-stat-scenes");if(g&&(g.textContent=ee>1?`1/${ee}`:"1/1"),e==="active"){const o=document.getElementById("nf-stat-status"),s=je[t]||t.toUpperCase();o&&(o.textContent=s)}else if(e==="done"&&r>=i){const o=document.getElementById("nf-stat-status");o&&(o.textContent="COMPLETE")}else if(e==="error"){const o=document.getElementById("nf-stat-status");o&&(o.textContent="ERROR")}}function H(t,e,a){if(!P)return;for(const i of U)for(const c of i.steps)c.id===t&&(c.status=e);for(const i of E)i.stepId===t&&(i.status=e);const r=document.getElementById(`nf-step-${t}`);r&&(r.className="nf-step",e==="active"?r.classList.add("nf-step-active"):e==="done"?r.classList.add("nf-step-done"):e==="error"&&r.classList.add("nf-step-error")),Ue(t,e),Se(),Ce()}function Se(){for(const t of U){const e=t.steps.filter(s=>s.status!=="skipped").length,a=t.steps.filter(s=>s.status==="done").length,r=t.steps.some(s=>s.status==="active"),i=e>0?Math.round(a/e*100):0,c=document.getElementById(`nf-pct-${t.id}`);c&&(c.textContent=`${i}%`);const g=document.getElementById(`nf-modbar-${t.id}`);g&&(g.style.width=`${i}%`);const o=document.getElementById(`nf-mod-${t.id}`);o&&(o.classList.remove("nf-active","nf-done"),i>=100?o.classList.add("nf-done"):r&&o.classList.add("nf-active"))}}function Ye(t){var r,i,c,g;ee=t;const e=new Map;for(const o of E)e.set(o.stepId,{status:o.status,progress:o.progress});E=oe(t);for(const o of E){const s=e.get(o.stepId);s&&(o.status=s.status,s.progress!==void 0&&(o.progress=s.progress))}Ne();{const o=U.find(s=>s.id==="video");if(o){const s=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:((r=o.steps.find(l=>l.id==="animate"))==null?void 0:r.status)||"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:((i=o.steps.find(l=>l.id==="vid-prompt"))==null?void 0:i.status)||"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:((c=o.steps.find(l=>l.id==="vid-generate"))==null?void 0:c.status)||"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:((g=o.steps.find(l=>l.id==="vid-wait"))==null?void 0:g.status)||"waiting",progress:0}];for(let l=2;l<=t;l++)s.push({id:`scene${l}-prompt`,label:`Scene ${l} Prompt`,status:"waiting"}),s.push({id:`scene${l}-gen`,label:`Scene ${l} Generate`,status:"waiting"}),s.push({id:`scene${l}-wait`,label:`Scene ${l} รอผล`,status:"waiting",progress:0});o.steps=s,$e(o)}}const a=U.find(o=>o.id==="render");if(a&&t>1){const o=a.steps.find(l=>l.id==="download");o&&(o.label="ดาวน์โหลด 720p");const s=a.steps.find(l=>l.id==="upscale");s&&(s.label="Full Video"),$e(a)}Se(),Ce()}function $e(t){const e=document.getElementById(`nf-mod-${t.id}`);if(!e)return;e.querySelectorAll(".nf-step, .nf-mod-progress").forEach(i=>i.remove()),t.steps.forEach(i=>{const c=document.createElement("div");c.className="nf-step",c.id=`nf-step-${i.id}`;let g="";i.progress!==void 0&&(g=`
                <div class="nf-progress-bar">
                    <div class="nf-progress-fill" id="nf-bar-${i.id}" style="width: 0%"></div>
                </div>
            `),c.innerHTML=`
            ...
            <span class="nf-step-label">${i.label}</span>
            ${g}
        `,e.appendChild(c)});const r=document.createElement("div");r.className="nf-mod-progress",r.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${t.id}"></div>`,e.appendChild(r)}function Y(t){t.replace(/^\[Netflow AI\]\s*/,"")}window.chrome={runtime:{sendMessage:()=>{},lastError:null},storage:{local:{get:(t,e)=>e==null?void 0:e({}),set:()=>{}}}};const se=new URLSearchParams(location.search).get("theme")||"red";Pe(se);Ye(2);De();setTimeout(()=>{Y("🚀 Overlay preview started — theme: "+se),H("settings","active")},400);setTimeout(()=>{H("settings","done"),H("upload","active"),Y("✅ Settings configured")},1600);setTimeout(()=>{H("upload","done"),H("prompt","active"),Y("✅ Images uploaded (2 refs)")},3e3);setTimeout(()=>{H("prompt","done"),H("generate","active"),Y("✅ Prompt injected"),Y("⏳ Waiting for Scene 1 generation...")},4500);setTimeout(()=>{H("generate","done"),H("scene2","active"),Y("✅ Scene 1 generated"),Y("⏳ Generating Scene 2...")},7e3);const le=document.createElement("div");le.style.cssText=`
    position: fixed; bottom: 16px; right: 16px; z-index: 99999999;
    display: flex; gap: 8px; align-items: center;
`;const Ve=["red","blue","green","yellow","purple"],Q={red:"#ef4444",blue:"#3b82f6",green:"#00ff41",yellow:"#eab308",purple:"#8b5cf6"};Ve.forEach(t=>{const e=document.createElement("button");e.textContent=t,e.style.cssText=`
        ...
        ${t===se?"opacity:1; box-shadow: 0 0 8px "+Q[t]:"opacity:0.5"};

...
@media (max-height: 600px) {
    ...
    `,e.appendChild(a),n.steps.forEach(r=>{const l=document.createElement("div");l.className="nf-step",l.id=`nf-step-${r.id}`;let x="";r.progress!==void 0&&(x=`
            ...
            ${x}
        `,e.appendChild(l)});const o=document.createElement("div");return o.className="nf-mod-progress",o.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${n.id}"></div>`,e.appendChild(o),e}function Te(){re=Date.now(),setInterval(()=>{const n=Math.floor((Date.now()-re)/1e3),e=String(Math.floor(n/60)).padStart(2,"0"),a=String(n%60).padStart(2,"0"),o=document.getElementById("nf-timer");o&&(o.textContent=`${e}:${a}`);const r=document.getElementById("nf-stat-elapsed");r&&(r.textContent=`${e}:${a}`)},1e3)}const Pe=50,se=120,pe=.4;let N=null,le=0,de=0,ce=0,Y=[];function Ae(n,e){Y=[];for(let a=0;a<Pe;a++){const o=Math.random();let r;o<.22?r=0:o<.4?r=1:o<.55?r=2:o<.68?r=3:o<.84?r=4:r=5;const l=Math.random()*n,x=Math.random()*e,i=50+Math.random()*220,p=Math.random()*Math.PI*2,s=(.003+Math.random()*.008)*(Math.random()>.5?1:-1);Y.push({x:r===0?Math.random()*n:l+Math.cos(p)*i,y:r===0?Math.random()*e:x+Math.sin(p)*i,vx:(Math.random()-.5)*pe,vy:(Math.random()-.5)*pe,radius:1.2+Math.random()*2.5,pulsePhase:Math.random()*Math.PI*2,pulseSpeed:.02+Math.random()*.03,motion:r,oCx:l,oCy:x,oRadius:i,oAngle:p,oSpeed:s})}}function ye(){if(!T)return;const n=T;if(F=n.getContext("2d"),!F)return;const e=()=>{n.width=window.innerWidth,n.height=window.innerHeight,Y.length===0&&Ae(n.width,n.height)};e(),window.addEventListener("resize",e);let a=null,o=0,r=0,l=0;function x(){if(!F||!T){B=null;return}if(B=requestAnimationFrame(x),V++,l++,l%3!==0)return;const i=F,p=T.width,s=T.height;i.fillStyle=`rgba(${C*.04|0},${E*.04|0},${S*.06|0},1)`,i.fillRect(0,0,p,s),(!a||o!==p||r!==s)&&(o=p,r=s,a=i.createRadialGradient(p*.5,s*.5,0,p*.5,s*.5,Math.max(p,s)*.6),a.addColorStop(0,`rgba(${C*.08|0},${E*.08|0},${S*.1|0},0.4)`),a.addColorStop(1,"rgba(0,0,0,0)")),i.fillStyle=a,i.fillRect(0,0,p,s);const m=Y,d=m.length,h=se*se;for(let f=0;f<d;f++){const t=m[f];if(t.pulsePhase+=t.pulseSpeed,t.motion===0)t.x+=t.vx,t.y+=t.vy,t.x<0?(t.x=0,t.vx=Math.abs(t.vx)*(.8+Math.random()*.4)):t.x>p&&(t.x=p,t.vx=-Math.abs(t.vx)*(.8+Math.random()*.4)),t.y<0?(t.y=0,t.vy=Math.abs(t.vy)*(.8+Math.random()*.4)):t.y>s&&(t.y=s,t.vy=-Math.abs(t.vy)*(.8+Math.random()*.4));else if(t.motion===1)t.oAngle+=t.oSpeed,t.x=t.oCx+Math.cos(t.oAngle)*t.oRadius,t.y=t.oCy+Math.sin(t.oAngle)*t.oRadius,t.oCx+=Math.sin(t.oAngle*.3)*.15,t.oCy+=Math.cos(t.oAngle*.3)*.15;else if(t.motion===2)t.oAngle+=t.oSpeed,t.x=t.oCx+Math.cos(t.oAngle)*t.oRadius,t.y=t.oCy+Math.sin(t.oAngle)*t.oRadius*.5,t.oCx+=Math.sin(t.oAngle*.2)*.1,t.oCy+=Math.cos(t.oAngle*.2)*.1;else if(t.motion===3){t.oAngle+=t.oSpeed;const g=t.oAngle,c=t.oRadius*.7;t.x=t.oCx+c*Math.cos(g),t.y=t.oCy+c*Math.sin(g)*Math.cos(g),t.oCx+=Math.sin(g*.15)*.12,t.oCy+=Math.cos(g*.15)*.12}else if(t.motion===4){t.oAngle+=t.oSpeed*1.2;const g=t.oRadius*(.5+.5*Math.abs(Math.sin(t.oAngle*.15)));t.x=t.oCx+Math.cos(t.oAngle)*g,t.y=t.oCy+Math.sin(t.oAngle)*g,t.oCx+=Math.sin(t.oAngle*.1)*.18,t.oCy+=Math.cos(t.oAngle*.1)*.18}else t.oAngle+=t.oSpeed,t.x+=t.vx*.8,t.y=t.oCy+Math.sin(t.oAngle+t.x*.008)*t.oRadius*.35,t.x<-30?t.x=p+30:t.x>p+30&&(t.x=-30),t.oCy+=Math.sin(t.oAngle*.1)*.08;if(t.motion>0){const g=t.oRadius+50;t.oCx<-g?t.oCx=p+g:t.oCx>p+g&&(t.oCx=-g),t.oCy<-g?t.oCy=s+g:t.oCy>s+g&&(t.oCy=-g)}}i.beginPath(),i.strokeStyle=`rgba(${C},${E},${S},0.06)`,i.lineWidth=.4;const k=new Path2D;for(let f=0;f<d;f++){const t=m[f];for(let g=f+1;g<d;g++){const c=m[g],b=t.x-c.x,z=t.y-c.y,R=b*b+z*z;R<h&&(1-R/h<.4?(i.moveTo(t.x,t.y),i.lineTo(c.x,c.y)):(k.moveTo(t.x,t.y),k.lineTo(c.x,c.y)))}}if(i.stroke(),i.strokeStyle=`rgba(${C},${E},${S},0.18)`,i.lineWidth=.8,i.stroke(k),!N||le!==C||de!==E||ce!==S){N=document.createElement("canvas");const f=48;N.width=f,N.height=f;const t=N.getContext("2d"),g=t.createRadialGradient(f/2,f/2,0,f/2,f/2,f/2);g.addColorStop(0,`rgba(${C},${E},${S},0.9)`),g.addColorStop(.3,`rgba(${C},${E},${S},0.35)`),g.addColorStop(1,`rgba(${C},${E},${S},0)`),t.fillStyle=g,t.fillRect(0,0,f,f),le=C,de=E,ce=S}const w=N;for(let f=0;f<d;f++){const t=m[f],g=.6+.4*Math.sin(t.pulsePhase),c=t.radius*5*(.8+g*.4);i.globalAlpha=.5+g*.4,i.drawImage(w,t.x-c/2,t.y-c/2,c,c)}i.globalAlpha=1,i.fillStyle="rgba(255,255,255,0.45)",i.beginPath();for(let f=0;f<d;f++){const t=m[f];if(t.radius>2){const g=.6+.4*Math.sin(t.pulsePhase),c=t.radius*(.8+g*.4)*.35;i.moveTo(t.x+c,t.y),i.arc(t.x,t.y,c,0,Math.PI*2)}}i.fill()}x()}let D=null;const q=560,Le=140,fe=q/2,ge=Le/2,$e=[];for(let n=0;n<=q;n+=8){const e=Math.abs(n-fe)/fe;$e.push(Math.pow(Math.min(1,e*1.6),.6))}const Re=[0,1,2,3].map(n=>({amp:10+n*5,freq:(1.2+n*.35)*Math.PI*2/q,off:n*.6,spd:.7+n*.12}));let K=!1;function Q(){if(_=requestAnimationFrame(Q),K=!K,K)return;if(J+=.07,!D){const e=document.getElementById("nf-engine-waves");if(!e){_=null;return}D=Array.from(e.querySelectorAll("path"))}const n=[];for(let e=0;e<D.length;e++){const a=Re[e],o=J*a.spd+a.off;n.length=0,n.push(`M 0 ${ge}`);let r=0;for(let l=0;l<=q;l+=8){const x=ge+a.amp*$e[r++]*Math.sin(l*a.freq+o);n.push(`L${l} ${x*10+.5|0}`)}D[e].setAttribute("d",n.join(" "))}}function Ne(){J=0,V=0,X=performance.now(),Q(),ye(),setInterval(()=>{const n=performance.now(),e=(n-X)/1e3;if(e>0){oe=Math.round(V/e);const a=document.getElementById("nf-stat-fps");a&&(a.textContent=`${oe}`)}V=0,X=n},1e3)}function ee(){let n=0;const e=$.filter(s=>s.status!=="skipped").length;for(const s of $){const m=document.getElementById(`nf-proc-${s.stepId}`);if(!m)continue;m.className="nf-proc-row";const d=m.querySelector(".nf-proc-badge");switch(s.status){case"done":m.classList.add("nf-proc-done"),d&&(d.textContent="✅ done"),n++;break;case"active":m.classList.add("nf-proc-active"),d&&(d.textContent=s.progress!==void 0&&s.progress>0?`⏳ ${s.progress}%`:"⏳ active");break;case"error":m.classList.add("nf-proc-error"),d&&(d.textContent="❌ error");break;case"skipped":m.classList.add("nf-proc-skipped"),d&&(d.textContent="— skip");break;default:m.classList.add("nf-proc-waiting"),d&&(d.textContent="(queued)")}}const a=$.findIndex(s=>s.status==="active"),o=a>=0?a+1:n>=e&&e>0?$.length:n,r=document.getElementById("nf-step-counter");r&&(r.textContent=`${o}/${$.length}`);const l=document.querySelector(".nf-core-title-val"),x=document.querySelector(".nf-status-dot");n>=e&&e>0?(l&&(l.textContent="COMPLETE",l.style.color=v.doneHex),x&&(x.style.background=v.doneHex,x.style.boxShadow=`0 0 8px rgba(${v.doneRgb},0.7)`)):$.some(m=>m.status==="error")?(l&&(l.textContent="ERROR",l.style.color="#f87171"),x&&(x.style.background="#ef4444",x.style.boxShadow="0 0 8px rgba(239,68,68,0.7)")):$.some(m=>m.status==="active")&&l&&(l.textContent="ACTIVE",l.style.color=v.hex,l.style.textShadow=`0 0 10px rgba(${v.rgb},0.5)`);const i=document.getElementById("nf-terminal"),p=i==null?void 0:i.querySelector(".nf-proc-active");p&&i&&p.scrollIntoView({behavior:"smooth",block:"center"})}function ve(){M&&M.isConnected||(W(),M=document.createElement("button"),M.id="nf-toggle-btn",M.className="nf-toggle-visible",M.innerHTML=O?be:he,M.title="ซ่อน/แสดง Netflow Overlay",M.style.cssText="position:fixed !important;top:20px !important;right:20px !important;z-index:2147483647 !important;width:48px !important;height:48px !important;border-radius:50% !important;border:2px solid rgba(255,255,255,0.5) !important;background:rgba(0,0,0,0.85) !important;color:#fff !important;font-size:23px !important;cursor:pointer !important;display:flex !important;align-items:center !important;justify-content:center !important;pointer-events:auto !important;",M.onclick=()=>ke(),document.body.appendChild(M))}function ke(){u&&(ve(),O?(u.classList.remove("nf-hidden"),u.classList.add("nf-visible"),u.style.opacity="1",u.style.pointerEvents="auto",M&&(M.innerHTML=he),O=!1,F&&T&&B===null&&ye(),_===null&&Q()):(u.classList.remove("nf-visible"),u.classList.add("nf-hidden"),u.style.opacity="0",u.style.pointerEvents="none",M&&(M.innerHTML=be),O=!0,B!==null&&(cancelAnimationFrame(B),B=null),_!==null&&(cancelAnimationFrame(_),_=null)))}const xe={red:"themes/theme-red.jpg",yellow:"themes/theme-yellow.jpg",blue:"themes/theme-blue.jpg",purple:"themes/theme-purple.jpg",green:"themes/theme-green.jpg"};function Me(){const n=document.getElementById("nf-core-monitor");if(!n)return;let e=G;if(!e)try{e=localStorage.getItem("netflow_app_theme")||"blue"}catch{e="blue"}const a=xe[e]||xe.blue;let o;try{o=chrome.runtime.getURL(a)}catch{o=`/${a}`}const r=v.rgb;n.style.backgroundImage=["linear-gradient(180deg, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.78) 45%, rgba(0,0,0,0.90) 100%)",`linear-gradient(180deg, rgba(${r},0.25) 0%, rgba(${r},0.12) 50%, rgba(${r},0.20) 100%)`,`url('${o}')`].join(", "),n.style.backgroundSize="auto, auto, 50%",n.style.backgroundPosition="center, center, center",n.style.backgroundRepeat="no-repeat, no-repeat, no-repeat",n.style.setProperty("--nf-bg-set","1"),n.style.border=`1.5px solid rgba(${r},0.45)`,n.style.boxShadow=`0 0 70px rgba(${r},0.22), 0 0 140px rgba(${r},0.1), inset 0 1px 0 rgba(${r},0.15)`}function _e(n=1){if(v=Ee(),me(),u&&u.isConnected){u.style.cssText="position:fixed !important;inset:0 !important;z-index:2147483647 !important;pointer-events:auto !important;opacity:1 !important;background:#0a0a0f !important;overflow:hidden !important;font-family:Inter,system-ui,-apple-system,sans-serif !important;",(!y||!y.isConnected)&&(y=null,W()),setTimeout(()=>{if(u)try{y!=null&&y.sheet&&y.sheet.cssRules.length>0&&(u.style.removeProperty("background"),u.style.removeProperty("font-family"),u.style.removeProperty("overflow"))}catch{}},200);for(const e of I)for(const a of e.steps)a.status="waiting",a.progress=a.progress!==void 0?0:void 0;H=n,$=U(n),we();for(const e of I)Z(e);if(te(),ee(),!u.querySelector(".nf-stop-btn")){const e=document.createElement("button");e.className="nf-stop-btn",e.innerHTML='<span class="nf-stop-icon"></span> หยุด',e.style.cssText="position:absolute !important;top:14px !important;right:110px !important;z-index:2147483646 !important;cursor:pointer !important;pointer-events:auto !important;background:rgba(255,60,60,0.08) !important;border:1px solid rgba(255,60,60,0.25) !important;border-radius:8px !important;color:rgba(255,100,100,0.8) !important;font-size:13px !important;padding:6px 14px !important;font-family:inherit !important;display:flex !important;align-items:center !important;gap:6px !important;",e.onclick=()=>{var a;window.__NETFLOW_STOP__=!0;try{A("⛔ ผู้ใช้หยุดการทำงาน")}catch{}try{typeof chrome<"u"&&((a=chrome.runtime)!=null&&a.sendMessage)&&chrome.runtime.sendMessage({action:"AUTOMATION_STOPPED"})}catch{}},u.appendChild(e)}O&&ke();return}u&&!u.isConnected&&(u=null),y&&(y.remove(),y=null),W();for(const e of I)for(const a of e.steps)a.status="waiting",a.progress=a.progress!==void 0?0:void 0;if(H=n,$=U(n),n>1){const e=I.find(o=>o.id==="video");if(e){const o=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:"waiting",progress:0}];for(let r=2;r<=n;r++)o.push({id:`scene${r}-prompt`,label:`Scene ${r} Prompt`,status:"waiting"}),o.push({id:`scene${r}-gen`,label:`Scene ${r} Generate`,status:"waiting"}),o.push({id:`scene${r}-wait`,label:`Scene ${r} รอผล`,status:"waiting",progress:0});e.steps=o}const a=I.find(o=>o.id==="render");if(a){const o=a.steps.find(l=>l.id==="download");o&&(o.label="ดาวน์โหลด 720p");const r=a.steps.find(l=>l.id==="upscale");r&&(r.label="Full Video")}}u=ze(),u.style.cssText="position:fixed !important;inset:0 !important;z-index:2147483647 !important;pointer-events:auto !important;opacity:1 !important;background:#0a0a0f !important;overflow:hidden !important;font-family:Inter,system-ui,-apple-system,sans-serif !important;",document.body.appendChild(u),u.classList.add("nf-visible"),O=!1,ve(),Te(),Ne(),requestAnimationFrame(()=>Me()),setTimeout(()=>{if(u)try{y!=null&&y.sheet&&y.sheet.cssRules.length>0&&(u.style.removeProperty("background"),u.style.removeProperty("font-family"),u.style.removeProperty("overflow"))}catch{}},200)}const Be={settings:"SETTINGS","upload-char":"UPLOAD","upload-prod":"UPLOAD","img-prompt":"PROMPT","img-generate":"GENERATE","img-wait":"IMG WAIT",animate:"ANIMATE","vid-prompt":"VID PROMPT","vid-generate":"VID GEN","vid-wait":"VID WAIT",download:"DOWNLOAD",upscale:"UPSCALE",open:"OPENING"};function Oe(n,e,a){const o=$.findIndex(d=>d.status==="active"),r=$.filter(d=>d.status==="done").length,l=$.length,x=o>=0?o+1:r>=l?l:r,i=document.getElementById("nf-stat-step");i&&(i.textContent=`${x}/${l}`);let p=1;for(const d of $)if(d.status==="active"||d.status==="done")if(d.stepId.startsWith("scene")){const h=d.stepId.match(/^scene(\d+)-/);h&&(p=Math.max(p,parseInt(h[1],10)))}else(d.stepId==="download"||d.stepId==="upscale"||d.stepId==="open")&&(p=H);const s=document.getElementById("nf-stat-scenes");if(s&&(s.textContent=H>1?`${p}/${H}`:"1/1"),e==="active"){const d=document.getElementById("nf-stat-status"),h=Be[n]||n.toUpperCase();d&&(d.textContent=h)}else if(e==="done"&&r>=l){const d=document.getElementById("nf-stat-status");d&&(d.textContent="COMPLETE")}else if(e==="error"){const d=document.getElementById("nf-stat-status");d&&(d.textContent="ERROR")}const m=document.getElementById("nf-stat-progress");m&&e==="active"&&(m.textContent="—")}function P(n,e,a){if(!u)return;for(const r of I)for(const l of r.steps)l.id===n&&(l.status=e);for(const r of $)r.stepId===n&&(r.status=e);const o=document.getElementById(`nf-step-${n}`);o&&(o.className="nf-step",e==="active"?o.classList.add("nf-step-active"):e==="done"?o.classList.add("nf-step-done"):e==="error"&&o.classList.add("nf-step-error")),Oe(n,e),te(),ee()}function te(){for(const n of I){const e=n.steps.filter(p=>p.status!=="skipped").length,a=n.steps.filter(p=>p.status==="done").length,o=n.steps.some(p=>p.status==="active"),r=e>0?Math.round(a/e*100):0,l=document.getElementById(`nf-pct-${n.id}`);l&&(l.textContent=`${r}%`);const x=document.getElementById(`nf-modbar-${n.id}`);x&&(x.style.width=`${r}%`);const i=document.getElementById(`nf-mod-${n.id}`);i&&(i.classList.remove("nf-active","nf-done"),r>=100?i.classList.add("nf-done"):o&&i.classList.add("nf-active"))}}function He(n){var o,r,l,x;H=n;const e=new Map;for(const i of $)e.set(i.stepId,{status:i.status,progress:i.progress});$=U(n);for(const i of $){const p=e.get(i.stepId);p&&(i.status=p.status,p.progress!==void 0&&(i.progress=p.progress))}we();{const i=I.find(p=>p.id==="video");if(i){const p=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:((o=i.steps.find(s=>s.id==="animate"))==null?void 0:o.status)||"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:((r=i.steps.find(s=>s.id==="vid-prompt"))==null?void 0:r.status)||"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:((l=i.steps.find(s=>s.id==="vid-generate"))==null?void 0:l.status)||"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:((x=i.steps.find(s=>s.id==="vid-wait"))==null?void 0:x.status)||"waiting",progress:0}];for(let s=2;s<=n;s++)p.push({id:`scene${s}-prompt`,label:`Scene ${s} Prompt`,status:"waiting"}),p.push({id:`scene${s}-gen`,label:`Scene ${s} Generate`,status:"waiting"}),p.push({id:`scene${s}-wait`,label:`Scene ${s} รอผล`,status:"waiting",progress:0});i.steps=p,Z(i)}}const a=I.find(i=>i.id==="render");if(a&&n>1){const i=a.steps.find(s=>s.id==="download");i&&(i.label="ดาวน์โหลด 720p");const p=a.steps.find(s=>s.id==="upscale");p&&(p.label="Full Video"),Z(a)}te(),ee()}function Z(n){const e=document.getElementById(`nf-mod-${n.id}`);if(!e)return;e.querySelectorAll(".nf-step, .nf-mod-progress").forEach(r=>r.remove()),n.steps.forEach(r=>{const l=document.createElement("div");l.className="nf-step",l.id=`nf-step-${r.id}`;let x="";r.progress!==void 0&&(x=`
                <div class="nf-progress-bar">
                    <div class="nf-progress-fill" id="nf-bar-${r.id}" style="width: 0%"></div>
                </div>
            `),l.innerHTML=`
            ...
            <span class="nf-step-label">${r.label}</span>
            ${x}
        `,e.appendChild(l)});const o=document.createElement("div");o.className="nf-mod-progress",o.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${n.id}"></div>`,e.appendChild(o)}function A(n){n.replace(/^\[Netflow AI\]\s*/,"")}window.chrome={runtime:{sendMessage:()=>{},lastError:null},storage:{local:{get:(n,e)=>e==null?void 0:e({}),set:()=>{}}}};const ne=new URLSearchParams(location.search).get("theme")||"red";Ce(ne);He(2);_e();setTimeout(()=>{A("🚀 Overlay preview started — theme: "+ne),P("settings","active")},400);setTimeout(()=>{P("settings","done"),P("upload","active"),A("✅ Settings configured")},1600);setTimeout(()=>{P("upload","done"),P("prompt","active"),A("✅ Images uploaded (2 refs)")},3e3);setTimeout(()=>{P("prompt","done"),P("generate","active"),A("✅ Prompt injected"),A("⏳ Waiting for Scene 1 generation...")},4500);setTimeout(()=>{P("generate","done"),P("scene2","active"),A("✅ Scene 1 generated"),A("⏳ Generating Scene 2...")},7e3);const ae=document.createElement("div");ae.style.cssText=`
    position: fixed; bottom: 16px; right: 16px; z-index: 99999999;
    display: flex; gap: 8px; align-items: center;
`;const Fe=["red","blue","green","yellow","purple"],j={red:"#ef4444",blue:"#3b82f6",green:"#00ff41",yellow:"#eab308",purple:"#8b5cf6"};Fe.forEach(n=>{const e=document.createElement("button");e.textContent=n,e.style.cssText=`
        background: ${j[n]}22; border: 1.5px solid ${j[n]};
        color: ${j[n]}; padding: 4px 10px; border-radius: 6px;
        cursor: pointer; font-size: 11px; font-family: monospace; font-weight: 700;
        ${n===ne?"opacity:1; box-shadow: 0 0 8px "+j[n]:"opacity:0.5"};
    `,e.onclick=()=>location.search="?theme="+n,ae.appendChild(e)});document.body.appendChild(ae);

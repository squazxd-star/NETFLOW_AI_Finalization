...
.nf-proc-skipped {
    ...
    `}function pe(){G||(G=document.createElement("style"),G.id="netflow-overlay-styles",G.textContent=ye(E),document.head.appendChild(G))}function fe(n){n.innerHTML="",I.forEach((e,i)=>{const r=document.createElement("div");r.className="nf-proc-row nf-proc-waiting",r.id=`nf-proc-${e.stepId}`,r.innerHTML=`
            <span class="nf-proc-num">${i+1}</span>
            <span class="nf-proc-dot"></span>
            <span class="nf-proc-label">${e.label}</span>
            <span class="nf-proc-badge">(queued)</span>
        `,n.appendChild(r)})}function ve(){const n=document.getElementById("nf-terminal");if(!n)return;fe(n);const e=document.getElementById("nf-step-counter");e&&(e.textContent=`0/${I.length}`)}function ie(n,e){let o="";for(let c=0;c<32;c++){const m=c/32*Math.PI*2,h=(c+.2)/32*Math.PI*2,u=(c+.5)/32*Math.PI*2,y=(c+.8)/32*Math.PI*2,v=(c+1)/32*Math.PI*2;o+=`${c===0?"M":"L"}${(120+104*Math.cos(m)).toFixed(1)},${(120+104*Math.sin(m)).toFixed(1)} `,o+=`L${(120+104*Math.cos(h)).toFixed(1)},${(120+104*Math.sin(h)).toFixed(1)} `,o+=`L${(120+116*Math.cos(u)).toFixed(1)},${(120+116*Math.sin(u)).toFixed(1)} `,o+=`L${(120+104*Math.cos(y)).toFixed(1)},${(120+104*Math.sin(y)).toFixed(1)} `,o+=`L${(120+104*Math.cos(v)).toFixed(1)},${(120+104*Math.sin(v)).toFixed(1)} `}o+="Z";const s=24,d=100,$=90;let M="";for(let c=0;c<s;c++){const m=c/s*Math.PI*2,h=(c+.25)/s*Math.PI*2,u=(c+.75)/s*Math.PI*2,y=(c+1)/s*Math.PI*2;M+=`${c===0?"M":"L"}${(120+$*Math.cos(m)).toFixed(1)},${(120+$*Math.sin(m)).toFixed(1)} `,M+=`L${(120+d*Math.cos(h)).toFixed(1)},${(120+d*Math.sin(h)).toFixed(1)} `,M+=`L${(120+d*Math.cos(u)).toFixed(1)},${(120+d*Math.sin(u)).toFixed(1)} `,M+=`L${(120+$*Math.cos(y)).toFixed(1)},${(120+$*Math.sin(y)).toFixed(1)} `}M+="Z";let C="";for(let c=0;c<64;c++){const m=c/64*Math.PI*2,h=c%4===0?117:119,u=c%4===0?124:122,y=c%4===0?.8:.4,v=c%4===0?.7:.35;C+=`<line x1="${(120+h*Math.cos(m)).toFixed(1)}" y1="${(120+h*Math.sin(m)).toFixed(1)}" x2="${(120+u*Math.cos(m)).toFixed(1)}" y2="${(120+u*Math.sin(m)).toFixed(1)}" stroke="rgba(${n},${v})" stroke-width="${y}"/>`}const x=26,t=78,p=68;let w="";for(let c=0;c<x;c++){const m=c/x*Math.PI*2,h=(c+.2)/x*Math.PI*2,u=(c+.5)/x*Math.PI*2,y=(c+.8)/x*Math.PI*2,v=(c+1)/x*Math.PI*2;w+=`${c===0?"M":"L"}${(120+p*Math.cos(m)).toFixed(1)},${(120+p*Math.sin(m)).toFixed(1)} `,w+=`L${(120+p*Math.cos(h)).toFixed(1)},${(120+p*Math.sin(h)).toFixed(1)} `,w+=`L${(120+t*Math.cos(u)).toFixed(1)},${(120+t*Math.sin(u)).toFixed(1)} `,w+=`L${(120+p*Math.cos(y)).toFixed(1)},${(120+p*Math.sin(y)).toFixed(1)} `,w+=`L${(120+p*Math.cos(v)).toFixed(1)},${(120+p*Math.sin(v)).toFixed(1)} `}w+="Z";let g="";for(let c=0;c<48;c++){const m=c/48*Math.PI*2,h=c%4===0?79:80,u=c%4===0?85:83,y=c%4===0?.6:.3,v=c%4===0?.6:.3;g+=`<line x1="${(120+h*Math.cos(m)).toFixed(1)}" y1="${(120+h*Math.sin(m)).toFixed(1)}" x2="${(120+u*Math.cos(m)).toFixed(1)}" y2="${(120+u*Math.sin(m)).toFixed(1)}" stroke="rgba(${e},${v})" stroke-width="${y}"/>`}function b(c,m,h,u,y){let v="";for(let L=0;L<h;L++){const te=L/h*Math.PI*2,ne=(L+.25)/h*Math.PI*2,ae=(L+.75)/h*Math.PI*2,oe=(L+1)/h*Math.PI*2;v+=`${L===0?"M":"L"}${(c+y*Math.cos(te)).toFixed(1)},${(m+y*Math.sin(te)).toFixed(1)} `,v+=`L${(c+u*Math.cos(ne)).toFixed(1)},${(m+u*Math.sin(ne)).toFixed(1)} `,v+=`L${(c+u*Math.cos(ae)).toFixed(1)},${(m+u*Math.sin(ae)).toFixed(1)} `,v+=`L${(c+y*Math.cos(oe)).toFixed(1)},${(m+y*Math.sin(oe)).toFixed(1)} `}return v+"Z"}const k=42,F=[],U=b(120,120,14,18,13);F.push(`<g class="nf-kinetic-sub" style="transform-origin:120px 120px">
    ...
    </g>`);for(let c=0;c<8;c++){const m=c/8*Math.PI*2,h=120+k*Math.cos(m),u=120+k*Math.sin(m),v=b(h,u,10,14,10),L=c%2===0?"":"animation-direction:reverse;";F.push(`<g class="nf-kinetic-sub" style="transform-origin:${h.toFixed(1)}px ${u.toFixed(1)}px;${L}">
...
`);let D="";for(let c=0;c<8;c++){const m=c/8*Math.PI*2,h=120+10*Math.cos(m),u=120+10*Math.sin(m),y=120+(k-10)*Math.cos(m),v=120+(k-10)*Math.sin(m);D+=`<line x1="${h.toFixed(1)}" y1="${u.toFixed(1)}" x2="${y.toFixed(1)}" y2="${v.toFixed(1)}" stroke="rgba(${e},0.25)" stroke-width="0.5"/>`}return`<svg width="80" height="80" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
    ...
    </svg>`}function ke(){const n=document.createElement("div");n.id="netflow-engine-overlay",B=document.createElement("canvas"),B.id="nf-matrix-canvas",n.appendChild(B);const e=document.createElement("div");e.className="nf-pat-data",n.appendChild(e);const i=document.createElement("div");i.className="nf-center-glow",n.appendChild(i);const r=document.createElement("div");r.className="nf-pat-noise",n.appendChild(r);const a=document.createElement("div");a.className="nf-vignette",n.appendChild(a);for(let g=0;g<3;g++){const b=document.createElement("div");b.className="nf-pulse-ring",n.appendChild(b)}["nf-deco-tl","nf-deco-tr","nf-deco-bl","nf-deco-br"].forEach(g=>{const b=document.createElement("div");b.className=`nf-corner-deco ${g}`,n.appendChild(b)});const f=document.createElement("button");f.className="nf-close-btn",f.textContent="✕ ซ่อน",f.onclick=()=>me(),n.appendChild(f);const l=document.createElement("div");l.className="nf-layout";const o=document.createElement("div");o.className="nf-core-monitor",o.id="nf-core-monitor";const s=document.createElement("div");s.className="nf-core-header",s.innerHTML=`
    ...
    `,e.appendChild(i),n.steps.forEach(a=>{const f=document.createElement("div");f.className="nf-step",f.id=`nf-step-${a.id}`;let l="";a.progress!==void 0&&(l=`
            ...
            ${l}
        `,e.appendChild(f)});const r=document.createElement("div");return r.className="nf-mod-progress",r.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${n.id}"></div>`,e.appendChild(r),e}function Ce(){re=Date.now(),setInterval(()=>{const n=Math.floor((Date.now()-re)/1e3),e=String(Math.floor(n/60)).padStart(2,"0"),i=String(n%60).padStart(2,"0"),r=document.getElementById("nf-timer");r&&(r.textContent=`${e}:${i}`);const a=document.getElementById("nf-stat-elapsed");a&&(a.textContent=`${e}:${i}`)},1e3)}const Ee=220,K=210,se=.4;let X=[];function Ie(n,e){X=[];for(let i=0;i<Ee;i++){const r=Math.random();let a;r<.22?a=0:r<.4?a=1:r<.55?a=2:r<.68?a=3:r<.84?a=4:a=5;const f=Math.random()*n,l=Math.random()*e,o=50+Math.random()*220,s=Math.random()*Math.PI*2,d=(.003+Math.random()*.008)*(Math.random()>.5?1:-1);X.push({x:a===0?Math.random()*n:f+Math.cos(s)*o,y:a===0?Math.random()*e:l+Math.sin(s)*o,vx:(Math.random()-.5)*se,vy:(Math.random()-.5)*se,radius:1.2+Math.random()*2.5,pulsePhase:Math.random()*Math.PI*2,pulseSpeed:.02+Math.random()*.03,motion:a,oCx:f,oCy:l,oRadius:o,oAngle:s,oSpeed:d})}}function Se(){if(!B)return;const n=B;if(Y=n.getContext("2d"),!Y)return;const e=()=>{n.width=window.innerWidth,n.height=window.innerHeight,X.length===0&&Ie(n.width,n.height)};e(),window.addEventListener("resize",e);let i=null,r=0,a=0;function f(){if(!Y||!B)return;requestAnimationFrame(f);const l=Y,o=B.width,s=B.height;l.fillStyle=`rgba(${T*.04|0},${R*.04|0},${A*.06|0},1)`,l.fillRect(0,0,o,s),(!i||r!==o||a!==s)&&(r=o,a=s,i=l.createRadialGradient(o*.5,s*.5,0,o*.5,s*.5,Math.max(o,s)*.6),i.addColorStop(0,`rgba(${T*.08|0},${R*.08|0},${A*.1|0},0.4)`),i.addColorStop(1,"rgba(0,0,0,0)")),l.fillStyle=i,l.fillRect(0,0,o,s);const d=X,$=d.length,M=K*K;for(let x=0;x<$;x++){const t=d[x];if(t.pulsePhase+=t.pulseSpeed,t.motion===0)t.x+=t.vx,t.y+=t.vy,t.x<0?(t.x=0,t.vx=Math.abs(t.vx)*(.8+Math.random()*.4)):t.x>o&&(t.x=o,t.vx=-Math.abs(t.vx)*(.8+Math.random()*.4)),t.y<0?(t.y=0,t.vy=Math.abs(t.vy)*(.8+Math.random()*.4)):t.y>s&&(t.y=s,t.vy=-Math.abs(t.vy)*(.8+Math.random()*.4));else if(t.motion===1)t.oAngle+=t.oSpeed,t.x=t.oCx+Math.cos(t.oAngle)*t.oRadius,t.y=t.oCy+Math.sin(t.oAngle)*t.oRadius,t.oCx+=Math.sin(t.oAngle*.3)*.15,t.oCy+=Math.cos(t.oAngle*.3)*.15;else if(t.motion===2)t.oAngle+=t.oSpeed,t.x=t.oCx+Math.cos(t.oAngle)*t.oRadius,t.y=t.oCy+Math.sin(t.oAngle)*t.oRadius*.5,t.oCx+=Math.sin(t.oAngle*.2)*.1,t.oCy+=Math.cos(t.oAngle*.2)*.1;else if(t.motion===3){t.oAngle+=t.oSpeed;const p=t.oAngle,w=t.oRadius*.7;t.x=t.oCx+w*Math.cos(p),t.y=t.oCy+w*Math.sin(p)*Math.cos(p),t.oCx+=Math.sin(p*.15)*.12,t.oCy+=Math.cos(p*.15)*.12}else if(t.motion===4){t.oAngle+=t.oSpeed*1.2;const p=t.oRadius*(.5+.5*Math.abs(Math.sin(t.oAngle*.15)));t.x=t.oCx+Math.cos(t.oAngle)*p,t.y=t.oCy+Math.sin(t.oAngle)*p,t.oCx+=Math.sin(t.oAngle*.1)*.18,t.oCy+=Math.cos(t.oAngle*.1)*.18}else t.oAngle+=t.oSpeed,t.x+=t.vx*.8,t.y=t.oCy+Math.sin(t.oAngle+t.x*.008)*t.oRadius*.35,t.x<-30?t.x=o+30:t.x>o+30&&(t.x=-30),t.oCy+=Math.sin(t.oAngle*.1)*.08;if(t.motion>0){const p=t.oRadius+50;t.oCx<-p?t.oCx=o+p:t.oCx>o+p&&(t.oCx=-p),t.oCy<-p?t.oCy=s+p:t.oCy>s+p&&(t.oCy=-p)}}const C=5;for(let x=0;x<C;x++){const t=x/C,p=(x+1)/C,w=((t+p)*.5*.35).toFixed(3);l.beginPath(),l.strokeStyle=`rgba(${T},${R},${A},${w})`,l.lineWidth=(t+p)*.5*1.2;for(let g=0;g<$;g++){const b=d[g];for(let k=g+1;k<$;k++){const F=d[k],U=b.x-F.x,V=b.y-F.y,D=U*U+V*V;if(D<M){const c=1-Math.sqrt(D)/K;c>=t&&c<p&&(l.moveTo(b.x,b.y),l.lineTo(F.x,F.y))}}}l.stroke()}l.save(),l.shadowColor=`rgba(${T},${R},${A},0.8)`,l.shadowBlur=25;for(let x=0;x<$;x++){const t=d[x],p=.6+.4*Math.sin(t.pulsePhase),w=t.radius*(.8+p*.4),g=T+(255-T)*.7*p|0,b=R+(255-R)*.7*p|0,k=A+(255-A)*.7*p|0;l.beginPath(),l.arc(t.x,t.y,w,0,Math.PI*2),l.fillStyle=`rgba(${g},${b},${k},${(.6+p*.4).toFixed(2)})`,l.fill()}l.restore(),l.fillStyle="rgba(255,255,255,0.45)",l.beginPath();for(let x=0;x<$;x++){const t=d[x];if(t.radius>2){const p=.6+.4*Math.sin(t.pulsePhase),w=t.radius*(.8+p*.4)*.35;l.moveTo(t.x+w,t.y),l.arc(t.x,t.y,w,0,Math.PI*2)}}l.fill(),l.fillStyle=`rgba(${T},${R},${A},0.08)`;for(let x=0;x<6;x++)l.fillRect(Math.random()*o,Math.random()*s,1,1)}f()}function ge(){requestAnimationFrame(ge),J+=.035;const n=document.getElementById("nf-engine-waves");if(!n)return;const e=560,i=140,r=e/2;n.querySelectorAll("path").forEach(f=>{const l=parseInt(f.getAttribute("data-wave-idx")||"0",10),o=10+l*5,s=1.2+l*.35,d=l*.6,$=.7+l*.12;let M=`M 0 ${i/2}`;for(let C=0;C<=e;C+=3){const x=Math.abs(C-r)/r,t=Math.pow(Math.min(1,x*1.6),.6),p=i/2+o*t*Math.sin(C/e*s*Math.PI*2+J*$+d);M+=` L ${C} ${Math.round(p*10)/10}`}f.setAttribute("d",M)})}function Pe(){J=0,ge(),Se(),setInterval(()=>{const n=document.getElementById("nf-stat-freq"),e=document.getElementById("nf-stat-lat1"),i=document.getElementById("nf-stat-lat2"),r=document.getElementById("nf-stat-buf");n&&(n.textContent=`${(4.5+Math.random()*.5).toFixed(1)} GHz`),e&&(e.textContent=`${Math.floor(12+Math.random()*10)}ms`),i&&(i.textContent=`${Math.floor(12+Math.random()*10)}ms`),r&&(r.textContent=`${Math.floor(90+Math.random()*9)}%`)},2e3)}function be(){let n=0;const e=I.filter(o=>o.status!=="skipped").length;for(const o of I){const s=document.getElementById(`nf-proc-${o.stepId}`);if(!s)continue;s.className="nf-proc-row";const d=s.querySelector(".nf-proc-badge");switch(o.status){case"done":s.classList.add("nf-proc-done"),d&&(d.textContent="✅ done"),n++;break;case"active":s.classList.add("nf-proc-active"),d&&(d.textContent=o.progress!==void 0&&o.progress>0?`⏳ ${o.progress}%`:"⏳ active");break;case"error":s.classList.add("nf-proc-error"),d&&(d.textContent="❌ error");break;case"skipped":s.classList.add("nf-proc-skipped"),d&&(d.textContent="— skip");break;default:s.classList.add("nf-proc-waiting"),d&&(d.textContent="(queued)")}}const i=document.getElementById("nf-step-counter");i&&(i.textContent=`${n}/${I.length}`);const r=document.querySelector(".nf-core-title-val"),a=document.querySelector(".nf-status-dot");n>=e&&e>0?(r&&(r.textContent="COMPLETE",r.style.color=E.doneHex),a&&(a.style.background=E.doneHex,a.style.boxShadow=`0 0 8px rgba(${E.doneRgb},0.7)`)):I.some(s=>s.status==="error")?(r&&(r.textContent="ERROR",r.style.color="#f87171"),a&&(a.style.background="#ef4444",a.style.boxShadow="0 0 8px rgba(239,68,68,0.7)")):I.some(s=>s.status==="active")&&r&&(r.textContent="ACTIVE",r.style.color=E.hex,r.style.textShadow=`0 0 10px rgba(${E.rgb},0.5)`);const f=document.getElementById("nf-terminal"),l=f==null?void 0:f.querySelector(".nf-proc-active");l&&f&&l.scrollIntoView({behavior:"smooth",block:"center"})}function xe(){P&&P.isConnected||(pe(),P=document.createElement("button"),P.id="nf-toggle-btn",P.className="nf-toggle-visible",P.innerHTML=_?"⚡":"✕",P.title="ซ่อน/แสดง Netflow Overlay",P.onclick=()=>me(),document.body.appendChild(P))}function me(){if(S)if(xe(),_){S.classList.remove("nf-hidden"),S.classList.add("nf-visible"),P&&(P.innerHTML="✕"),_=!1;try{localStorage.removeItem("nf_overlay_hidden")}catch{}}else{S.classList.remove("nf-visible"),S.classList.add("nf-hidden"),P&&(P.innerHTML="⚡"),_=!0;try{localStorage.setItem("nf_overlay_hidden","1")}catch{}}}const le={red:"themes/theme-red.jpg",yellow:"themes/theme-yellow.jpg",blue:"themes/theme-blue.jpg",purple:"themes/theme-purple.jpg",green:"themes/theme-green.jpg"};function he(){const n=document.getElementById("nf-core-monitor");if(!n)return;let e=j;if(!e)try{e=localStorage.getItem("netflow_app_theme")||"red"}catch{e="red"}const i=le[e]||le.red;let r;try{r=chrome.runtime.getURL(i)}catch{r=`/${i}`}const a=E.rgb;n.style.backgroundImage=["linear-gradient(180deg, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.78) 45%, rgba(0,0,0,0.90) 100%)",`linear-gradient(180deg, rgba(${a},0.25) 0%, rgba(${a},0.12) 50%, rgba(${a},0.20) 100%)`,`url('${r}')`].join(", "),n.style.backgroundSize="auto, auto, cover",n.style.backgroundPosition="center, center, center",n.style.backgroundRepeat="no-repeat, no-repeat, no-repeat",n.style.setProperty("--nf-bg-set","1"),n.style.border=`1.5px solid rgba(${a},0.45)`,n.style.boxShadow=`0 0 70px rgba(${a},0.22), 0 0 140px rgba(${a},0.1), inset 0 1px 0 rgba(${a},0.15)`}function Fe(n=1){if(E=we(),ce(),S&&S.isConnected)return;if(S&&!S.isConnected&&(S=null),G&&(G.remove(),G=null),pe(),W=n,I=Z(n),n>1){const i=H.find(a=>a.id==="video");if(i){const a=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:"waiting",progress:0}];for(let f=2;f<=n;f++)a.push({id:`scene${f}-prompt`,label:`Scene ${f} Prompt`,status:"waiting"}),a.push({id:`scene${f}-gen`,label:`Scene ${f} Generate`,status:"waiting"}),a.push({id:`scene${f}-wait`,label:`Scene ${f} รอผล`,status:"waiting",progress:0});i.steps=a}const r=H.find(a=>a.id==="render");if(r){const a=r.steps.find(l=>l.id==="download");a&&(a.label="ดาวน์โหลด 720p");const f=r.steps.find(l=>l.id==="upscale");f&&(f.label="Full Video")}}S=ke(),document.body.appendChild(S),!!localStorage.getItem("nf_overlay_hidden")?(S.classList.remove("nf-visible"),S.classList.add("nf-hidden"),_=!0):_=!1,xe(),Ce(),Pe(),requestAnimationFrame(()=>he())}const Le={settings:"CONFIG","upload-char":"UPLOAD","upload-prod":"UPLOAD","img-prompt":"PROMPT","img-generate":"GENERATE","img-wait":"IMG WAIT",animate:"ANIMATE","vid-prompt":"VID PROMPT","vid-generate":"VID GEN","vid-wait":"VID WAIT",download:"DOWNLOAD",upscale:"UPSCALE",open:"OPENING"};function Te(n,e,i){const r=I.filter(o=>o.status==="done").length,a=I.length,f=document.getElementById("nf-stat-step");f&&(f.textContent=`${r}/${a}`);const l=document.getElementById("nf-stat-scenes");if(l&&(l.textContent=W>1?`1/${W}`:"1/1"),e==="active"){const o=document.getElementById("nf-stat-status"),s=Le[n]||n.toUpperCase();o&&(o.textContent=s)}else if(e==="done"&&r>=a){const o=document.getElementById("nf-stat-status");o&&(o.textContent="COMPLETE")}else if(e==="error"){const o=document.getElementById("nf-stat-status");o&&(o.textContent="ERROR")}}function N(n,e,i){if(!S)return;for(const a of H)for(const f of a.steps)f.id===n&&(f.status=e);for(const a of I)a.stepId===n&&(a.status=e);const r=document.getElementById(`nf-step-${n}`);r&&(r.className="nf-step",e==="active"?r.classList.add("nf-step-active"):e==="done"?r.classList.add("nf-step-done"):e==="error"&&r.classList.add("nf-step-error")),Te(n,e),ue(),be()}function ue(){for(const n of H){const e=n.steps.filter(s=>s.status!=="skipped").length,i=n.steps.filter(s=>s.status==="done").length,r=n.steps.some(s=>s.status==="active"),a=e>0?Math.round(i/e*100):0,f=document.getElementById(`nf-pct-${n.id}`);f&&(f.textContent=`${a}%`);const l=document.getElementById(`nf-modbar-${n.id}`);l&&(l.style.width=`${a}%`);const o=document.getElementById(`nf-mod-${n.id}`);o&&(o.classList.remove("nf-active","nf-done"),a>=100?o.classList.add("nf-done"):r&&o.classList.add("nf-active"))}}function Re(n){var r,a,f,l;W=n;const e=new Map;for(const o of I)e.set(o.stepId,{status:o.status,progress:o.progress});I=Z(n);for(const o of I){const s=e.get(o.stepId);s&&(o.status=s.status,s.progress!==void 0&&(o.progress=s.progress))}ve();{const o=H.find(s=>s.id==="video");if(o){const s=[{id:"animate",label:"สลับเป็นโหมดวิดีโอ",status:((r=o.steps.find(d=>d.id==="animate"))==null?void 0:r.status)||"waiting"},{id:"vid-prompt",label:"Scene 1 Prompt",status:((a=o.steps.find(d=>d.id==="vid-prompt"))==null?void 0:a.status)||"waiting"},{id:"vid-generate",label:"Scene 1 Generate",status:((f=o.steps.find(d=>d.id==="vid-generate"))==null?void 0:f.status)||"waiting"},{id:"vid-wait",label:"Scene 1 รอผล",status:((l=o.steps.find(d=>d.id==="vid-wait"))==null?void 0:l.status)||"waiting",progress:0}];for(let d=2;d<=n;d++)s.push({id:`scene${d}-prompt`,label:`Scene ${d} Prompt`,status:"waiting"}),s.push({id:`scene${d}-gen`,label:`Scene ${d} Generate`,status:"waiting"}),s.push({id:`scene${d}-wait`,label:`Scene ${d} รอผล`,status:"waiting",progress:0});o.steps=s,de(o)}}const i=H.find(o=>o.id==="render");if(i&&n>1){const o=i.steps.find(d=>d.id==="download");o&&(o.label="ดาวน์โหลด 720p");const s=i.steps.find(d=>d.id==="upscale");s&&(s.label="Full Video"),de(i)}ue(),be()}function de(n){const e=document.getElementById(`nf-mod-${n.id}`);if(!e)return;e.querySelectorAll(".nf-step, .nf-mod-progress").forEach(a=>a.remove()),n.steps.forEach(a=>{const f=document.createElement("div");f.className="nf-step",f.id=`nf-step-${a.id}`;let l="";a.progress!==void 0&&(l=`
                <div class="nf-progress-bar">
                    <div class="nf-progress-fill" id="nf-bar-${a.id}" style="width: 0%"></div>
                </div>
            `),f.innerHTML=`
            ...
            <span class="nf-step-label">${a.label}</span>
            ${l}
        `,e.appendChild(f)});const r=document.createElement("div");r.className="nf-mod-progress",r.innerHTML=`<div class="nf-mod-progress-fill" id="nf-modbar-${n.id}"></div>`,e.appendChild(r)}function O(n){n.replace(/^\[Netflow AI\]\s*/,"")}window.chrome={runtime:{sendMessage:()=>{},lastError:null},storage:{local:{get:(n,e)=>e==null?void 0:e({}),set:()=>{}}}};const Q=new URLSearchParams(location.search).get("theme")||"red";$e(Q);Re(2);Fe();setTimeout(()=>{O("🚀 Overlay preview started — theme: "+Q),N("settings","active")},400);setTimeout(()=>{N("settings","done"),N("upload","active"),O("✅ Settings configured")},1600);setTimeout(()=>{N("upload","done"),N("prompt","active"),O("✅ Images uploaded (2 refs)")},3e3);setTimeout(()=>{N("prompt","done"),N("generate","active"),O("✅ Prompt injected"),O("⏳ Waiting for Scene 1 generation...")},4500);setTimeout(()=>{N("generate","done"),N("scene2","active"),O("✅ Scene 1 generated"),O("⏳ Generating Scene 2...")},7e3);const ee=document.createElement("div");ee.style.cssText=`
...
`;const Ae=["red","blue","green","yellow","purple"],q={red:"#ef4444",blue:"#3b82f6",green:"#00ff41",yellow:"#eab308",purple:"#8b5cf6"};Ae.forEach(n=>{const e=document.createElement("button");e.textContent=n,e.style.cssText=`
        ...
        ${n===Q?"opacity:1; box-shadow: 0 0 8px "+q[n]:"opacity:0.5"};
    `,e.onclick=()=>location.search="?theme="+n,ee.appendChild(e)});document.body.appendChild(ee);
